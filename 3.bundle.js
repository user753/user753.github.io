webpackJsonpNewGames([3],{

/***/ 144:
/*!***********************************************!*\
  !*** ./src/games/polyglot/wordsearch/meta.ts ***!
  \***********************************************/
/*! exports provided: gameId, metaData */
/*! all exports used */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "gameId", function() { return gameId; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "metaData", function() { return metaData; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__assets__ = __webpack_require__(/*! ./assets */ 200);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__components_mainComponent__ = __webpack_require__(/*! ./components/mainComponent */ 304);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__levels__ = __webpack_require__(/*! ./levels */ 307);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__logic__ = __webpack_require__(/*! ./logic */ 308);




var gameId = "wordsearch";
var scoreOptions = {
    scoreAnswer: 100,
    scoreBonus: 0,
    onlyFullAnswer: true,
    scoreBonusPerLevel: 5
};
var levelOptions = {
    levels: __WEBPACK_IMPORTED_MODULE_2__levels__["a" /* levels */],
    levelsSeqCounts: [{ seqCount: 3, maxLevel: 100 }]
};
var metaData = {
    gameId: gameId,
    title: "Word search",
    logic: __WEBPACK_IMPORTED_MODULE_3__logic__["a" /* Logic */],
    component: __WEBPACK_IMPORTED_MODULE_1__components_mainComponent__["a" /* MainComponent */],
    scoreOptions: scoreOptions,
    levelOptions: levelOptions,
    ignoreError: true,
    noResetSeqCountOnError: true,
    darkBg: true,
    sounds: __WEBPACK_IMPORTED_MODULE_0__assets__["c" /* Sounds */].keys,
    images: __WEBPACK_IMPORTED_MODULE_0__assets__["b" /* Images */].data,
    tutorialTaskCount: 3,
    atlasCount: 4,
    size: {
        width: 2048,
        height: 1280,
        heightMin: 1152
    }
};


/***/ }),

/***/ 154:
/*!*********************************************!*\
  !*** ./src/core/ui/controls/GraphicsImg.ts ***!
  \*********************************************/
/*! exports provided: GraphicsImg, BtnDefaultImg */
/*! exports used: BtnDefaultImg, GraphicsImg */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return GraphicsImg; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return BtnDefaultImg; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_ui_controls_control__ = __webpack_require__(/*! core/ui/controls/control */ 11);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_services_instances__ = __webpack_require__(/*! core/ui/services/instances */ 1);




var GraphicsImg = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](GraphicsImg, _super);
    function GraphicsImg(draw) {
        var _this = _super.call(this) || this;
        _this.draw = draw;
        _this.img = _this.add(__WEBPACK_IMPORTED_MODULE_3_core_ui_services_instances__["b" /* phaserService */].drawSprite(draw));
        return _this;
    }
    GraphicsImg.prototype.updateGraphics = function () {
        if (this.game) {
            var graphics = this.game.make.graphics(0, 0);
            this.draw(graphics);
            this.img.loadTexture(graphics.generateTexture());
            graphics.destroy(true);
        }
    };
    return GraphicsImg;
}(__WEBPACK_IMPORTED_MODULE_2_core_ui_controls_control__["a" /* Control */]));

var BtnDefaultImg = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](BtnDefaultImg, _super);
    function BtnDefaultImg(props) {
        var _this = this;
        var size = props.size || __WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__["a" /* Sizes */].create({ width: 500, height: 100 });
        _this = _super.call(this, function (g) {
            g.beginFill(props.color);
            g.drawRoundedRect(0, 0, size.width, size.height, size.height / 2);
        }) || this;
        return _this;
    }
    return BtnDefaultImg;
}(GraphicsImg));



/***/ }),

/***/ 155:
/*!***************************************************************!*\
  !*** ./src/core/ui/components/games/gameUI/panelComponent.ts ***!
  \***************************************************************/
/*! exports provided: PanelComponent */
/*! exports used: PanelComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return PanelComponent; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_ui_components_component__ = __webpack_require__(/*! core/ui/components/component */ 12);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_controls_GraphicsImg__ = __webpack_require__(/*! core/ui/controls/GraphicsImg */ 154);




var PanelComponent = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](PanelComponent, _super);
    // private control: NinePatchComponent
    function PanelComponent() {
        var _this = _super.call(this) || this;
        _this.positions = __WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__["a" /* Sizes */].create({
            radius: 20,
        });
        // this.control = this.add(new NinePatchComponent(CoreImages.panelList))
        _this.control = _this.add(new __WEBPACK_IMPORTED_MODULE_3_core_ui_controls_GraphicsImg__["b" /* GraphicsImg */](function (g) {
            if (_this.props) {
                g.beginFill(_this.props.color, _this.props.alpha || 0.4);
                g.drawRoundedRect(0, 0, _this.props.size.width, _this.props.size.height, _this.positions.radius);
                g.endFill();
            }
        }));
        return _this;
    }
    PanelComponent.prototype.update = function () {
    };
    PanelComponent.prototype.onPropsUpdate = function () {
        this.control.updateGraphics();
        // this.control.setSize(this.props.size)
    };
    return PanelComponent;
}(__WEBPACK_IMPORTED_MODULE_2_core_ui_components_component__["a" /* Component */]));



/***/ }),

/***/ 156:
/*!***************************************************************!*\
  !*** ./src/core/ui/components/games/baseMainGameComponent.ts ***!
  \***************************************************************/
/*! exports provided: durations, BaseMainGameComponent */
/*! exports used: BaseMainGameComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* unused harmony export durations */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return BaseMainGameComponent; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_assets__ = __webpack_require__(/*! core/assets */ 5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_assets_assetUtl__ = __webpack_require__(/*! core/assets/assetUtl */ 35);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_logic_sys__ = __webpack_require__(/*! core/logic/sys */ 6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_core_services__ = __webpack_require__(/*! core/services */ 7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_core_ui_animations_signals__ = __webpack_require__(/*! core/ui/animations/signals */ 22);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_core_ui_components_answers_answerControlType__ = __webpack_require__(/*! core/ui/components/answers/answerControlType */ 65);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_core_ui_components_games_gameComponent__ = __webpack_require__(/*! core/ui/components/games/gameComponent */ 33);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__ = __webpack_require__(/*! core/ui/services/instances */ 1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9_dev__ = __webpack_require__(/*! dev */ 13);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10_mobx__ = __webpack_require__(/*! mobx */ 10);











var bgLeftKey = "bg_left";
var introInterval = 2000;
var introDelay = 300;
var durations = {
    introAnimationDelay: 100,
    introInterval: introInterval,
    introDelay: introDelay
};
var BaseMainGameComponent = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](BaseMainGameComponent, _super);
    function BaseMainGameComponent(context) {
        var _this = _super.call(this, context) || this;
        _this.statusOptions = {
            offset: 0,
            waitCompleteBeforeNewTask: false,
        };
        _this.introOptions = {
            disableSound: false,
            runOnNewTaskAfter: false
        };
        _this.answersControl = __WEBPACK_IMPORTED_MODULE_6_core_ui_components_answers_answerControlType__["a" /* AnswerControlType */].None;
        context.uiService.rootComponent = _this;
        context.gameState = _this.initialState();
        _this.fixedToCamera = true;
        _this.onDestroy.addOnce(function () {
            _this.context.uiService.mainComponent.onNewTask.dispose();
            _this.context.uiService.mainComponent.onAnswer.dispose();
            __WEBPACK_IMPORTED_MODULE_4_core_services__["d" /* audioService */].stopAll();
        });
        _this.onChangeForRun(function (x) { return __WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__["e" /* screenSizeService */].screenSize; }, function (x) {
            _this.cameraOffset.y = __WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__["e" /* screenSizeService */].topOffset;
        });
        return _this;
    }
    BaseMainGameComponent.prototype.initialState = function () {
        return {};
    };
    Object.defineProperty(BaseMainGameComponent.prototype, "cameraBottom", {
        get: function () {
            var bottom = __WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__["e" /* screenSizeService */].height - this.cameraOffset.y;
            return bottom;
        },
        enumerable: true,
        configurable: true
    });
    BaseMainGameComponent.prototype.prepareDefaultIntroAnimation = function (delay) {
        var _this = this;
        if (delay === void 0) { delay = durations.introDelay; }
        var bgLeftOffset = 0;
        var bgLeft = undefined;
        if (!__WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__["e" /* screenSizeService */].isPortrait && __WEBPACK_IMPORTED_MODULE_2_core_assets_assetUtl__["a" /* AssetUtl */].checkImageKey(this.gameKey(bgLeftKey))) {
            bgLeft = this.addImage([bgLeftKey, [0, 0]]);
            var width = -bgLeft.width + 2;
            bgLeft.x = width;
            bgLeftOffset = -width;
        }
        this.cameraOffset.x = -__WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__["e" /* screenSizeService */].realLeftOffset + bgLeftOffset;
        var tween = this.game.add.tween(this.cameraOffset).to({ x: 0 }, durations.introInterval, Phaser.Easing.Cubic.InOut, false, delay);
        if (bgLeft) {
            tween.onComplete.addOnce(function () { return _this.remove(bgLeft, true); });
        }
        return tween;
    };
    BaseMainGameComponent.prototype.introAnimation = function () {
        var tween = this.prepareDefaultIntroAnimation();
        return tween;
    };
    BaseMainGameComponent.prototype.startIntro = function () {
        var _this = this;
        if (__WEBPACK_IMPORTED_MODULE_3_core_logic_sys__["a" /* Sys */].devMode && !__WEBPACK_IMPORTED_MODULE_9_dev__["a" /* Dev */].devModeIntro) {
            this.context.uiService.mainComponent.onIntroComplete.dispatch();
            return __WEBPACK_IMPORTED_MODULE_5_core_ui_animations_signals__["a" /* Signals */].memorize();
        }
        if (!this.introOptions.disableSound) {
            this.playSound(__WEBPACK_IMPORTED_MODULE_1_core_assets__["e" /* CoreSounds */].intro, false, true);
        }
        var tween = this.introAnimation();
        var signal = __WEBPACK_IMPORTED_MODULE_5_core_ui_animations_signals__["a" /* Signals */].bind(this.delay(durations.introAnimationDelay), function () {
            tween.start();
            return tween.onComplete;
        });
        signal.addOnce(function () { return _this.context.uiService.mainComponent.onIntroComplete.dispatch(); });
        this.disableInputOn(signal);
        return signal;
    };
    BaseMainGameComponent.prototype.onAfterInit = function () {
        this.initTaskRendering();
    };
    BaseMainGameComponent.prototype.initTaskRendering = function () {
        var _this = this;
        this.onChangeFor(function () { return _this.gameState.lastAnswer; }, function (x) {
            _this.context.uiService.mainComponent.onAnswer.dispatch(x);
        });
        this.onChangeFor(function () { return _this.context.gameService.gameCycleService.state.levelTask.task; }, function (x) {
            if (_this.statusOptions.waitCompleteBeforeNewTask) {
                _this.disableInputOn(_this.context.uiService.answerStatus.onAnimationComplete).addOnce(function () {
                    _this.callRenderNewTask(x);
                });
            }
            else {
                _this.callRenderNewTask(x);
            }
        });
        this.callRenderNewTask(this.context.gameService.gameCycleService.state.levelTask.task);
    };
    BaseMainGameComponent.prototype.callRenderNewTask = function (task) {
        var _this = this;
        if (this.introOptions.runOnNewTaskAfter && this.isFirstTask) {
            this.context.uiService.mainComponent.onIntroComplete.addOnce(function () {
                _this.context.uiService.mainComponent.onNewTask.dispatch(Object(__WEBPACK_IMPORTED_MODULE_10_mobx__["f" /* toJS */])(task));
            });
        }
        else {
            this.context.uiService.mainComponent.onNewTask.dispatch(Object(__WEBPACK_IMPORTED_MODULE_10_mobx__["f" /* toJS */])(task));
        }
    };
    BaseMainGameComponent.prototype.playBackgroundLoop = function () {
        var _this = this;
        this.context.uiService.mainComponent.onIntroComplete.addOnce(function () { return _this.playSound("bg", true); });
    };
    return BaseMainGameComponent;
}(__WEBPACK_IMPORTED_MODULE_7_core_ui_components_games_gameComponent__["a" /* GameComponent */]));



/***/ }),

/***/ 157:
/*!*************************************!*\
  !*** ./src/core/utils/circleSeq.ts ***!
  \*************************************/
/*! exports provided: CircleSeq, UniqSeq */
/*! exports used: CircleSeq, UniqSeq */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return CircleSeq; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return UniqSeq; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_core_utils_randomGenerator__ = __webpack_require__(/*! core/utils/randomGenerator */ 64);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_utils_undef__ = __webpack_require__(/*! core/utils/undef */ 8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2_lodash__);



var CircleSeq = /** @class */ (function () {
    function CircleSeq(array) {
        this.array = array;
        this.index = 0;
    }
    CircleSeq.prototype.next = function () {
        var index = this.index;
        this.index = (this.index + 1) % this.array.length;
        return this.array[index];
    };
    CircleSeq.prototype.nextArray = function (count) {
        var _this = this;
        return __WEBPACK_IMPORTED_MODULE_2_lodash__["range"](count).map(function () { return _this.next(); });
    };
    return CircleSeq;
}());

var UniqSeq = /** @class */ (function () {
    function UniqSeq(max, rng) {
        if (rng === void 0) { rng = new __WEBPACK_IMPORTED_MODULE_0_core_utils_randomGenerator__["a" /* RandomGenerator */](); }
        this.max = max;
        this.rng = rng;
        this.index = 0;
        this.list = [];
        this.reset();
    }
    UniqSeq.prototype.checkMax = function (newMax) {
        if (newMax != this.max) {
            this.max = newMax;
            this.reset();
        }
        return this;
    };
    UniqSeq.prototype.reset = function () {
        var newList = this.rng.uniqArray(this.max, this.max);
        if (this.list.length > 0) {
            var last = __WEBPACK_IMPORTED_MODULE_2_lodash__["drop"](this.list, Math.floor(this.list.length * 0.6));
            var first = __WEBPACK_IMPORTED_MODULE_2_lodash__["difference"](newList, last);
            newList = first.concat(this.rng.shuffle(last));
        }
        this.list = newList;
    };
    UniqSeq.prototype.updateIndex = function () {
        this.index += 1;
        if (this.index >= this.list.length) {
            this.reset();
            this.index = 0;
        }
    };
    UniqSeq.prototype.nextArray = function (count) {
        var _this = this;
        return __WEBPACK_IMPORTED_MODULE_2_lodash__["range"](count).map(function () { return _this.next(); });
    };
    UniqSeq.prototype.next = function (max) {
        if (max) {
            this.checkMax(max);
        }
        var v = this.list[this.index];
        this.updateIndex();
        return v;
    };
    UniqSeq.prototype.nextSkip = function (v) {
        var n = this.next();
        if (this.list.length > 1 && n == v) {
            return this.nextSkip(v);
        }
        return n;
    };
    UniqSeq.prototype.nextFrom = function (arr, skip) {
        this.checkMax(arr.length);
        return arr[__WEBPACK_IMPORTED_MODULE_1_core_utils_undef__["a" /* Undef */].is(skip) ? this.next() : this.nextSkip(arr.indexOf(skip))];
    };
    UniqSeq.prototype.nextSubsetFrom = function (arr, count) {
        var _this = this;
        return __WEBPACK_IMPORTED_MODULE_2_lodash__["range"](count).map(function () { return _this.nextFrom(arr); });
    };
    return UniqSeq;
}());



/***/ }),

/***/ 158:
/*!**********************************************************************!*\
  !*** ./src/core/ui/components/games/gameUI/tutorialTextComponent.ts ***!
  \**********************************************************************/
/*! exports provided: TutorialTextComponent */
/*! exports used: TutorialTextComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return TutorialTextComponent; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_assets_index__ = __webpack_require__(/*! core/assets/index */ 5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_assets_images__ = __webpack_require__(/*! core/assets/images */ 46);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_commonAnimations__ = __webpack_require__(/*! core/ui/animations/commonAnimations */ 23);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_core_ui_components_games_gameComponent__ = __webpack_require__(/*! core/ui/components/games/gameComponent */ 33);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_core_ui_components_games_gameUI_panelComponent__ = __webpack_require__(/*! core/ui/components/games/gameUI/panelComponent */ 155);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_core_ui_controls_index__ = __webpack_require__(/*! core/ui/controls/index */ 4);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_core_ui_controls_btn__ = __webpack_require__(/*! core/ui/controls/btn */ 61);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9_core_ui_services_instances__ = __webpack_require__(/*! core/ui/services/instances */ 1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10_core_utils_undef__ = __webpack_require__(/*! core/utils/undef */ 8);











var TutorialTextComponent = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](TutorialTextComponent, _super);
    function TutorialTextComponent() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.positions = __WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__["a" /* Sizes */].create({
            marginTop: 40,
            textMarginX: 40,
            padding: 80,
            buttonMargin: 10,
            btnWidth: 200,
            btn: { x: 250, y: 90 }
        });
        _this.positionsPortrait = __WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__["a" /* Sizes */].update(_this.positions, {
            marginTop: 20,
            textMarginX: 40,
            padding: 40,
        });
        _this.getPositions = function () { return __WEBPACK_IMPORTED_MODULE_9_core_ui_services_instances__["e" /* screenSizeService */].isPortrait ? _this.positionsPortrait : _this.positions; };
        _this.marginTop = 0;
        return _this;
    }
    TutorialTextComponent.prototype.make = function (props) {
        var _this = this;
        if (props === void 0) { props = {}; }
        this.marginTop = props.marginTop || this.getPositions().marginTop;
        this.minHeight = props.minHeight;
        this.panel = props.panel || {};
        this.back = this.add(new __WEBPACK_IMPORTED_MODULE_6_core_ui_components_games_gameUI_panelComponent__["a" /* PanelComponent */]()).anchorCenterX();
        this.text = this.add(new __WEBPACK_IMPORTED_MODULE_7_core_ui_controls_index__["g" /* Txt */]("", __WEBPACK_IMPORTED_MODULE_0_tslib__["a" /* __assign */]({}, __WEBPACK_IMPORTED_MODULE_1_core_assets_index__["c" /* CoreFonts */].tutorial(false), { align: props.textAlign || "left" }))).anchorCenter();
        var stepCount = props.stepCount || 1;
        if (stepCount > 1) {
            this.tutorialStore.setStepCount(stepCount);
            this.nextBtn = this.back.add(new __WEBPACK_IMPORTED_MODULE_8_core_ui_controls_btn__["b" /* IconBtn */]("далее", __WEBPACK_IMPORTED_MODULE_2_core_assets_images__["a" /* CoreImages */].next, function () {
                _this.tutorialStore.nextTutorialState();
            }, this.positions.buttonMargin));
            __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_commonAnimations__["a" /* CommonAnimations */].highlight(this.nextBtn);
            this.onChangeForRun(function () { return _this.tutorialStore.isLastStep; }, function (isLastStep) {
                _this.nextBtn.visible = !isLastStep;
            });
        }
        this.onChangeForRun(function () { return __WEBPACK_IMPORTED_MODULE_9_core_ui_services_instances__["e" /* screenSizeService */].screenSize; }, function (_a) {
            var width = _a.width, height = _a.height;
            // const btnWidth = this.nextBtn ? this.positions.btnWidth : 0
            var btnWidth = 0;
            _this.text.textControl.wordWrapWidth = __WEBPACK_IMPORTED_MODULE_9_core_ui_services_instances__["e" /* screenSizeService */].realWidth - _this.getPositions().textMarginX * 2 - btnWidth;
            _this.updateSize();
        });
        return this;
    };
    TutorialTextComponent.prototype.updateSize = function () {
        var textHeight = this.text.height + this.getPositions().padding;
        var height = this.minHeight ? Math.max(this.minHeight, textHeight) : textHeight;
        this.back.updatePos(__WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__["a" /* Sizes */].w2, this.marginTop - __WEBPACK_IMPORTED_MODULE_9_core_ui_services_instances__["e" /* screenSizeService */].topOffset);
        var width = __WEBPACK_IMPORTED_MODULE_10_core_utils_undef__["a" /* Undef */].map(this.panel.size, function (x) { return x.width; }) || __WEBPACK_IMPORTED_MODULE_9_core_ui_services_instances__["e" /* screenSizeService */].realWidth - this.getPositions().padding;
        this.back.setProps(__WEBPACK_IMPORTED_MODULE_0_tslib__["a" /* __assign */]({}, this.panel, { size: { width: width, height: height } }));
        this.text.updatePos(__WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__["a" /* Sizes */].w2, this.back.height / 2 + this.back.y);
        this.text.updatePivot();
        this.back.visible = !!this.text.text;
        if (this.nextBtn) {
            this.nextBtn.updatePos(this.back.width - this.positions.btn.x, this.back.height - this.positions.btn.y);
        }
        this.back.updatePivot();
    };
    TutorialTextComponent.prototype.onPropsUpdate = function () {
        this.text.text = this.props;
        this.updateSize();
    };
    return TutorialTextComponent;
}(__WEBPACK_IMPORTED_MODULE_5_core_ui_components_games_gameComponent__["a" /* GameComponent */]));



/***/ }),

/***/ 162:
/*!*****************************************************************!*\
  !*** ./src/core/ui/components/games/introAnimationComponent.ts ***!
  \*****************************************************************/
/*! exports provided: IntroAnimationComponent */
/*! exports used: IntroAnimationComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return IntroAnimationComponent; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_ui_animations_tweens__ = __webpack_require__(/*! core/ui/animations/tweens */ 28);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_components_index__ = __webpack_require__(/*! core/ui/components/index */ 29);




var durations = {
    animation: 1000,
    delay: 700
};
var IntroAnimationComponent = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](IntroAnimationComponent, _super);
    function IntroAnimationComponent() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.positions = __WEBPACK_IMPORTED_MODULE_2_core_ui_common_sizes__["a" /* Sizes */].create({
            wordMargin: { x: 20, y: 0 }
        });
        return _this;
    }
    IntroAnimationComponent.prototype.onInit = function () {
        this.addImage("bg_intro");
    };
    IntroAnimationComponent.prototype.startAnimation = function (notStart) {
        var _this = this;
        if (notStart === void 0) { notStart = false; }
        return __WEBPACK_IMPORTED_MODULE_1_core_ui_animations_tweens__["a" /* Tweens */].start({
            target: this,
            to: { symmetricScale: 3.5, alpha: 0 },
            delay: durations.delay,
            duration: durations.animation,
            notStart: notStart,
            easing: Phaser.Easing.Sinusoidal.InOut,
            onComplete: function () { return _this.destroy(true); }
        });
    };
    IntroAnimationComponent.durations = durations;
    return IntroAnimationComponent;
}(__WEBPACK_IMPORTED_MODULE_3_core_ui_components_index__["e" /* GameComponent */]));



/***/ }),

/***/ 164:
/*!*************************************************************************!*\
  !*** ./src/core/ui/controls/behaviors/introComplexAnimationBehavior.ts ***!
  \*************************************************************************/
/*! exports provided: IntroComplexAnimationBehavior */
/*! exports used: IntroComplexAnimationBehavior */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return IntroComplexAnimationBehavior; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_ui_components_games_introAnimationComponent__ = __webpack_require__(/*! core/ui/components/games/introAnimationComponent */ 162);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_controls_behaviors_behavior__ = __webpack_require__(/*! core/ui/controls/behaviors/behavior */ 30);




var IntroComplexAnimationBehavior = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](IntroComplexAnimationBehavior, _super);
    function IntroComplexAnimationBehavior() {
        return _super.call(this) || this;
    }
    IntroComplexAnimationBehavior.prototype.onInit = function () {
        var duration = __WEBPACK_IMPORTED_MODULE_2_core_ui_components_games_introAnimationComponent__["a" /* IntroAnimationComponent */].durations.delay;
        this.tween = this.control.prepareDefaultIntroAnimation(duration);
        this.intro = this.control.add(this.control.createComponent(__WEBPACK_IMPORTED_MODULE_2_core_ui_components_games_introAnimationComponent__["a" /* IntroAnimationComponent */]))
            .anchorCenter().updatePos(__WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__["a" /* Sizes */].w2, __WEBPACK_IMPORTED_MODULE_1_core_ui_common_sizes__["a" /* Sizes */].h2);
        this.intro.x -= this.control.cameraOffset.x;
        this.onComplete = this.tween.onComplete;
    };
    IntroComplexAnimationBehavior.prototype.start = function () {
        this.intro.startAnimation();
        this.tween.start();
    };
    return IntroComplexAnimationBehavior;
}(__WEBPACK_IMPORTED_MODULE_3_core_ui_controls_behaviors_behavior__["a" /* Behavior */]));



/***/ }),

/***/ 183:
/*!**************************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator-core/src/index.js ***!
  \**************************************************************/
/*! dynamic exports provided */
/*! all exports used */
/***/ (function(module, exports, __webpack_require__) {

exports.GridMask = __webpack_require__(/*! ./GridMask */ 312);
exports.Maze = __webpack_require__(/*! ./Maze */ 313);
exports.directions = __webpack_require__(/*! ./directions */ 214);


/***/ }),

/***/ 190:
/*!***********************************!*\
  !*** ./src/core/utils/matrixs.ts ***!
  \***********************************/
/*! exports provided: Matrixs */
/*! exports used: Matrixs */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return Matrixs; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_core_utils_arrays__ = __webpack_require__(/*! core/utils/arrays */ 26);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_utils_tuples__ = __webpack_require__(/*! core/utils/tuples */ 24);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_utils_undef__ = __webpack_require__(/*! core/utils/undef */ 8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_lodash__);




var Matrixs;
(function (Matrixs) {
    Matrixs.matrixDirectionList = ["up", "down", "left", "right"];
    Matrixs.directions = {
        up: { x: 0, y: -1 },
        down: { x: 0, y: 1 },
        left: { x: -1, y: 0 },
        right: { x: 1, y: 0 }
    };
    Matrixs.oppositeDirections = {
        up: "down",
        down: "up",
        left: "right",
        right: "left"
    };
    function move(p, dirs) {
        console.log(p, dirs);
        return dirs.reduce(function (_a, v) {
            var x = _a[0], y = _a[1];
            var _b = Matrixs.directions[v], dx = _b.x, dy = _b.y;
            return Object(__WEBPACK_IMPORTED_MODULE_1_core_utils_tuples__["a" /* tuple */])(x + dx, y + dy);
        }, p);
    }
    Matrixs.move = move;
    function isInside(_a, _b) {
        var x = _a[0], y = _a[1];
        var width = _b.width, height = _b.height;
        return x >= 0 && y >= 0 && x < width && y < height;
    }
    Matrixs.isInside = isInside;
    function onBoard(_a, _b) {
        var x = _a[0], y = _a[1];
        var width = _b.width, height = _b.height;
        return x == 0 || y == 0 || x == width - 1 || y == height - 1;
    }
    Matrixs.onBoard = onBoard;
    function getSize(m) {
        return { width: m.length == 0 ? 0 : m[0].length, height: m.length };
    }
    Matrixs.getSize = getSize;
    function directionIsAvailable(dir, p, size, obstacles) {
        var newP = move(p, [dir]);
        return isInside(newP, size) && !__WEBPACK_IMPORTED_MODULE_0_core_utils_arrays__["a" /* Arrays */].includesWith(obstacles, newP);
    }
    Matrixs.directionIsAvailable = directionIsAvailable;
    function getAvailableDirections(p, size, obstacle) {
        return Matrixs.matrixDirectionList.filter(function (dir) { return directionIsAvailable(dir, p, size, obstacle); });
    }
    Matrixs.getAvailableDirections = getAvailableDirections;
    function get(m, _a, def) {
        var x = _a[0], y = _a[1];
        var row = m[y];
        return row ? row[x] : def;
    }
    Matrixs.get = get;
    function set(m, _a, value) {
        var x = _a[0], y = _a[1];
        var row = m[y];
        if (__WEBPACK_IMPORTED_MODULE_2_core_utils_undef__["a" /* Undef */].is(row)) {
            row = [];
            m[y] = row;
        }
        row[x] = value;
    }
    Matrixs.set = set;
    function compact(m) {
        return m.map(__WEBPACK_IMPORTED_MODULE_3_lodash__["compact"]);
    }
    Matrixs.compact = compact;
    function create(colCount, rowCount, fn) {
        return __WEBPACK_IMPORTED_MODULE_3_lodash__["range"](rowCount).map(function (y) { return __WEBPACK_IMPORTED_MODULE_3_lodash__["range"](colCount).map(function (x) { return fn(x, y); }); });
    }
    Matrixs.create = create;
    function map(m, fn) {
        return m.map(function (row, y) { return row.map(function (item, x) { return fn(item, x, y); }); });
    }
    Matrixs.map = map;
    function intersection(f, s) {
        var first = [];
        var second = [];
        var both = [];
        __WEBPACK_IMPORTED_MODULE_3_lodash__["range"](Math.max(f.length, s.length)).forEach(function (y) {
            __WEBPACK_IMPORTED_MODULE_3_lodash__["range"](Math.max((f[y] || []).length, (s[y] || []).length)).forEach(function (x) {
                var p = Object(__WEBPACK_IMPORTED_MODULE_1_core_utils_tuples__["a" /* tuple */])(x, y);
                var fe = get(f, p);
                var se = get(s, p);
                if (!__WEBPACK_IMPORTED_MODULE_2_core_utils_undef__["a" /* Undef */].is(fe) && !__WEBPACK_IMPORTED_MODULE_2_core_utils_undef__["a" /* Undef */].is(se)) {
                    both.push([fe, se, p]);
                }
                else if (!__WEBPACK_IMPORTED_MODULE_2_core_utils_undef__["a" /* Undef */].is(fe)) {
                    first.push([fe, p]);
                }
                else if (!__WEBPACK_IMPORTED_MODULE_2_core_utils_undef__["a" /* Undef */].is(se)) {
                    second.push([se, p]);
                }
            });
        });
        return Object(__WEBPACK_IMPORTED_MODULE_1_core_utils_tuples__["a" /* tuple */])(first, second, both);
    }
    Matrixs.intersection = intersection;
})(Matrixs || (Matrixs = {}));


/***/ }),

/***/ 200:
/*!*************************************************!*\
  !*** ./src/games/polyglot/wordsearch/assets.ts ***!
  \*************************************************/
/*! exports provided: Translations, Sounds, Fonts, Images */
/*! exports used: Fonts, Images, Sounds, Translations */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "d", function() { return Translations; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return Sounds; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return Fonts; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return Images; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_core_assets_fonts__ = __webpack_require__(/*! core/assets/fonts */ 34);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_lodash__);


var Translations;
(function (Translations) {
    Translations.title = "title";
})(Translations || (Translations = {}));
var Sounds;
(function (Sounds) {
    Sounds.arrow = "arrow";
    Sounds.perevorot = "perevorot";
    Sounds.bg = "bg";
    Sounds.letter = "letter";
    Sounds.povorot = "povorot";
    Sounds.visip = "visip";
    Sounds.keys = [
        Sounds.arrow,
        Sounds.perevorot,
        Sounds.bg,
        Sounds.letter,
        Sounds.povorot,
        Sounds.visip,
    ];
})(Sounds || (Sounds = {}));
var Fonts;
(function (Fonts) {
    var Colors;
    (function (Colors) {
        Colors.wrong = "#B22109";
        Colors.correct = "#7E42DC";
        Colors.correct2 = "#0989C9";
        Colors.normal = "#682321";
        Colors.gray = "#333333";
    })(Colors = Fonts.Colors || (Fonts.Colors = {}));
    Fonts.letter = {
        fontSize: 180, font: __WEBPACK_IMPORTED_MODULE_0_core_assets_fonts__["c" /* CoreFonts */].MontserratSemiBold, fill: Colors.normal
    };
})(Fonts || (Fonts = {}));
var Images;
(function (Images) {
    Images.bg = "bg";
    Images.arrow = "arrow";
    Images.circle = "circle";
    Images.cube = function (id, size) { return "cube_" + size + "_" + id; };
    Images.cubeList = function (size) { return __WEBPACK_IMPORTED_MODULE_1_lodash__["range"](11).map(function (i) { return Images.cube(i, size); }); };
    Images.data = {
        sprites: []
    };
})(Images || (Images = {}));


/***/ }),

/***/ 212:
/*!*******************************!*\
  !*** ./src/core/utils/utl.ts ***!
  \*******************************/
/*! exports provided: Utl */
/*! exports used: Utl */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return Utl; });
var Utl;
(function (Utl) {
    function swap(_a, swap) {
        var x = _a[0], y = _a[1];
        return swap ? [y, x] : [x, y];
    }
    Utl.swap = swap;
})(Utl || (Utl = {}));


/***/ }),

/***/ 213:
/*!*********************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator/src/index.js ***!
  \*********************************************************/
/*! dynamic exports provided */
/*! exports used: generate */
/***/ (function(module, exports, __webpack_require__) {

exports.generate = __webpack_require__(/*! ./generate */ 311);
exports.GridMask = __webpack_require__(/*! @sbj42/maze-generator-core */ 183).GridMask;


/***/ }),

/***/ 214:
/*!*******************************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator-core/src/directions.js ***!
  \*******************************************************************/
/*! dynamic exports provided */
/*! all exports used */
/***/ (function(module, exports) {

var dirs = {};

/**
 * @typedef {integer} Direction
 */

dirs.NORTH = 0;
dirs.EAST =  1;
dirs.SOUTH = 2;
dirs.WEST =  3;

dirs.ALL = [dirs.NORTH, dirs.EAST, dirs.SOUTH, dirs.WEST];

/**
 * Returns the opposite direction
 *
 * @param {Direction} dir
 * @return {Direction}
 */
dirs.opposite = function(dir) {
    switch (dir) {
        case 0/*dirs.NORTH*/: return 2;
        case 1/*dirs.EAST*/: return 3;
        case 2/*dirs.SOUTH*/: return 0;
        case 3/*dirs.WEST*/: return 1;
        default: throw new Error('bad direction: ' + dir);
    }
};

/**
 * Returns the x component of the direction vector
 *
 * @param {Direction} dir
 * @return {integer}
 */
dirs.dx = function(dir) {
    switch (dir) {
        case 0/*dirs.NORTH*/: return 0;
        case 1/*dirs.EAST*/: return 1;
        case 2/*dirs.SOUTH*/: return 0;
        case 3/*dirs.WEST*/: return -1;
        default: throw new Error('bad direction: ' + dir);
    }
};

/**
 * Returns the y component of the direction vector
 *
 * @param {Direction} dir
 * @return {integer}
 */
dirs.dy = function(dir) {
    switch (dir) {
        case 0/*dirs.NORTH*/: return -1;
        case 1/*dirs.EAST*/: return 0;
        case 2/*dirs.SOUTH*/: return 1;
        case 3/*dirs.WEST*/: return 0;
        default: throw new Error('bad direction: ' + dir);
    }
};

/**
 * Moves the coordinate in the given direction
 *
 * @param {integer} x
 * @param {integer} y
 * @param {Direction} dir
 * @return {integer[]}
 */
dirs.move = function(x, y, dir) {
    switch (dir) {
        case 0/*dirs.NORTH*/: return [x, y-1];
        case 1/*dirs.EAST*/: return [x+1, y];
        case 2/*dirs.SOUTH*/: return [x, y+1];
        case 3/*dirs.WEST*/: return [x-1, y];
        default: throw new Error('bad direction: ' + dir);
    }
};

/**
 * Returns a bitmask for the direction, useful for
 * storing a bunch of direction boolean values in
 * a single integer.
 *
 * @param {Direction} dir
 * @return {integer}
 */
dirs.bitmask = function(dir) {
    return 1 << dir;
};

module.exports = dirs;


/***/ }),

/***/ 304:
/*!*******************************************************************!*\
  !*** ./src/games/polyglot/wordsearch/components/mainComponent.ts ***!
  \*******************************************************************/
/*! exports provided: MainComponent */
/*! exports used: MainComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return MainComponent; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_types_logic__ = __webpack_require__(/*! core/types/logic */ 21);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_ui_animations_commonAnimations__ = __webpack_require__(/*! core/ui/animations/commonAnimations */ 23);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_tweens__ = __webpack_require__(/*! core/ui/animations/tweens */ 28);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_core_ui_components_games_baseMainGameComponent__ = __webpack_require__(/*! core/ui/components/games/baseMainGameComponent */ 156);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_core_ui_components_games_gameUI_tutorialTextComponent__ = __webpack_require__(/*! core/ui/components/games/gameUI/tutorialTextComponent */ 158);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_core_ui_controls_behaviors_introComplexAnimationBehavior__ = __webpack_require__(/*! core/ui/controls/behaviors/introComplexAnimationBehavior */ 164);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__ = __webpack_require__(/*! core/ui/services/instances */ 1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9_core_utils_randomGenerator__ = __webpack_require__(/*! core/utils/randomGenerator */ 64);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_assets__ = __webpack_require__(/*! games/polyglot/wordsearch/assets */ 200);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_11_games_polyglot_wordsearch_components_fieldComponent__ = __webpack_require__(/*! games/polyglot/wordsearch/components/fieldComponent */ 305);












var durations = {
    arrow: 1000
};
var MainComponent = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](MainComponent, _super);
    function MainComponent() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.positions = __WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__["a" /* Sizes */].create({
            circle: [270, 70],
            fieldOffset: { y: -20 }
        });
        _this.positionsPortrait = __WEBPACK_IMPORTED_MODULE_4_core_ui_common_sizes__["a" /* Sizes */].expand(_this.positions, {});
        _this.getPositions = function () { return __WEBPACK_IMPORTED_MODULE_8_core_ui_services_instances__["e" /* screenSizeService */].isPortrait ? _this.positionsPortrait : _this.positions; };
        _this.rng = new __WEBPACK_IMPORTED_MODULE_9_core_utils_randomGenerator__["a" /* RandomGenerator */]();
        return _this;
    }
    MainComponent.prototype.onInit = function () {
        this.statusOptions.waitCompleteBeforeNewTask = true;
        this.addImage([__WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_assets__["b" /* Images */].bg, [0, 0]]);
        this.addImage([__WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_assets__["b" /* Images */].circle, this.positions.circle]).anchorCenter();
        this.arrow = this.addImage([__WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_assets__["b" /* Images */].arrow, this.positions.circle]).anchorCenter();
        this.add(this.createComponent(__WEBPACK_IMPORTED_MODULE_11_games_polyglot_wordsearch_components_fieldComponent__["a" /* FieldComponent */])).anchorCenter()
            .updatePos(this.center()[0], this.center()[1] + this.positions.fieldOffset.y);
        this.startIntro();
        if (this.isTutorial) {
            var tutorial_1 = this.add(this.createComponent(__WEBPACK_IMPORTED_MODULE_6_core_ui_components_games_gameUI_tutorialTextComponent__["a" /* TutorialTextComponent */])).make().setProps(this.translate(__WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_assets__["d" /* Translations */].title));
            tutorial_1.visible = false;
            this.context.uiService.mainComponent.onIntroComplete.addOnce(function () { return __WEBPACK_IMPORTED_MODULE_2_core_ui_animations_commonAnimations__["a" /* CommonAnimations */].setVisible(tutorial_1, true); });
        }
        this.playBackgroundLoop();
        // const hint = this.add(this.createComponent(HintComponent)).defaultPosition()
        // hint.visible = false
        // if (!this.isTutorial) {
        //   this.services.uiService.mainComponent.onIntroComplete.addOnce(() => hint.visible = true)
        // }
        // this.services.uiService.hintComponent.onHint.addOnce(() => {
        //   this.playerAnswer("shuffle")
        // })
    };
    MainComponent.prototype.introAnimation = function () {
        var behavior = new __WEBPACK_IMPORTED_MODULE_7_core_ui_controls_behaviors_introComplexAnimationBehavior__["a" /* IntroComplexAnimationBehavior */]();
        this.addBehavior(behavior);
        return behavior;
    };
    MainComponent.prototype.onAnswer = function (answer) {
        if (answer.status == __WEBPACK_IMPORTED_MODULE_1_core_types_logic__["a" /* AnswerStatus */].Correct) {
            this.animationArrow(1);
        }
    };
    MainComponent.prototype.animationArrow = function (rounds) {
        this.playSound(__WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_assets__["c" /* Sounds */].arrow);
        this.arrow.angle = 20;
        __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_tweens__["a" /* Tweens */].start({
            to: { angle: this.arrow.angle + 360 * rounds },
            target: this.arrow,
            duration: durations.arrow * rounds,
            easing: Phaser.Easing.Cubic.Out
        });
    };
    MainComponent.prototype.onNewTask = function (task) {
        var _this = this;
        this.onIntroCompleteOrMemorize.addOnce(function () {
            if (_this.isNewLevel) {
                _this.animationArrow(2);
            }
        });
    };
    return MainComponent;
}(__WEBPACK_IMPORTED_MODULE_5_core_ui_components_games_baseMainGameComponent__["a" /* BaseMainGameComponent */]));



/***/ }),

/***/ 305:
/*!********************************************************************!*\
  !*** ./src/games/polyglot/wordsearch/components/fieldComponent.ts ***!
  \********************************************************************/
/*! exports provided: FieldComponent */
/*! exports used: FieldComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return FieldComponent; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_logic_sys__ = __webpack_require__(/*! core/logic/sys */ 6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_types_logic__ = __webpack_require__(/*! core/types/logic */ 21);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_commonAnimations__ = __webpack_require__(/*! core/ui/animations/commonAnimations */ 23);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_core_ui_animations_signals__ = __webpack_require__(/*! core/ui/animations/signals */ 22);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_core_ui_animations_tweens__ = __webpack_require__(/*! core/ui/animations/tweens */ 28);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_core_ui_common_sizes__ = __webpack_require__(/*! core/ui/common/sizes */ 3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_core_ui_components_index__ = __webpack_require__(/*! core/ui/components/index */ 29);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_core_ui_controls_behaviors_positionShiftBehavior__ = __webpack_require__(/*! core/ui/controls/behaviors/positionShiftBehavior */ 306);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9_core_ui_controls_btn__ = __webpack_require__(/*! core/ui/controls/btn */ 61);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10_core_ui_controls_controlUtl__ = __webpack_require__(/*! core/ui/controls/controlUtl */ 63);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__ = __webpack_require__(/*! core/ui/services/instances */ 1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_12_core_utils_matrixs__ = __webpack_require__(/*! core/utils/matrixs */ 190);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_13_core_utils_randomGenerator__ = __webpack_require__(/*! core/utils/randomGenerator */ 64);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_14_core_utils_tuples__ = __webpack_require__(/*! core/utils/tuples */ 24);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_15_core_utils_utl__ = __webpack_require__(/*! core/utils/utl */ 212);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__ = __webpack_require__(/*! games/polyglot/wordsearch/assets */ 200);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_17_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_17_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_17_lodash__);


















var Colors = __WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["a" /* Fonts */].Colors;
var durations = {
    delay: 100,
    answerDelay: 1000,
    move: 500,
    hide: 700,
    rotate: 50
};
var simpleAnimation = !__WEBPACK_IMPORTED_MODULE_1_core_logic_sys__["a" /* Sys */].isDesktop;
var bigLetter = ["ё", "й"];
var FieldComponent = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](FieldComponent, _super);
    function FieldComponent() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.positions = __WEBPACK_IMPORTED_MODULE_6_core_ui_common_sizes__["a" /* Sizes */].create({
            margin: { x: 320, y: 360 },
            text: { y: 141 },
            textBig: { y: 154 },
            answerMargin: -45,
            border: 100,
            cubeSize: 300,
            shift: 10,
            start: { x: -1000, y: -1000 }
        });
        _this.buttons = [];
        _this.rng = new __WEBPACK_IMPORTED_MODULE_13_core_utils_randomGenerator__["a" /* RandomGenerator */]();
        _this.prevShifts = [];
        _this.checkSignal = new Phaser.Signal();
        return _this;
    }
    FieldComponent.prototype.getMinSize = function (task) {
        var size = this.getSize();
        var height = size.heigth / task.field.length;
        var width = size.width / task.field[0].length;
        return Math.min(height, width);
    };
    FieldComponent.prototype.getSize = function () {
        return {
            width: __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__["e" /* screenSizeService */].realWidth - this.positions.border,
            heigth: __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__["e" /* screenSizeService */].realHeight - this.positions.border
        };
    };
    FieldComponent.prototype.updateButtonPositions = function (cubeSize) {
        this.onNewTask(this.getCurrentTask());
        // Matrixs.map(this.buttons, (btn, x, y) => {
        //   this.setButtonPosition(btn, [x, y], cubeSize)
        // })
        // this.updatePivot()
        // const size = this.getSize()
        // ControlUtl.scaleToFit(this, size.width, size.heigth)
    };
    FieldComponent.prototype.setButtonPosition = function (btn, p, cubeSize) {
        var _a = __WEBPACK_IMPORTED_MODULE_15_core_utils_utl__["a" /* Utl */].swap(p, __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__["e" /* screenSizeService */].isPortrait), xi = _a[0], yi = _a[1];
        btn.updatePos(this.positions.margin.x * cubeSize.scale * xi, this.positions.margin.y * cubeSize.scale * yi);
    };
    FieldComponent.prototype.getCubeSize = function (task) {
        var _this = this;
        var minSize = this.getMinSize(task);
        return __WEBPACK_IMPORTED_MODULE_17_lodash__["sortBy"](__WEBPACK_IMPORTED_MODULE_17_lodash__["range"](3).map(function (size) {
            var scale = 1 - 0.25 * size;
            var imgSize = _this.positions.cubeSize * scale;
            var diff = Math.abs(imgSize - minSize);
            return Object(__WEBPACK_IMPORTED_MODULE_14_core_utils_tuples__["a" /* tuple */])({ size: size, scale: scale }, diff);
        }), function (_a) {
            var _ = _a[0], diff = _a[1];
            return diff;
        })[0][0];
    };
    FieldComponent.prototype.newButtonAnimation = function (btn, simpleAnimation) {
        return simpleAnimation
            ? __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_commonAnimations__["a" /* CommonAnimations */].show(btn, true)
            : __WEBPACK_IMPORTED_MODULE_5_core_ui_animations_tweens__["a" /* Tweens */].start({
                target: btn,
                duration: durations.move,
                to: this.positions.start,
                noStopCurrentTweens: true,
                easing: Phaser.Easing.Cubic.Out,
                reverse: true,
                noAutoReset: true,
                onComplete: function () {
                    btn.getBehavior(__WEBPACK_IMPORTED_MODULE_8_core_ui_controls_behaviors_positionShiftBehavior__["a" /* PositionShiftBehavior */]).disabled = false;
                }
            });
    };
    FieldComponent.prototype.checkButtons = function (task) {
        var _this = this;
        var field = task.field;
        var _a = __WEBPACK_IMPORTED_MODULE_12_core_utils_matrixs__["a" /* Matrixs */].intersection(this.buttons, task.field), buttons = _a[0], letters = _a[1], both = _a[2];
        var cubeSize = this.getCubeSize(task);
        buttons.forEach(function (_a) {
            var x = _a[0], p = _a[1];
            __WEBPACK_IMPORTED_MODULE_12_core_utils_matrixs__["a" /* Matrixs */].set(_this.buttons, p, undefined);
            _this.removeChildAndDestroy(x);
        });
        var newButtons = letters.map(function (_a) {
            var x = _a[0], p = _a[1];
            var btn = _this.createButton(x, p, cubeSize);
            var _b = __WEBPACK_IMPORTED_MODULE_15_core_utils_utl__["a" /* Utl */].swap(p, __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__["e" /* screenSizeService */].isPortrait), xi = _b[0], yi = _b[1];
            btn.updatePos(_this.positions.margin.x * cubeSize.scale * xi, _this.positions.margin.y * cubeSize.scale * yi);
            var shift = btn.getBehavior(__WEBPACK_IMPORTED_MODULE_8_core_ui_controls_behaviors_positionShiftBehavior__["a" /* PositionShiftBehavior */]);
            shift.randomShift(false);
            shift.disabled = true;
            __WEBPACK_IMPORTED_MODULE_12_core_utils_matrixs__["a" /* Matrixs */].set(_this.buttons, p, btn);
            _this.addChild(btn);
            btn.imgControl.img.animations.add("rotate", __WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["b" /* Images */].cubeList(cubeSize.size).map(function (x) { return _this.gameKey(x); }), durations.rotate);
            return btn;
        });
        this.updatePivot();
        var size = this.getSize();
        __WEBPACK_IMPORTED_MODULE_10_core_ui_controls_controlUtl__["a" /* ControlUtl */].scaleToFit(this, size.width, size.heigth);
        __WEBPACK_IMPORTED_MODULE_4_core_ui_animations_signals__["a" /* Signals */].waitAll(newButtons.map(function (btn) { return _this.newButtonAnimation(btn, simpleAnimation).onComplete; })).addOnce(function () {
            {
                _this.answersAreReady();
            }
        });
        if (!simpleAnimation) {
            if (newButtons.length > 0) {
                this.playSound(__WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["c" /* Sounds */].visip);
            }
        }
        both.forEach(function (_a) {
            var b = _a[0], l = _a[1], p = _a[2];
            var newText = l.toUpperCase();
            b.textControl.fill = Colors.normal;
            if (b.text != newText || task.newField) {
                if (simpleAnimation) {
                    b.text = newText;
                    __WEBPACK_IMPORTED_MODULE_3_core_ui_animations_commonAnimations__["a" /* CommonAnimations */].show(b.textControl, true, durations.hide);
                }
                else {
                    b.text = "";
                    b.imgControl.img.animations.play("rotate").onComplete.addOnce(function () {
                        b.text = newText;
                    });
                }
            }
        });
        if (both.length > 0) {
            this.playSound(__WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["c" /* Sounds */].perevorot);
        }
        this.buttons = __WEBPACK_IMPORTED_MODULE_12_core_utils_matrixs__["a" /* Matrixs */].compact(this.buttons);
    };
    FieldComponent.prototype.createButton = function (l, answer, cubeSize) {
        var _this = this;
        var pointerLocation = __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__["e" /* screenSizeService */].isPortrait ? "bottom" : "right";
        var btn = this.addAnswerBehavior(new __WEBPACK_IMPORTED_MODULE_9_core_ui_controls_btn__["c" /* ImageBtn */](l.toUpperCase(), this.gameKey(__WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["b" /* Images */].cube(0, cubeSize.size)), function () {
            _this.playerAnswer(answer);
            if (_this.isTutorial) {
                _this.playerAnswer("check");
            }
            else {
                _this.checkSignal.dispose();
                _this.checkSignal = _this.delay(durations.answerDelay);
                _this.checkSignal.addOnce(function () {
                    _this.playerAnswer("check");
                });
            }
        }, __WEBPACK_IMPORTED_MODULE_0_tslib__["a" /* __assign */]({}, __WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["a" /* Fonts */].letter, { fontSize: __WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["a" /* Fonts */].letter.fontSize * cubeSize.scale })), answer, {
            location: pointerLocation,
            margin: this.positions.answerMargin
        }, true).updateFields({ tag: answer.toString() }).anchorCenter();
        btn.textControl.y = (__WEBPACK_IMPORTED_MODULE_17_lodash__["includes"](bigLetter, l) ? this.positions.textBig.y : this.positions.text.y) * cubeSize.scale;
        var shift = new __WEBPACK_IMPORTED_MODULE_8_core_ui_controls_behaviors_positionShiftBehavior__["a" /* PositionShiftBehavior */](this.positions.shift, 0);
        btn.addBehavior(shift);
        btn.onHover(function (out) {
            if (!out) {
                shift.randomShift(true);
                _this.playSound(__WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["c" /* Sounds */].povorot);
            }
        });
        btn.updatePivot();
        return btn;
    };
    FieldComponent.prototype.onNewTask = function (task) {
        var _this = this;
        this.onIntroCompleteOrMemorize.addOnce(function () {
            if (_this.isNewLevel) {
                _this.buttons = [];
                _this.removeAll(true);
            }
            _this.checkButtons(task);
        });
    };
    FieldComponent.prototype.onInit = function () {
        var _this = this;
        this.onChangeFor(function () { return __WEBPACK_IMPORTED_MODULE_11_core_ui_services_instances__["e" /* screenSizeService */].isPortrait; }, function () { return _this.updateButtonPositions(_this.getCubeSize(_this.getCurrentTask())); });
    };
    FieldComponent.prototype.onAnswer = function (answer) {
        var _this = this;
        var buttonList = __WEBPACK_IMPORTED_MODULE_17_lodash__["flatten"](this.buttons);
        var button = buttonList.find(function (x) { return x.tag == answer.answerId.toString(); });
        if (!button) {
            return;
        }
        if (answer.status == __WEBPACK_IMPORTED_MODULE_2_core_types_logic__["a" /* AnswerStatus */].Error) {
            var task = answer.levelTask.task;
            buttonList.forEach(function (x) { return x.textControl.fill = Colors.normal; });
            var correctAnswer = task.createdAnswers.find(function (x) { return __WEBPACK_IMPORTED_MODULE_17_lodash__["some"](x, function (x) { return __WEBPACK_IMPORTED_MODULE_17_lodash__["isEqual"](x, answer.answerId); }); })
                || task.createdAnswers[0];
            correctAnswer.forEach(function (_a) {
                var x = _a[0], y = _a[1];
                _this.buttons[y][x].textControl.fill = Colors.correct2;
            });
            button.textControl.fill = Colors.wrong;
        }
        else if (__WEBPACK_IMPORTED_MODULE_2_core_types_logic__["a" /* AnswerStatus */].isCorrect(answer.status)) {
            button.textControl.fill = button.textControl.fill == Colors.correct ? Colors.normal : Colors.correct;
            this.playSound(__WEBPACK_IMPORTED_MODULE_16_games_polyglot_wordsearch_assets__["c" /* Sounds */].letter);
        }
    };
    FieldComponent.prototype.onPropsUpdate = function () {
    };
    return FieldComponent;
}(__WEBPACK_IMPORTED_MODULE_7_core_ui_components_index__["e" /* GameComponent */]));



/***/ }),

/***/ 306:
/*!*****************************************************************!*\
  !*** ./src/core/ui/controls/behaviors/positionShiftBehavior.ts ***!
  \*****************************************************************/
/*! exports provided: PositionShiftBehavior */
/*! exports used: PositionShiftBehavior */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return PositionShiftBehavior; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_ui_animations_tweens__ = __webpack_require__(/*! core/ui/animations/tweens */ 28);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_ui_controls_behaviors_behavior__ = __webpack_require__(/*! core/ui/controls/behaviors/behavior */ 30);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_lodash__);




var durations = {
    tween: 300
};
var PositionShiftBehavior = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](PositionShiftBehavior, _super);
    function PositionShiftBehavior(max, maxAngle) {
        var _this = _super.call(this) || this;
        _this.max = max;
        _this.maxAngle = maxAngle;
        _this.currentShift = {
            x: 0,
            y: 0,
            angle: 0,
        };
        _this.disabled = false;
        return _this;
    }
    PositionShiftBehavior.prototype.resetPosition = function () {
        this.control.x -= this.currentShift.x;
        this.control.y -= this.currentShift.y;
        this.control.angle -= this.currentShift.angle;
        this.currentShift = { x: 0, y: 0, angle: 0 };
    };
    PositionShiftBehavior.prototype.shift = function (newShift, animation) {
        if (this.disabled) {
            return;
        }
        var newX = this.control.x - this.currentShift.x + newShift.x;
        var newY = this.control.y - this.currentShift.y + newShift.y;
        var newAngle = this.control.angle - this.currentShift.angle + newShift.angle;
        if (animation) {
            __WEBPACK_IMPORTED_MODULE_1_core_ui_animations_tweens__["a" /* Tweens */].start({
                target: this.control,
                noStopCurrentTweens: true,
                to: {
                    x: newX,
                    y: newY,
                    angle: newAngle
                },
                duration: durations.tween,
                easing: Phaser.Easing.Sinusoidal.Out
            });
        }
        else {
            this.control.x = newX;
            this.control.y = newY;
            this.control.angle = newAngle;
        }
        this.currentShift = newShift;
    };
    PositionShiftBehavior.prototype.randomShift = function (animation) {
        var _this = this;
        var randomPosition = function () { return __WEBPACK_IMPORTED_MODULE_3_lodash__["random"](-_this.max, _this.max); };
        var newShift = {
            x: randomPosition(),
            y: randomPosition(),
            angle: __WEBPACK_IMPORTED_MODULE_3_lodash__["random"](-this.maxAngle, this.maxAngle)
        };
        this.shift(newShift, animation);
    };
    PositionShiftBehavior.prototype.onInit = function () {
    };
    return PositionShiftBehavior;
}(__WEBPACK_IMPORTED_MODULE_2_core_ui_controls_behaviors_behavior__["a" /* Behavior */]));



/***/ }),

/***/ 307:
/*!*************************************************!*\
  !*** ./src/games/polyglot/wordsearch/levels.ts ***!
  \*************************************************/
/*! exports provided: levels */
/*! exports used: levels */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return levels; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_core_types_logic__ = __webpack_require__(/*! core/types/logic */ 21);

function level(w, h) {
    return { w: w, h: h };
}
var levels = Object(__WEBPACK_IMPORTED_MODULE_0_core_types_logic__["d" /* createLevelsFromArray */])([
    level(4, 3),
    level(4, 4),
    level(5, 4),
    level(5, 5),
    level(6, 5),
    level(7, 5),
    level(8, 5),
    level(9, 5),
]);


/***/ }),

/***/ 308:
/*!************************************************!*\
  !*** ./src/games/polyglot/wordsearch/logic.ts ***!
  \************************************************/
/*! exports provided: WordSearchAnswer, Logic */
/*! exports used: Logic */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* unused harmony export WordSearchAnswer */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return Logic; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tslib__ = __webpack_require__(/*! tslib */ 0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_core_logic__ = __webpack_require__(/*! core/logic */ 25);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_core_logic_sys__ = __webpack_require__(/*! core/logic/sys */ 6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_types_logic__ = __webpack_require__(/*! core/types/logic */ 21);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_core_utils__ = __webpack_require__(/*! core/utils */ 9);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_core_utils_arrays__ = __webpack_require__(/*! core/utils/arrays */ 26);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_core_utils_circleSeq__ = __webpack_require__(/*! core/utils/circleSeq */ 157);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_core_utils_numbers__ = __webpack_require__(/*! core/utils/numbers */ 31);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_core_utils_tuples__ = __webpack_require__(/*! core/utils/tuples */ 24);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9_core_utils_undef__ = __webpack_require__(/*! core/utils/undef */ 8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_words__ = __webpack_require__(/*! games/polyglot/wordsearch/words */ 309);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_11_games_utils_mazeUtl__ = __webpack_require__(/*! games/utils/mazeUtl */ 310);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_12_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_12_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_12_lodash__);













var letters = "абвгдеёжзиёкламнопрстуфхцчшщъыьэюя".split("");
var goodLetters = "оаеинтрслвкпмудя".split("");
var WordSearchAnswer = /** @class */ (function () {
    function WordSearchAnswer(isTutorial, field, tutorialAnswers, allWords) {
        this.isTutorial = isTutorial;
        this.field = field;
        this.tutorialAnswers = tutorialAnswers;
        this.allWords = allWords;
        this.prevAnswers = [];
        this.currentWord = "";
        this._isCompleted = false;
        this._isChecked = false;
    }
    Object.defineProperty(WordSearchAnswer.prototype, "restAnswers", {
        get: function () {
            return [this.tutorialAnswers[this.prevAnswers.length]];
        },
        enumerable: true,
        configurable: true
    });
    WordSearchAnswer.prototype.check = function (answerId) {
        if (answerId == "check") {
            this._isChecked = true;
            this.checkIsCompleted();
            return true;
        }
        this._isChecked = false;
        this._isCompleted = false;
        var x = answerId[0], y = answerId[1];
        var prevAnswer = __WEBPACK_IMPORTED_MODULE_12_lodash__["last"](this.prevAnswers);
        var distance = prevAnswer ? Math.abs(prevAnswer[0] - x) + (Math.abs(prevAnswer[1] - y)) : -1;
        if (distance == 0) {
            this.currentWord = this.currentWord.slice(0, -1);
            this.prevAnswers.pop();
            return true;
        }
        if (prevAnswer && ((distance != 1) || __WEBPACK_IMPORTED_MODULE_12_lodash__["some"](this.prevAnswers, function (p) { return __WEBPACK_IMPORTED_MODULE_12_lodash__["isEqual"](p, [x, y]); }))) {
            return "none";
        }
        if (this.isTutorial && !__WEBPACK_IMPORTED_MODULE_12_lodash__["isEqual"]([x, y], this.restAnswers[0])) {
            return false;
        }
        var answer = this.field[y][x];
        this.currentWord = this.currentWord + answer;
        if (this.checkIsCompleted()) {
            this.prevAnswers.push([x, y]);
            return true;
        }
        return false;
    };
    WordSearchAnswer.prototype.checkIsCompleted = function () {
        var _a = __WEBPACK_IMPORTED_MODULE_5_core_utils_arrays__["a" /* Arrays */].searchByPrefix(this.allWords, this.currentWord), min = _a[0], max = _a[1];
        if (min >= 0) {
            this._isCompleted = this.allWords[min] == this.currentWord;
            return true;
        }
        return false;
    };
    Object.defineProperty(WordSearchAnswer.prototype, "isCompleted", {
        get: function () {
            return this._isCompleted && this._isChecked;
        },
        enumerable: true,
        configurable: true
    });
    return WordSearchAnswer;
}());

var Logic = /** @class */ (function (_super) {
    __WEBPACK_IMPORTED_MODULE_0_tslib__["c" /* __extends */](Logic, _super);
    function Logic() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.rng = new __WEBPACK_IMPORTED_MODULE_4_core_utils__["e" /* RandomGenerator */]();
        _this.wordSeq = new __WEBPACK_IMPORTED_MODULE_6_core_utils_circleSeq__["b" /* UniqSeq */](1);
        return _this;
    }
    Logic.prototype.createPath = function (level, shiftLength) {
        if (Math.min(level.w, level.h) < 4) {
            return __WEBPACK_IMPORTED_MODULE_11_games_utils_mazeUtl__["a" /* MazeUtl */].simplePath(this.rng, level.w, level.h);
        }
        else {
            var path = __WEBPACK_IMPORTED_MODULE_11_games_utils_mazeUtl__["a" /* MazeUtl */].createLabyrinthPath(level.w, level.h);
            return __WEBPACK_IMPORTED_MODULE_5_core_utils_arrays__["a" /* Arrays */].shift(path, shiftLength);
        }
    };
    Logic.prototype.updateField = function (field, word, path) {
        var letters = word.split("");
        path.forEach(function (_a, i) {
            var x = _a[0], y = _a[1];
            field[y][x] = letters[i];
        });
    };
    Logic.prototype.addWords = function (words, length, allWords) {
        if (length < allWords.minLength) {
            return;
        }
        var word = length <= allWords.maxLength
            ? this.rng.sample(allWords.byLength[length])
            : this.wordSeq.nextFrom(allWords.words);
        words.push(word);
        this.addWords(words, length - word.length, allWords);
    };
    Logic.prototype.randomLetters = function (length, exclude) {
        if (exclude === void 0) { exclude = []; }
        return this.rng.subset(__WEBPACK_IMPORTED_MODULE_12_lodash__["difference"](goodLetters, exclude), length).join("");
    };
    Logic.prototype.newTask = function (level, allWords) {
        var _this = this;
        var field = __WEBPACK_IMPORTED_MODULE_12_lodash__["range"](level.h).map(function (x) { return __WEBPACK_IMPORTED_MODULE_12_lodash__["times"](level.w, function () { return ""; }); });
        var path = this.createPath(level, __WEBPACK_IMPORTED_MODULE_7_core_utils_numbers__["a" /* Numbers */].count(allWords.minLength, 2));
        var words = [];
        this.addWords(words, path.length, allWords);
        if (__WEBPACK_IMPORTED_MODULE_2_core_logic_sys__["a" /* Sys */].showLog) {
            console.log(words);
        }
        var restPath = path;
        var answers = [];
        words.forEach(function (word) {
            var _a = __WEBPACK_IMPORTED_MODULE_5_core_utils_arrays__["a" /* Arrays */].split(restPath, word.length), wordPath = _a[0], rest = _a[1];
            restPath = rest;
            var path = _this.rng.bool() ? wordPath : __WEBPACK_IMPORTED_MODULE_12_lodash__["reverse"](wordPath);
            answers.push(wordPath);
            _this.updateField(field, word, wordPath);
        });
        this.fillEmpty(field);
        return Object(__WEBPACK_IMPORTED_MODULE_8_core_utils_tuples__["a" /* tuple */])(field, answers);
    };
    Logic.prototype.fillEmpty = function (field) {
        var _this = this;
        __WEBPACK_IMPORTED_MODULE_12_lodash__["range"](field.length).forEach(function (i) { return field[i] = field[i].map(function (x) { return x ? x : _this.rng.sample(goodLetters); }); });
    };
    Logic.prototype.updateTask = function (_a) {
        var prevTask = _a[0], _ = _a[1];
        var answers = prevTask.answers.prevAnswers;
        var restAnswers = prevTask.createdAnswers.filter(function (x) { return !__WEBPACK_IMPORTED_MODULE_5_core_utils_arrays__["a" /* Arrays */].isIntercepted(x, answers); });
        if (restAnswers.length == 0) {
            return undefined;
        }
        var field = prevTask.field;
        this.updateField(field, this.randomLetters(answers.length, answers.map(function (_a) {
            var x = _a[0], y = _a[1];
            return field[y][x];
        })), answers);
        return Object(__WEBPACK_IMPORTED_MODULE_8_core_utils_tuples__["a" /* tuple */])(field, restAnswers);
    };
    Logic.prototype.nextTask = function (level) {
        var minSize = __WEBPACK_IMPORTED_MODULE_7_core_utils_numbers__["a" /* Numbers */].count(level.w + level.h, 2);
        var maxWordLength = __WEBPACK_IMPORTED_MODULE_7_core_utils_numbers__["a" /* Numbers */].between(minSize, __WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_words__["a" /* Words */].minLength, __WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_words__["a" /* Words */].maxLength);
        var allWords = __WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_words__["a" /* Words */].getWords(maxWordLength);
        var prevTaskAnswers = this.getPrevTaskAnswers();
        var lastAnswerStatus = __WEBPACK_IMPORTED_MODULE_9_core_utils_undef__["a" /* Undef */].map(this.lastAnswer(), function (x) { return x.status; });
        var newField = this.taskCreatorState.isNewLevel || lastAnswerStatus == __WEBPACK_IMPORTED_MODULE_3_core_types_logic__["a" /* AnswerStatus */].Error;
        var _a = newField
            ? this.newTask(level, allWords)
            : (this.updateTask(prevTaskAnswers) || this.newTask(level, allWords)), field = _a[0], createdAnswers = _a[1];
        var answers = new WordSearchAnswer(!!this.gameData.settings.isTutorial, field, createdAnswers[0], __WEBPACK_IMPORTED_MODULE_10_games_polyglot_wordsearch_words__["a" /* Words */].allWords);
        return {
            field: field,
            createdAnswers: createdAnswers,
            newField: newField,
            answers: answers
        };
    };
    return Logic;
}(__WEBPACK_IMPORTED_MODULE_1_core_logic__["c" /* GameLogic */]));



/***/ }),

/***/ 309:
/*!************************************************!*\
  !*** ./src/games/polyglot/wordsearch/words.ts ***!
  \************************************************/
/*! exports provided: Words */
/*! exports used: Words */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return Words; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_lodash__);

var Words;
(function (Words) {
    Words.minLength = 4;
    Words.maxLength = 6;
    var freqDictionary = [
        "туба",
        "нагон",
        "блог",
        "дрон",
        "плёнка",
        "пава",
        "тлен",
        "плут",
        "грот",
        "слёт",
        "дива",
        "сидр",
        "тюль",
        "бита",
        "стяг",
        "моно",
        "роба",
        "логарифм",
        "камнепад",
        "хват",
        "лоток",
        "манты",
        "соло",
        "сало",
        "год",
        "человек",
        "время",
        "дело",
        "жизнь",
        "день",
        "рука",
        "раз",
        "работа",
        "слово",
        "место",
        "лицо",
        "друг",
        "глаз",
        "вопрос",
        "дом",
        "сторона",
        "страна",
        "мир",
        "случай",
        "голова",
        "ребёнок",
        "сила",
        "конец",
        "вид",
        "система",
        "часть",
        "город",
        "отношение",
        "женщина",
        "деньги",
        "земля",
        "машина",
        "вода",
        "отец",
        "проблема",
        "час",
        "право",
        "нога",
        "решение",
        "дверь",
        "образ",
        "история",
        "власть",
        "закон",
        "война",
        "бог",
        "голос",
        "тысяча",
        "книга",
        "возможность",
        "результат",
        "ночь",
        "стол",
        "имя",
        "область",
        "статья",
        "число",
        "компания",
        "народ",
        "жена",
        "группа",
        "развитие",
        "процесс",
        "суд",
        "условие",
        "средство",
        "начало",
        "свет",
        "пора",
        "путь",
        "душа",
        "уровень",
        "форма",
        "связь",
        "минута",
        "улица",
        "вечер",
        "качество",
        "мысль",
        "дорога",
        "мать",
        "действие",
        "месяц",
        "государство",
        "язык",
        "любовь",
        "взгляд",
        "мама",
        "век",
        "школа",
        "цель",
        "общество",
        "деятельность",
        "организация",
        "президент",
        "комната",
        "порядок",
        "момент",
        "театр",
        "письмо",
        "утро",
        "помощь",
        "ситуация",
        "роль",
        "рубль",
        "смысл",
        "состояние",
        "квартира",
        "орган",
        "внимание",
        "тело",
        "труд",
        "сын",
        "мера",
        "смерть",
        "рынок",
        "программа",
        "задача",
        "предприятие",
        "окно",
        "разговор",
        "правительство",
        "семья",
        "производство",
        "информация",
        "положение",
        "центр",
        "ответ",
        "муж",
        "автор",
        "стена",
        "интерес",
        "федерация",
        "правило",
        "управление",
        "мужчина",
        "идея",
        "партия",
        "совет",
        "счёт",
        "сердце",
        "движение",
        "вещь",
        "материал",
        "неделя",
        "чувство",
        "глава",
        "наука",
        "ряд",
        "газета",
        "причина",
        "плечо",
        "цена",
        "план",
        "речь",
        "точка",
        "основа",
        "товарищ",
        "культура",
        "данные",
        "мнение",
        "документ",
        "институт",
        "ход",
        "проект",
        "встреча",
        "директор",
        "срок",
        "палец",
        "опыт",
        "служба",
        "судьба",
        "девушка",
        "очередь",
        "лес",
        "состав",
        "член",
        "количество",
        "событие",
        "объект",
        "зал",
        "создание",
        "значение",
        "период",
        "шаг",
        "брат",
        "искусство",
        "структура",
        "номер",
        "пример",
        "исследование",
        "гражданин",
        "игра",
        "начальник",
        "рост",
        "тема",
        "принцип",
        "метод",
        "тип",
        "фильм",
        "край",
        "гость",
        "воздух",
        "характер",
        "борьба",
        "использование",
        "размер",
        "образование",
        "мальчик",
        "кровь",
        "район",
        "небо",
        "нёбо",
        "армия",
        "класс",
        "представитель",
        "участие",
        "девочка",
        "политика",
        "герой",
        "картина",
        "доллар",
        "спина",
        "территория",
        "пол",
        "поле",
        "изменение",
        "направление",
        "рисунок",
        "течение",
        "церковь",
        "банк",
        "сцена",
        "население",
        "большинство",
        "музыка",
        "правда",
        "свобода",
        "память",
        "команда",
        "союз",
        "врач",
        "договор",
        "дерево",
        "факт",
        "хозяин",
        "природа",
        "угол",
        "телефон",
        "позиция",
        "двор",
        "писатель",
        "самолет",
        "объём",
        "род",
        "солнце",
        "вера",
        "берег",
        "спектакль",
        "фирма",
        "способ",
        "завод",
        "цвет",
        "журнал",
        "руководитель",
        "специалист",
        "оценка",
        "регион",
        "песня",
        "процент",
        "родитель",
        "море",
        "требование",
        "основание",
        "половина",
        "роман",
        "круг",
        "анализ",
        "стихи",
        "автомобиль",
        "экономика",
        "литература",
        "бумага",
        "поэт",
        "степень",
        "господин",
        "надежда",
        "предмет",
        "вариант",
        "министр",
        "граница",
        "дух",
        "модель",
        "операция",
        "пара",
        "сон",
        "название",
        "ум",
        "повод",
        "старик",
        "миллион",
        "успех",
        "счастье",
        "ребята",
        "кабинет",
        "магазин",
        "пространство",
        "выход",
        "удар",
        "база",
        "знание",
        "текст",
        "защита",
        "руководство",
        "площадь",
        "сознание",
        "возраст",
        "участник",
        "участок",
        "пункт",
        "линия",
        "желание",
        "папа",
        "доктор",
        "губа",
        "дочь",
        "среда",
        "председатель",
        "представление",
        "солдат",
        "художник",
        "волос",
        "оружие",
        "соответствие",
        "ветер",
        "парень",
        "зрение",
        "генерал",
        "огонь",
        "понятие",
        "строительство",
        "ухо",
        "грудь",
        "нос",
        "страх",
        "услуга",
        "содержание",
        "радость",
        "безопасность",
        "продукт",
        "комплекс",
        "бизнес",
        "сад",
        "сотрудник",
        "лето",
        "курс",
        "предложение",
        "рот",
        "технология",
        "реформа",
        "отсутствие",
        "собака",
        "камень",
        "будущее",
        "рассказ",
        "контроль",
        "река",
        "продукция",
        "сумма",
        "техника",
        "здание",
        "сфера",
        "необходимость",
        "фонд",
        "подготовка",
        "лист",
        "республика",
        "хозяйство",
        "воля",
        "бюджет",
        "снег",
        "деревня",
        "мужик",
        "элемент",
        "обстоятельство",
        "немец",
        "победа",
        "источник",
        "звезда",
        "выбор",
        "масса",
        "итог",
        "сестра",
        "практика",
        "проведение",
        "карман",
        "слава",
        "кухня",
        "определение",
        "функция",
        "войско",
        "комиссия",
        "применение",
        "капитан",
        "работник",
        "обеспечение",
        "офицер",
        "фамилия",
        "предел",
        "выборы",
        "ученый",
        "бутылка",
        "бой",
        "теория",
        "зона",
        "отдел",
        "зуб",
        "разработка",
        "личность",
        "гора",
        "товар",
        "метр",
        "праздник",
        "влияние",
        "читатель",
        "удовольствие",
        "актёр",
        "слеза",
        "ответственность",
        "учитель",
        "акт",
        "боль",
        "множество",
        "особенность",
        "показатель",
        "корабль",
        "звук",
        "впечатление",
        "частность",
        "детство",
        "вывод",
        "профессор",
        "доля",
        "норма",
        "прошлое",
        "командир",
        "коридор",
        "поддержка",
        "рамка",
        "враг",
        "этап",
        "чёрт",
        "дед",
        "собрание",
        "приём",
        "болезнь",
        "клетка",
        "кожа",
        "заявление",
        "попытка",
        "сравнение",
        "расчёт",
        "депутат",
        "комитет",
        "знак",
        "дядя",
        "учёт",
        "хлеб",
        "чай",
        "режим",
        "целое",
        "вирус",
        "выражение",
        "здоровье",
        "зима",
        "десяток",
        "глубина",
        "сеть",
        "студент",
        "секунда",
        "скорость",
        "поиск",
        "суть",
        "налог",
        "ошибка",
        "доход",
        "режиссёр",
        "поверхность",
        "ощущение",
        "карта",
        "клуб",
        "станция",
        "революция",
        "колено",
        "министерство",
        "стекло",
        "этаж",
        "высота",
        "бабушка",
        "трубка",
        "газ",
        "мастер",
        "поведение",
        "столица",
        "механизм",
        "передача",
        "способность",
        "подход",
        "энергия",
        "существование",
        "исполнение",
        "кино",
        "сожаление",
        "заместитель",
        "ресурс",
        "акция",
        "рождение",
        "администрация",
        "стоимость",
        "улыбка",
        "артист",
        "сосед",
        "фраза",
        "фигура",
        "субъект",
        "реакция",
        "список",
        "фотография",
        "журналист",
        "май",
        "нарушение",
        "заседание",
        "толпа",
        "больница",
        "существо",
        "свойство",
        "долг",
        "поколение",
        "животное",
        "схема",
        "усилие",
        "отличие",
        "остров",
        "противник",
        "волна",
        "реализация",
        "страница",
        "формирование",
        "житель",
        "красота",
        "птица",
        "растение",
        "тень",
        "явление",
        "храм",
        "запах",
        "водка",
        "наличие",
        "ужас",
        "одежда",
        "кресло",
        "больной",
        "поезд",
        "университет",
        "традиция",
        "адрес",
        "декабрь",
        "ладонь",
        "сведение",
        "цветок",
        "лидер",
        "октябрь",
        "занятие",
        "сентябрь",
        "помещение",
        "январь",
        "зритель",
        "редакция",
        "стиль",
        "весна",
        "фактор",
        "август",
        "известие",
        "зависимость",
        "охрана",
        "оборудование",
        "концерт",
        "отделение",
        "расход",
        "выставка",
        "милиция",
        "переход",
        "эпоха",
        "запад",
        "произведение",
        "родина",
        "собственность",
        "тайна",
        "трава",
        "лагерь",
        "имущество",
        "кровать",
        "аппарат",
        "середина",
        "март",
        "клиент",
        "дама",
        "фронт",
        "отрасль",
        "стул",
        "беседа",
        "законодательство",
        "продажа",
        "повышение",
        "музей",
        "след",
        "полковник",
        "сомнение",
        "понимание",
        "апрель",
        "князь",
        "рыба",
        "дума",
        "кодекс",
        "сутки",
        "чудо",
        "шея",
        "судья",
        "крыша",
        "настроение",
        "поток",
        "должность",
        "преступление",
        "мозг",
        "честь",
        "пост",
        "еврей",
        "июнь",
        "сотня",
        "дождь",
        "лестница",
        "дача",
        "установка",
        "появление",
        "получение",
        "образец",
        "труба",
        "главное",
        "осень",
        "костюм",
        "баба",
        "ценность",
        "обязанность",
        "пьеса",
        "таблица",
        "вино",
        "воспоминание",
        "лошадь",
        "коллега",
        "организм",
        "ученик",
        "учреждение",
        "открытие",
        "том",
        "черта",
        "характеристика",
        "выполнение",
        "оборона",
        "выступление",
        "температура",
        "перспектива",
        "подруга",
        "приказ",
        "жертва",
        "ресторан",
        "километр",
        "спор",
        "вкус",
        "признак",
        "промышленность",
        "американец",
        "лоб",
        "заключение",
        "восток",
        "исключение",
        "ключ",
        "постановление",
        "слой",
        "бок",
        "июль",
        "перевод",
        "секретарь",
        "кусок",
        "слух",
        "польза",
        "звонок",
        "обстановка",
        "чиновник",
        "соглашение",
        "деталь",
        "русский",
        "тишина",
        "зарплата",
        "билет",
        "подарок",
        "тюрьма",
        "ящик",
        "конкурс",
        "книжка",
        "изучение",
        "просьба",
        "царь",
        "публика",
        "смех",
        "сообщение",
        "угроза",
        "беда",
        "блок",
        "достижение",
        "назначение",
        "реклама",
        "портрет",
        "масло",
        "стакан",
        "урок",
        "часы",
        "крик",
        "творчество",
        "телевизор",
        "инструмент",
        "концепция",
        "лейтенант",
        "экран",
        "дно",
        "реальность",
        "канал",
        "мясо",
        "знакомый",
        "щека",
        "конфликт",
        "переговоры",
        "запись",
        "вагон",
        "площадка",
        "последствие",
        "сотрудничество",
        "зеркало",
        "тон",
        "академия",
        "палата",
        "потребность",
        "ноябрь",
        "увеличение",
        "дурак",
        "поездка",
        "обед",
        "потеря",
        "февраль",
        "мероприятие",
        "парк",
        "принятие",
        "устройство",
        "вещество",
        "категория",
        "сезон",
        "гостиница",
        "издание",
        "объединение",
        "темнота",
        "человечество",
        "колесо",
        "опасность",
        "разрешение",
        "воздействие",
        "коллектив",
        "камера",
        "запас",
        "следствие",
        "длина",
        "крыло",
        "округ",
        "фон",
        "кандидат",
        "родственник",
        "давление",
        "присутствие",
        "взаимодействие",
        "доска",
        "партнёр",
        "двигатель",
        "шум",
        "достоинство",
        "грех",
        "нож",
        "полёт",
        "страсть",
        "испытание",
        "истина",
        "оплата",
        "разница",
        "водитель",
        "пакет",
        "снижение",
        "формула",
        "живот",
        "капитал",
        "мост",
        "новость",
        "эффект",
        "вход",
        "губернатор",
        "доклад",
        "смена",
        "убийство",
        "эксперт",
        "автобус",
        "платье",
        "кадр",
        "тётя",
        "общение",
        "психология",
        "лев",
        "порог",
        "проверка",
        "процедура",
        "рабочий",
        "ремонт",
        "обращение",
        "обучение",
        "ожидание",
        "памятник",
        "корень",
        "наблюдение",
        "буква",
        "доказательство",
        "признание",
        "постель",
        "штаб",
        "владелец",
        "компьютер",
        "инженер",
        "старуха",
        "лодка",
        "ракета",
        "серия",
        "шутка",
        "вершина",
        "выпуск",
        "кулак",
        "лёд",
        "торговля",
        "нефть",
        "молодёжь",
        "цифра",
        "корпус",
        "недостаток",
        "сапог",
        "сущность",
        "талант",
        "эффективность",
        "кофе",
        "полоса",
        "основное",
        "рассмотрение",
        "сбор",
        "штат",
        "следователь",
        "жилье",
        "мешок",
        "описание",
        "куст",
        "отказ",
        "замок",
        "редактор",
        "дворец",
        "забота",
        "пиво",
        "диван",
        "столик",
        "эксперимент",
        "печать",
        "кольцо",
        "пистолет",
        "воспитание",
        "начальство",
        "профессия",
        "ворота",
        "добро",
        "дружба",
        "покой",
        "риск",
        "окончание",
        "дым",
        "брак",
        "величина",
        "записка",
        "инициатива",
        "совесть",
        "активность",
        "кость",
        "спорт",
        "кредит",
        "господь",
        "майор",
        "конференция",
        "потолок",
        "библиотека",
        "помощник",
        "конструкция",
        "отдых",
        "ручка",
        "металл",
        "молоко",
        "прокурор",
        "транспорт",
        "поэзия",
        "соединение",
        "краска",
        "расстояние",
        "мечта",
        "село",
        "еда",
        "зло",
        "подразделение",
        "сюжет",
        "рубеж",
        "сигнал",
        "атмосфера",
        "крест",
        "вес",
        "взрыв",
        "контакт",
        "сигарета",
        "восторг",
        "золото",
        "почва",
        "премия",
        "король",
        "подъезд",
        "шанс",
        "автомат",
        "заказ",
        "мальчишка",
        "очки",
        "миг",
        "штука",
        "чтение",
        "посёлок",
        "свидетель",
        "ставка",
        "сумка",
        "удивление",
        "хвост",
        "песок",
        "поворот",
        "возвращение",
        "мгновение",
        "статус",
        "озеро",
        "строй",
        "параметр",
        "сказка",
        "тенденция",
        "вина",
        "дыхание",
        "версия",
        "масштаб",
        "монастырь",
        "хозяйка",
        "дочка",
        "танец",
        "эксплуатация",
        "коммунист",
        "пенсия",
        "приятель",
        "объяснение",
        "набор",
        "производитель",
        "пыль",
        "философия",
        "мощность",
        "обязательство",
        "уход",
        "горло",
        "кризис",
        "указание",
        "плата",
        "яблоко",
        "препарат",
        "действительность",
        "москвич",
        "остаток",
        "изображение",
        "сделка",
        "сочинение",
        "покупатель",
        "танк",
        "затрата",
        "строка",
        "единица",
        "обработка",
        "чемпионат",
        "лаборатория",
        "милиционер",
        "регистрация",
        "изделие",
        "молитва",
        "погода",
        "съезд",
        "телевидение",
        "введение",
        "пресса",
        "цепь",
        "корреспондент",
        "планета",
        "минимум",
        "обсуждение",
        "полк",
        "вокзал",
        "гибель",
        "прокуратура",
        "согласие",
        "конь",
        "могила",
        "стенка",
        "тревога",
        "напряжение",
        "логика",
        "распоряжение",
        "задание",
        "кошка",
        "распределение",
        "сценарий",
        "бригада",
        "пятно",
        "гроб",
        "ограничение",
        "надпись",
        "термин",
        "ткань",
        "визит",
        "океан",
        "предприниматель",
        "прибор",
        "уважение",
        "ведомство",
        "паспорт",
        "пауза",
        "дата",
        "постановка",
        "поступок",
        "ствол",
        "готовность",
        "тепло",
        "адвокат",
        "контракт",
        "радио",
        "академик",
        "кампания",
        "оборот",
        "агентство",
        "зверь",
        "нация",
        "потребитель",
        "банка",
        "коробка",
        "осуществление",
        "секрет",
        "темп",
        "метро",
        "дедушка",
        "инвестиция",
        "десятилетие",
        "преимущество",
        "публикация",
        "стихотворение",
        "отчёт",
        "пальто",
        "узел",
        "обмен",
        "порода",
        "прибыль",
        "тоска",
        "заболевание",
        "стремление",
        "фестиваль",
        "шкаф",
        "горе",
        "забор",
        "вооружение",
        "мебель",
        "мороз",
        "перемена",
        "костёр",
        "марка",
        "протокол",
        "привычка",
        "трудность",
        "грязь",
        "матч",
        "плод",
        "спрос",
        "пассажир",
        "стандарт",
        "финансирование",
        "мотор",
        "стадия",
        "звание",
        "утверждение",
        "критерий",
        "север",
        "борт",
        "наказание",
        "раздел",
        "конституция",
        "сектор",
        "склад",
        "сохранение",
        "учение",
        "карандаш",
        "мотив",
        "распространение",
        "арест",
        "знакомство",
        "мэр",
        "символ",
        "полномочие",
        "уверенность",
        "отряд",
        "разведка",
        "лечение",
        "мода",
        "поражение",
        "рубашка",
        "луна",
        "нужда",
        "привет",
        "собор",
        "съёмка",
        "груз",
        "подробность",
        "трагедия",
        "политик",
        "луч",
        "спасение",
        "экзамен",
        "добыча",
        "проявление",
        "пуля",
        "жест",
        "свидетельство",
        "ассоциация",
        "салон",
        "корова",
        "молчание",
        "отпуск",
        "питание",
        "девчонка",
        "капля",
        "дар",
        "поставка",
        "оркестр",
        "интервью",
        "критика",
        "миллиард",
        "пожар",
        "холод",
        "шар",
        "компонент",
        "статистика",
        "тётка",
        "цикл",
        "эпизод",
        "взрослый",
        "обида",
        "обслуживание",
        "потенциал",
        "ненависть",
        "персонаж",
        "руль",
        "совещание",
        "религия",
        "молодость",
        "различие",
        "вклад",
        "карьера",
        "кладбище",
        "облако",
        "гонка",
        "рукав",
        "сокращение",
        "туман",
        "шофёр",
        "бандит",
        "чемодан",
        "атака",
        "роза",
        "указ",
        "восстановление",
        "сложность",
        "игрок",
        "построение",
        "продолжение",
        "скандал",
        "труп",
        "происхождение",
        "путешествие",
        "справедливость",
        "тонна",
        "гарантия",
        "вождь",
        "месторождение",
        "мина",
        "орден",
        "пластинка",
        "крестьянин",
        "юноша",
        "бабка",
        "вызов",
        "издательство",
        "полка",
        "дитя",
        "покупка",
        "бровь",
        "доверие",
        "колонна",
        "падение",
        "стратегия",
        "боец",
        "интернет",
        "ведение",
        "тарелка",
        "удача",
        "деятель",
        "выстрел",
        "полгода",
        "куртка",
        "флот",
        "бытие",
        "единство",
        "оператор",
        "физика",
        "мышление",
        "приход",
        "шеф",
        "одиночество",
        "шапка",
        "ложка",
        "подпись",
        "актриса",
        "колхоз",
        "куча",
        "прогноз",
        "цивилизация",
        "белок",
        "кот",
        "остановка",
        "платок",
        "локоть",
        "методика",
        "мука",
        "яйцо",
        "голод",
        "жалоба",
        "концентрация",
        "музыкант",
        "охота",
        "повесть",
        "глупость",
        "картошка",
        "переулок",
        "домик",
        "исполнитель",
        "страдание",
        "ветка",
        "заказчик",
        "лапа",
        "священник",
        "восприятие",
        "заведение",
        "лекарство",
        "мелочь",
        "пища",
        "свадьба",
        "подушка",
        "справка",
        "тьма",
        "юг",
        "башня",
        "обвинение",
        "соль",
        "тариф",
        "умение",
        "агент",
        "гений",
        "иск",
        "лекция",
        "нагрузка",
        "намерение",
        "хранение",
        "ботинок",
        "подвиг",
        "воскресенье",
        "молодец",
        "авиация",
        "дневник",
        "лётчик",
        "одеяло",
        "вертолёт",
        "приговор",
        "протяжение",
        "внук",
        "сочетание",
        "баня",
        "демократия",
        "диалог",
        "гриб",
        "расширение",
        "волнение",
        "полиция",
        "богатство",
        "измерение",
        "отчаяние",
        "пиджак",
        "сахар",
        "столетие",
        "цирк",
        "благо",
        "пользование",
        "точность",
        "коллекция",
        "манера",
        "полчаса",
        "цех",
        "бумажка",
        "подъём",
        "свидание",
        "смесь",
        "страхование",
        "империя",
        "красавица",
        "разум",
        "гордость",
        "динамика",
        "дон",
        "студия",
        "плита",
        "пояс",
        "суббота",
        "аэропорт",
        "брюки",
        "рукопись",
        "авторитет",
        "граф",
        "дорожка",
        "исследователь",
        "историк",
        "кафе",
        "тренер",
        "мышь",
        "окружение",
        "замечание",
        "замысел",
        "мышца",
        "продавец",
        "соотношение",
        "карточка",
        "фабрика",
        "посол",
        "катастрофа",
        "пачка",
        "столовая",
        "ужин",
        "установление",
        "училище",
        "шоссе",
        "акционер",
        "городок",
        "картинка",
        "легенда",
        "соображение",
        "сообщество",
        "икона",
        "коньяк",
        "посетитель",
        "собеседник",
        "волк",
        "сопротивление",
        "халат",
        "вуз",
        "жанр",
        "идеал",
        "поправка",
        "юность",
        "лента",
        "частица",
        "бомба",
        "композитор",
        "крыльцо",
        "пациент",
        "сок",
        "тяжесть",
        "штамм",
        "замена",
        "кусочек",
        "победитель",
        "приезд",
        "экспедиция",
        "аспект",
        "интеллигенция",
        "противоречие",
        "военный",
        "преступник",
        "строение",
        "ущерб",
        "идеология",
        "подросток",
        "правление",
        "чемпион",
        "архив",
        "дискуссия",
        "освобождение",
        "сооружение",
        "копейка",
        "нерв",
        "раствор",
        "туалет",
        "хор",
        "анекдот",
        "сырье",
        "численность",
        "штаны",
        "плёнка",
        "пребывание",
        "репетиция",
        "отражение",
        "удовлетворение",
        "экипаж",
        "награда",
        "поход",
        "иностранец",
        "морда",
        "перо",
        "претензия",
        "россиянин",
        "проза",
        "четверть",
        "архитектура",
        "любопытство",
        "маска",
        "опера",
        "благодарность",
        "госпиталь",
        "маршрут",
        "сержант",
        "шляпа",
        "экземпляр",
        "бельё",
        "ведро",
        "возникновение",
        "мастерская",
        "офис",
        "слабость",
        "ссылка",
        "быт",
        "лампа",
        "толк",
        "убийца",
        "фантазия",
        "вселенная",
        "любитель",
        "малыш",
        "призыв",
        "облик",
        "преподаватель",
        "сайт",
        "горизонт",
        "дивизия",
        "изба",
        "копия",
        "регулирование",
        "натура",
        "доступ",
        "законопроект",
        "подвал",
        "порт",
        "уголь",
        "укрепление",
        "устав",
        "бассейн",
        "инстанция",
        "медведь",
        "отъезд",
        "архитектор",
        "космос",
        "меч",
        "охранник",
        "судно",
        "чашка",
        "воображение",
        "квартал",
        "курица",
        "преобразование",
        "топливо",
        "элита",
        "ветвь",
        "девица",
        "инвестор",
        "инструкция",
        "кнопка",
        "почта",
        "юмор",
        "палка",
        "пустота",
        "фонарь",
        "наркотик",
        "премьер",
        "актив",
        "пользователь",
        "соревнование",
        "столб",
        "француз",
        "холм",
        "большевик",
        "печь",
        "сиденье",
        "выплата",
        "отставка",
        "перестройка",
        "сорт",
        "трасса",
        "ангел",
        "лифт",
        "профессионал",
        "рекомендация",
        "градус",
        "запрос",
        "кабина",
        "приобретение",
        "террорист",
        "штраф",
        "медаль",
        "последовательность",
        "приглашение",
        "уголок",
        "фото",
        "перерыв",
        "разрыв",
        "департамент",
        "комментарий",
        "орудие",
        "отель",
        "эмоция",
        "аудитория",
        "жидкость",
        "коэффициент",
        "мышка",
        "ножка",
        "спутник",
        "верёвка",
        "взаимоотношения",
        "посольство",
        "старушка",
        "стать",
        "ступень",
        "борода",
        "дисциплина",
        "затылок",
        "невеста",
        "факультет",
        "группировка",
        "дыра",
        "император",
        "свеча",
        "вероятность",
        "блюдо",
        "грань",
        "критик",
        "организатор",
        "парламент",
        "посадка",
        "посуда",
        "царство",
        "экспорт",
        "боевик",
        "неприятность",
        "портфель",
        "конкуренция",
        "сторонник",
        "супруг",
        "персонал",
        "показание",
        "послание",
        "предоставление",
        "профиль",
        "учебник",
        "игрушка",
        "крышка",
        "наступление",
        "оформление",
        "понедельник",
        "премьера",
        "телеграмма",
        "высказывание",
        "конкурент",
        "ложь",
        "пёс",
        "скала",
        "грузовик",
        "перечень",
        "ритм",
        "такси",
        "улучшение",
        "философ",
        "воин",
        "жара",
        "олигарх",
        "строчка",
        "фрагмент",
        "бюро",
        "проспект",
        "самоуправление",
        "сборная",
        "зерно",
        "платёж",
        "потребление",
        "вред",
        "кафедра",
        "напиток",
        "яма",
        "англичанин",
        "лицензия",
        "пот",
        "пятница",
        "рота",
        "семейство",
        "сосна",
        "убеждение",
        "форум",
        "шампанское",
        "оттенок",
        "пустыня",
        "сборник",
        "военнослужащий",
        "объявление",
        "поза",
        "блеск",
        "папка",
        "размышление",
        "современник",
        "авария",
        "прогулка",
        "секс",
        "среднее",
        "степь",
        "бизнесмен",
        "дура",
        "ковёр",
        "пламя",
        "старость",
        "стрельба",
        "верх",
        "внедрение",
        "вступление",
        "рана",
        "треть",
        "ус",
        "флаг",
        "чистота",
        "крепость",
        "психолог",
        "расположение",
        "фаза",
        "асфальт",
        "валюта",
        "палатка",
        "перевозка",
        "стойка",
        "холодильник",
        "ветеран",
        "вор",
        "выделение",
        "листок",
        "предок",
        "расследование",
        "решетка",
        "скамейка",
        "торжество",
        "экспертиза",
        "делегация",
        "миф",
        "соблюдение",
        "ступенька",
        "турнир",
        "щель",
        "гнев",
        "льгота",
        "менеджер",
        "предположение",
        "батарея",
        "контекст",
        "познание",
        "железо",
        "избиратель",
        "рейтинг",
        "скважина",
        "специальность",
        "аргумент",
        "дверца",
        "огонёк",
        "прогресс",
        "строитель",
        "ген",
        "певец",
        "ремень",
        "соперник",
        "соседка",
        "завтрак",
        "подозрение",
        "ружьё",
        "балкон",
        "заслуга",
        "изготовление",
        "контора",
        "огород",
        "приложение",
        "речка",
        "социализм",
        "терроризм",
        "уничтожение",
        "эра",
        "трамвай",
        "финал",
        "галстук",
        "колбаса",
        "математика",
        "независимость",
        "патриарх",
        "реконструкция",
        "набережная",
        "недра",
        "переживание",
        "баланс",
        "девка",
        "знамя",
        "медицина",
        "юбилей",
        "кирпич",
        "несчастье",
        "плащ",
        "похороны",
        "рюмка",
        "совершенствование",
        "исход",
        "ликвидация",
        "оболочка",
        "общее",
        "поступление",
        "усталость",
        "школьник",
        "аренда",
        "комбинат",
        "местность",
        "нападение",
        "биография",
        "допрос",
        "звено",
        "лишение",
        "модернизация",
        "футбол",
        "грипп",
        "запрет",
        "командировка",
        "общественность",
        "протест",
        "суп",
        "каша",
        "командование",
        "муха",
        "педагог",
        "видение",
        "интонация",
        "общежитие",
        "печка",
        "рассуждение",
        "семинар",
        "собственник",
        "спектр",
        "турист",
        "шёпот",
        "молния",
        "охотник",
        "пилот",
        "плоть",
        "семя",
        "завершение",
        "композиция",
        "самец",
        "фракция",
        "череп",
        "подача",
        "разрушение",
        "бочка",
        "гараж",
        "коммунизм",
        "носитель",
        "подтверждение",
        "совокупность",
        "тренировка",
        "алгоритм",
        "глазок",
        "корпорация",
        "насилие",
        "президиум",
        "принадлежность",
        "провод",
        "близость",
        "возрождение",
        "заключенный",
        "кружка",
        "мяч",
        "референдум",
        "сосуд",
        "спальня",
        "диск",
        "поэма",
        "размещение",
        "склон",
        "юбка",
        "акцент",
        "ванная",
        "инвалид",
        "лавка",
        "пар",
        "платформа",
        "финансы",
        "цепочка",
        "бокал",
        "кисть",
        "подбородок",
        "бар",
        "голосование",
        "идиот",
        "инфекция",
        "килограмм",
        "койка",
        "облегчение",
        "привлечение",
        "рай",
        "трибуна",
        "заявка",
        "обычай",
        "резерв",
        "тур",
        "якорь",
        "диплом",
        "дрова",
        "живопись",
        "учительница",
        "ядро",
        "ворот",
        "загадка",
        "заметка",
        "окошко",
        "опора",
        "приватизация",
        "учёба",
        "шинель",
        "конструктор",
        "ноготь",
        "предупреждение",
        "национальность",
        "отечество",
        "топор",
        "физик",
        "филиал",
        "барон",
        "владыка",
        "диаметр",
        "конверт",
        "митрополит",
        "пляж",
        "покрытие",
        "спортсмен",
        "тест",
        "барак",
        "болото",
        "евангелие",
        "кукла",
        "прекращение",
        "совершение",
        "старшина",
        "эволюция",
        "жених",
        "лозунг",
        "носок",
        "патрон",
        "поручение",
        "сыворотка",
        "неудача",
        "рассвет",
        "сволочь",
        "стабильность",
        "эфир",
        "мелодия",
        "обувь",
        "ширина",
        "гастроли",
        "намёк",
        "раб",
        "раздражение",
        "раненый",
        "котёл",
        "мастерство",
        "освоение",
        "полнота",
        "принц",
        "пруд",
        "колебание",
        "снаряд",
        "зелень",
        "индивид",
        "лук",
        "популярность",
        "пушка",
        "ток",
        "крем",
        "пароход",
        "звон",
        "инфраструктура",
        "машинка",
        "обещание",
        "препятствие",
        "слушатель",
        "супруга",
        "бедро",
        "иллюзия",
        "наслаждение",
        "пейзаж",
        "разделение",
        "упражнение",
        "внешность",
        "гвоздь",
        "гостиная",
        "недоумение",
        "порыв",
        "сетка",
        "союзник",
        "поставщик",
        "проход",
        "советник",
        "квалификация",
        "колодец",
        "пенсионер",
        "реплика",
        "сарай",
        "словарь",
        "туча",
        "мудрость",
        "опрос",
        "подполковник",
        "провинция",
        "простор",
        "слон",
        "чин",
        "ад",
        "альбом",
        "вена",
        "обследование",
        "святой",
        "старт",
        "шкура",
        "антитело",
        "близкие",
        "вспышка",
        "драка",
        "младенец",
        "нота",
        "оппозиция",
        "пособие",
        "происшествие",
        "взор",
        "жалость",
        "защитник",
        "посещение",
        "постройка",
        "проба",
        "снимок",
        "стук",
        "тетрадь",
        "японец",
        "казак",
        "кислота",
        "мусор",
        "отверстие",
        "правонарушение",
        "христианин",
        "дефицит",
        "кран",
        "нежность",
        "переводчик",
        "спасибо",
        "галерея",
        "держава",
        "зависть",
        "моряк",
        "прохожий",
        "рог",
        "самка",
        "туфля",
        "химия",
        "велосипед",
        "вечность",
        "ерунда",
        "инспектор",
        "мадам",
        "племя",
        "свинья",
        "тротуар",
        "функционирование",
        "хрен",
        "батальон",
        "коррупция",
        "кубок",
        "родные",
        "стрелка",
        "аромат",
        "неожиданность",
        "поклонник",
        "трактор",
        "усиление",
        "ягода",
        "битва",
        "головка",
        "зрелище",
        "монах",
        "приоритет",
        "тряпка",
        "грамм",
        "дырка",
        "ил",
        "комедия",
        "отмена",
        "побег",
        "провал",
        "разряд",
        "сходство",
        "тыл",
        "базар",
        "бревно",
        "дежурный",
        "драма",
        "климат",
        "наследник",
        "наследство",
        "недвижимость",
        "нить",
        "переработка",
        "пропаганда",
        "равновесие",
        "синтез",
        "спинка",
        "должник",
        "ранг",
        "стыд",
        "христианство",
        "выявление",
        "график",
        "доза",
        "интерьер",
        "королева",
        "кредитор",
        "отбор",
        "приз",
        "уменьшение",
        "урожай",
        "барьер",
        "духи",
        "ёлка",
        "захват",
        "клиника",
        "надзор",
        "частота",
        "бык",
        "гитара",
        "лужа",
        "плен",
        "рак",
        "спирт",
        "кузов",
        "материя",
        "матрос",
        "развод",
        "сантиметр",
        "создатель",
        "хирург",
        "бензин",
        "ванна",
        "государь",
        "желудок",
        "изобретение",
        "миссия",
        "рейс",
        "терпение",
        "грохот",
        "жир",
        "канон",
        "компенсация",
        "представительство",
        "репутация",
        "рыбак",
        "устойчивость",
        "щит",
        "апостол",
        "бульвар",
        "выдача",
        "уравнение",
        "четверг",
        "энергетика",
        "ансамбль",
        "диссертация",
        "заяц",
        "капуста",
        "комиссар",
        "консультация",
        "лампочка",
        "подоконник",
        "ребро",
        "столкновение",
        "приключение",
        "простота",
        "фрукт",
        "вдова",
        "взятка",
        "включение",
        "гармония",
        "община",
        "ось",
        "пулемет",
        "транслит",
        "цитата",
        "чайник",
        "балет",
        "весть",
        "казнь",
        "командующий",
        "окраина",
        "пророк",
        "спаситель",
        "бабочка",
        "навык",
        "пение",
        "пробка",
        "ручей",
        "сэр",
        "бред",
        "лёгкость",
        "молекула",
        "разведчик",
        "рама",
        "рубаха",
        "таблетка",
        "взвод",
        "дистанция",
        "кислород",
        "палочка",
        "поцелуй",
        "прихожая",
        "сессия",
        "служащий",
        "убыток",
        "увлечение",
        "коллегия",
        "окружающее",
        "сумерки",
        "ярость",
        "банкротство",
        "касса",
        "лауреат",
        "прыжок",
        "тройка",
        "щенок",
        "вздох",
        "выходной",
        "гнездо",
        "демонстрация",
        "феномен",
        "юрист",
        "возбуждение",
        "героиня",
        "дополнение",
        "ирония",
        "официант",
        "специфика",
        "стрела",
        "чеченец",
        "шахта",
        "воплощение",
        "выгода",
        "дизайн",
        "милость",
        "очко",
        "потомок",
        "рецепт",
        "атом",
        "букет",
        "верность",
        "змея",
        "китаец",
        "мораль",
        "оказание",
        "полотенце",
        "сопровождение",
        "суждение",
        "тираж",
        "шпион",
        "алкоголь",
        "жилец",
        "осколок",
        "палуба",
        "превращение",
        "спокойствие",
        "тезис",
        "электроэнергия",
        "берёза",
        "важность",
        "веко",
        "задолженность",
        "залог",
        "накопление",
        "пещера",
        "приступ",
        "проектирование",
        "спичка",
        "струя",
        "тактика",
        "уста",
        "внесение",
        "здравоохранение",
        "мужество",
        "наименование",
        "плакат",
        "психика",
        "электричество",
        "буря",
        "закономерность",
        "очерк",
        "производительность",
        "церемония",
        "аэродром",
        "буфет",
        "винтовка",
        "жажда",
        "надёжность",
        "плоскость",
        "плотность",
        "сдача",
        "скот",
        "творение",
        "выработка",
        "гипотеза",
        "зад",
        "заложник",
        "инстинкт",
        "кружок",
        "максимум",
        "овощи",
        "полдень",
        "совпадение",
        "интерпретация",
        "классификация",
        "конгресс",
        "кончик",
        "перчатка",
        "поддержание",
        "пропасть",
        "пуговица",
        "толщина",
        "диапазон",
        "наблюдатель",
        "нитка",
        "орёл",
        "поляна",
        "проволока",
        "рояль",
        "аплодисменты",
        "конфета",
        "легкое",
        "пацан",
        "суета",
        "травма",
        "украшение",
        "видимость",
        "козёл",
        "лопата",
        "наряд",
        "пустяк",
        "сыр",
        "тайга",
        "вмешательство",
        "выпускник",
        "дальнейшее",
        "декорация",
        "джинсы",
        "излучение",
        "корзина",
        "невозможность",
        "ориентация",
        "петля",
        "печаль",
        "полотно",
        "прелесть",
        "разнообразие",
        "упор",
        "владение",
        "вторник",
        "дьявол",
        "епископ",
        "пласт",
        "подружка",
        "полоска",
        "пресс-конференция",
        "пропуск",
        "редкость",
        "бригадир",
        "дипломат",
        "колония",
        "коса",
        "литератор",
        "обман",
        "отходы",
        "печень",
        "премьер-министр",
        "витрина",
        "госпожа",
        "кинотеатр",
        "код",
        "плюс",
        "шарик",
        "шахматы",
        "батюшка",
        "диагноз",
        "долина",
        "обнаружение",
        "огурец",
        "открытка",
        "прощение",
        "рыцарь",
        "рычаг",
        "симпатия",
        "стихия",
        "горечь",
        "злоба",
        "кора",
        "любовник",
        "мороженое",
        "секретарша",
        "трещина",
        "марш",
        "нуль",
        "ограда",
        "песнь",
        "политбюро",
        "предпочтение",
        "реформирование",
        "чаша",
        "шуба",
        "аппаратура",
        "витамин",
        "доставка",
        "заработок",
        "мент",
        "неправда",
        "прощание",
        "содействие",
        "стадо",
        "толчок",
        "агрегат",
        "бедность",
        "беспокойство",
        "калитка",
        "мёд",
        "окраска",
        "повреждение",
        "совершенство",
        "татарин",
        "вакцина",
        "глина",
        "мотоцикл",
        "полицейский",
        "скрипка",
        "сука",
        "учащийся",
        "фонтан",
        "бег",
        "восхищение",
        "выезд",
        "закат",
        "интеграция",
        "лыжа",
        "повестка",
        "продвижение",
        "соотечественник",
        "сочувствие",
        "становление",
        "договорённость",
        "планирование",
        "простыня",
        "служение",
        "спецслужба",
        "физиономия",
        "аналитик",
        "жительство",
        "интеллигент",
        "приготовление",
        "примета",
        "развлечение",
        "висок",
        "гад",
        "даль",
        "евро",
        "индекс",
        "классик",
        "маршал",
        "обзор",
        "отчётность",
        "путешественник",
        "завет",
        "искра",
        "погон",
        "употребление",
        "хаос",
        "взнос",
        "возврат",
        "дуб",
        "жар",
        "значимость",
        "луг",
        "нрав",
        "одиночка",
        "профсоюз",
        "слуга",
        "случайность",
        "телега",
        "воротник",
        "импульс",
        "классика",
        "крыса",
        "крючок",
        "культ",
        "объятие",
        "переписка",
        "санаторий",
        "скульптура",
        "сынок",
        "формулировка",
        "хата",
        "шов",
        "деяние",
        "довод",
        "кандидатура",
        "комплект",
        "находка",
        "отвращение",
        "солнышко",
        "спуск",
        "стадион",
        "чайка",
        "гром",
        "грунт",
        "монета",
        "обыск",
        "овца",
        "особняк",
        "станок",
        "стоянка",
        "десятка",
        "злость",
        "капитализм",
        "кошмар",
        "наследие",
        "обезьяна",
        "перемещение",
        "православие",
        "распад",
        "самоубийство",
        "сражение",
        "старичок",
        "усадьба",
        "го",
        "заря",
        "истребитель",
        "комбинация",
        "олимпиада",
        "очаг",
        "папироса",
        "энтузиазм",
        "ять",
        "величие",
        "заряд",
        "компромисс",
        "купол",
        "отзыв",
        "преодоление",
        "репертуар",
        "рюкзак",
        "сено",
        "согласование",
        "фундамент",
        "жилище",
        "изумление",
        "насекомое",
        "освещение",
        "плавание",
        "пленум",
        "позор",
        "прочность",
        "разбойник",
        "стройка",
        "челюсть",
        "аукцион",
        "благополучие",
        "вопль",
        "гражданство",
        "катер",
        "опасение",
        "полночь",
        "прилавок",
        "чертёж",
        "эмиграция",
        "эшелон",
        "возражение",
        "интрига",
        "квадрат",
        "обитатель",
        "порция",
        "секция",
        "старец",
        "цыган",
        "шерсть",
        "ясность",
        "вал",
        "грамота",
        "доброта",
        "документация",
        "занятость",
        "коляска",
        "консерватория",
        "порошок",
        "преступность",
        "размах",
        "расстрел",
        "составление",
        "труппа",
        "уплата",
        "фашист",
        "экономист",
        "алмаз",
        "интервал",
        "купе",
        "литр",
        "минус",
        "обряд",
        "переворот",
        "поляк",
        "предчувствие",
        "тупик",
        "упаковка",
        "экспозиция",
        "дракон",
        "импорт",
        "инфляция",
        "мыло",
        "налогообложение",
        "недоверие",
        "панель",
        "разочарование",
        "сюрприз",
        "тропа",
        "беженец",
        "досада",
        "древесина",
        "издержки",
        "коммуникация",
        "любовница",
        "модуль",
        "нетерпение",
        "певица",
        "подобие",
        "поединок",
        "рельс",
        "святитель",
        "участковый",
        "яд",
        "внучка",
        "горожанин",
        "дилер",
        "желающий",
        "кооператив",
        "митинг",
        "моделирование",
        "обложка",
        "обоснование",
        "парадокс",
        "ресница",
        "рождество",
        "удостоверение",
        "ферма",
        "вице-премьер",
        "горка",
        "горшок",
        "декларация",
        "дизайнер",
        "каблук",
        "кассета",
        "микрофон",
        "недовольство",
        "осмотр",
        "отклонение",
        "паника",
        "пирог",
        "пожелание",
        "походка",
        "почерк",
        "равенство",
        "рекорд",
        "сталь",
        "табак",
        "тяга",
        "шоу",
        "коза",
        "монолог",
        "ниша",
        "почка",
        "пятерка",
        "свод",
        "тварь",
        "душ",
        "заготовка",
        "залив",
        "испуг",
        "конвенция",
        "медсестра",
        "мрак",
        "мундир",
        "пузырь",
        "статуя",
        "тысячелетие",
        "удобство",
        "аллея",
        "бездна",
        "величество",
        "вилка",
        "гул",
        "иерархия",
        "компетенция",
        "матушка",
        "оппонент",
        "петух",
        "пик",
        "пирамида",
        "покойник",
        "поручик",
        "противостояние",
        "разбирательство",
        "рожа",
        "руда",
        "связка",
        "урегулирование",
        "фокус",
        "аналог",
        "занавес",
        "знаток",
        "кастрюля",
        "концерн",
        "мат",
        "монополия",
        "насаждение",
        "показ",
        "продолжительность",
        "резолюция",
        "творец",
        "глоток",
        "жестокость",
        "колокол",
        "мерседес",
        "овраг",
        "ответчик",
        "племянник",
        "экскурсия",
        "барабан",
        "беременность",
        "верхушка",
        "виза",
        "возмещение",
        "гонщик",
        "гроза",
        "занавеска",
        "интенсивность",
        "кристалл",
        "орех",
        "песенка",
        "полюс",
        "принцесса",
        "проезд",
        "русло",
        "утрата",
        "электричка",
        "администратор",
        "будка",
        "верста",
        "граната",
        "дева",
        "заговор",
        "известность",
        "обновление",
        "окрестность",
        "олень",
        "оправдание",
        "пена",
        "писание",
        "приближение",
        "прозвище",
        "прохождение",
        "стража",
        "толкование",
        "шок",
        "бухгалтер",
        "ввод",
        "догадка",
        "икра",
        "кличка",
        "континент",
        "модификация",
        "роскошь",
        "рубка",
        "рыбка",
        "сокровище",
        "тропинка",
        "трусы",
        "формат",
        "биология",
        "зам",
        "красавец",
        "новинка",
        "помидор",
        "причёска",
        "рецензия",
        "салат",
        "экономия",
        "эстрада",
        "аппетит",
        "банда",
        "борец",
        "интуиция",
        "контур",
        "манёвр",
        "обрыв",
        "отметка",
        "проживание",
        "разрез",
        "ревность",
        "теракт",
        "фуражка",
        "хохот",
        "шина",
        "валенок",
        "диагностика",
        "заблуждение",
        "заповедь",
        "каникулы",
        "комар",
        "контейнер",
        "координата",
        "люк",
        "майка",
        "нары",
        "парус",
        "перец",
        "пират",
        "сегмент",
        "стенд",
        "тигр",
        "устранение",
        "аптека",
        "бак",
        "безумие",
        "дежурство",
        "ислам",
        "кустарник",
        "мёртвый",
        "мониторинг",
        "откровение",
        "павильон",
        "подчинённый",
        "проводник",
        "ранение",
        "санкция",
        "серебро",
        "составляющая",
        "табличка",
        "закуска",
        "календарь",
        "кулиса",
        "орбита",
        "очистка",
        "просвещение",
        "расписание",
        "свитер",
        "терраса",
        "укол",
        "упрёк",
        "холдинг",
        "эхо",
        "грусть",
        "лебедь",
        "первое",
        "призрак",
        "работодатель",
        "разработчик",
        "ритуал",
        "стая",
        "схватка",
        "террор",
        "хутор",
        "биржа",
        "взыскание",
        "вице-президент",
        "гимн",
        "задержка",
        "исток",
        "мощь",
        "налёт",
        "пересмотр",
        "пионер",
        "бал",
        "братец",
        "грек",
        "губерния",
        "джип",
        "дирижёр",
        "железа",
        "инспекция",
        "итальянец",
        "новичок",
        "обладатель",
        "обочина",
        "окоп",
        "побережье",
        "приёмная",
        "прорыв",
        "тормоз",
        "фасад",
        "аналогия",
        "восстание",
        "дерьмо",
        "истец",
        "космонавт",
        "маг",
        "пошлина",
        "пытка",
        "сервис",
        "скамья",
        "смелость",
        "табуретка",
        "тост",
        "турок",
        "фильтр",
        "хлопоты",
        "ассортимент",
        "воробей",
        "гимназия",
        "загрязнение",
        "котёнок",
        "мистер",
        "негр",
        "пехота",
        "презрение",
        "претендент",
        "прирост",
        "ребятишки",
        "стон",
        "треугольник",
        "трудящийся",
        "участь",
        "альтернатива",
        "анкета",
        "бутерброд",
        "возмущение",
        "гепатит",
        "интеллект",
        "предательство",
        "промысел",
        "райком",
        "сало",
        "сторож",
        "человечек",
        "ваза",
        "волокно",
        "грузин",
        "игла",
        "курсант",
        "магнитофон",
        "пальма",
        "складка",
        "сумочка",
        "теща",
        "шедевр",
        "этика",
        "юстиция",
        "бес",
        "веселье",
        "весы",
        "виски",
        "исчезновение",
        "обком",
        "персона",
        "правосудие",
        "предпосылка",
        "проститутка",
        "торг",
        "троллейбус",
        "фигурка",
        "влага",
        "деление",
        "закупка",
        "листва",
        "ложа",
        "мусульманин",
        "низ",
        "новое",
        "партизан",
        "подписание",
        "проповедь",
        "сеанс",
        "слияние",
        "совхоз",
        "сустав",
        "уборка",
        "упоминание",
        "бомж",
        "законодатель",
        "мэрия",
        "отрывок",
        "палач",
        "первенство",
        "покров",
        "предшественник",
        "разгар",
        "шут",
        "водоём",
        "география",
        "измена",
        "инструктор",
        "каталог",
        "клоун",
        "локомотив",
        "нахождение",
        "облигация",
        "переезд",
        "подбор",
        "раковина",
        "скидка",
        "такт",
        "треск",
        "башка",
        "иллюстрация",
        "истерика",
        "общность",
        "оптимизация",
        "репрессия",
        "сплав",
        "ссора",
        "струна",
        "артиллерия",
        "библия",
        "голубь",
        "запуск",
        "медик",
        "наставник",
        "полигон",
        "продавщица",
        "соратник",
        "спонсор",
        "унижение",
        "вознаграждение",
        "гусь",
        "давность",
        "насос",
        "обломок",
        "перепись",
        "потрясение",
        "раздумье",
        "рёв",
        "сияние",
        "скука",
        "старина",
        "удаление",
        "факс",
        "чепуха",
        "шкала",
        "адмирал",
        "арена",
        "веранда",
        "земляк",
        "издатель",
        "имидж",
        "комсомол",
        "крошка",
        "ласка",
        "обозначение",
        "подсудимый",
        "поп",
        "профилактика",
        "судопроизводство",
        "титул",
        "всадник",
        "казарма",
        "киоск",
        "матрица",
        "перекрёсток",
        "посылка",
        "прах",
        "скульптор",
        "таз",
        "чердак",
        "эмигрант",
        "ярмарка",
        "араб",
        "бедствие",
        "вложение",
        "генштаб",
        "гонорар",
        "драматург",
        "ёмкость",
        "консультант",
        "око",
        "парта",
        "повар",
        "розыск",
        "стаж",
        "стебель",
        "ходатайство",
        "амбиция",
        "дворник",
        "затея",
        "косметика",
        "кредитование",
        "массив",
        "менеджмент",
        "налогоплательщик",
        "оптимизм",
        "парад",
        "патриот",
        "передвижение",
        "подсчёт",
        "поклон",
        "превосходство",
        "пучок",
        "соблазн",
        "усмешка",
        "шлем",
        "агрессия",
        "балл",
        "банкир",
        "барышня",
        "газон",
        "жюри",
        "мех",
        "миграция",
        "нищета",
        "нора",
        "отпечаток",
        "посредник",
        "пристрастие",
        "пятка",
        "сейф",
        "утка",
        "достоверность",
        "заражение",
        "изложение",
        "крона",
        "лига",
        "математик",
        "механик",
        "обвиняемый",
        "отступление",
        "помеха",
        "презентация",
        "прикосновение",
        "промежуток",
        "растерянность",
        "синяк",
        "современность",
        "цензура",
        "энциклопедия",
        "арка",
        "варенье",
        "гимнастёрка",
        "ловушка",
        "мамочка",
        "местечко",
        "осознание",
        "полость",
        "предпринимательство",
        "престол",
        "примечание",
        "просмотр",
        "рывок",
        "сериал",
        "благодать",
        "блин",
        "блокнот",
        "вдохновение",
        "вектор",
        "взлёт",
        "змей",
        "исповедь",
        "кривая",
        "курорт",
        "лопатка",
        "негодяй",
        "острота",
        "пепел",
        "порок",
        "произвол",
        "птичка",
        "седло",
        "фара",
        "штурм",
        "арсенал",
        "боеприпас",
        "болельщик",
        "гигант",
        "задница",
        "купюра",
        "лавочка",
        "лик",
        "парочка",
        "повествование",
        "правота",
        "предлог",
        "проём",
        "силуэт",
        "скатерть",
        "уступка",
        "фашизм",
        "фотограф",
        "чушь",
        "адаптация",
        "ворона",
        "глагол",
        "коленка",
        "медь",
        "неопределённость",
        "ноздря",
        "поликлиника",
        "прапорщик",
        "сани",
        "торговец",
        "трансформация",
        "узор",
        "блондинка",
        "бокс",
        "выживание",
        "галактика",
        "динамо",
        "дрожь",
        "закрытие",
        "каюта",
        "кишка",
        "комендант",
        "купец",
        "противодействие",
        "самостоятельность",
        "сдвиг",
        "снятие",
        "содержимое",
        "сундук",
        "трубопровод",
        "увольнение",
        "цилиндр",
        "град",
        "европеец",
        "заросль",
        "зоопарк",
        "избыток",
        "кончина",
        "молоток",
        "поселение",
        "преследование",
        "приветствие",
        "респондент",
        "свалка",
        "стопка",
        "стресс",
        "теннис",
        "тонкость",
        "холл",
        "штора",
        "бремя",
        "гадость",
        "дальность",
        "заклинание",
        "законность",
        "картофель",
        "лад",
        "мамаша",
        "покаяние",
        "развалина",
        "сбыт",
        "фюрер",
        "бульон",
        "должное",
        "дуга",
        "колледж",
        "координация",
        "корм",
        "ландшафт",
        "морщина",
        "нищий",
        "одобрение",
        "осужденный",
        "партнёрство",
        "пафос",
        "перелом",
        "плач",
        "правитель",
        "преданность",
        "равнодушие",
        "регламент",
        "роща",
        "склонность",
        "трюк",
        "целостность",
        "чекист",
        "бесконечность",
        "ведьма",
        "верующий",
        "взаимосвязь",
        "застройка",
        "индустрия",
        "кепка",
        "миска",
        "мостовая",
        "муравей",
        "наложение",
        "насмешка",
        "основатель",
        "пирожок",
        "прибытие",
        "ректор",
        "сирота",
        "собачка",
        "тумбочка",
        "управляющий",
        "багаж",
        "безобразие",
        "ветерок",
        "демократ",
        "древность",
        "зять",
        "кавалер",
        "клапан",
        "комсомолец",
        "конёк",
        "корточки",
        "косточка",
        "логистика",
        "манеж",
        "месть",
        "методология",
        "механика",
        "мостик",
        "направленность",
        "оригинал",
        "продюсер",
        "путёвка",
        "разборка",
        "ремесло",
        "стереотип",
        "чулок",
        "вой",
        "гвардия",
        "дрянь",
        "клятва",
        "комфорт",
        "кролик",
        "кумир",
        "монтаж",
        "наркоман",
        "окурок",
        "повторение",
        "провокация",
        "разбор",
        "разногласие",
        "садик",
        "свист",
        "сенатор",
        "чудовище",
        "азарт",
        "бунт",
        "геном",
        "гудок",
        "джаз",
        "дядька",
        "квота",
        "липа",
        "мемуары",
        "метафора",
        "муза",
        "нравственность",
        "отклик",
        "пасть",
        "покушение",
        "рубрика",
        "себестоимость",
        "сопоставление",
        "справочник",
        "троица",
        "экология",
        "жук",
        "иммунитет",
        "казино",
        "клок",
        "комбайн",
        "лимон",
        "меньшинство",
        "мощи",
        "оскорбление",
        "отопление",
        "предатель",
        "пружина",
        "разновидность",
        "самовар",
        "симптом",
        "стопа",
        "хоккей",
        "артистка",
        "белка",
        "бешенство",
        "богослужение",
        "вырост",
        "дефект",
        "застолье",
        "инфаркт",
        "княгиня",
        "коробочка",
        "норматив",
        "обход",
        "отчество",
        "поднос",
        "разъяснение",
        "ролик",
        "славянин",
        "слюна",
        "тематика",
        "торт",
        "банкет",
        "депрессия",
        "звёздочка",
        "конюшня",
        "лён",
        "лопасть",
        "металлургия",
        "мыслитель",
        "напряжённость",
        "обилие",
        "обрывок",
        "очертание",
        "патриотизм",
        "пленный",
        "плитка",
        "подвеска",
        "соседство",
        "тетрадка",
        "фунт",
        "хулиган",
        "шапочка",
        "эвакуация",
        "бас",
        "вывеска",
        "гектар",
        "добавка",
        "езда",
        "конвейер",
        "кошелёк",
        "меню",
        "обаяние",
        "оклад",
        "паровоз",
        "погоня",
        "пресс-служба",
        "пьянство",
        "старший",
        "товарищество",
        "часовой",
        "чувствительность",
        "бетон",
        "выпивка",
        "лезвие",
        "настоящее",
        "нехватка",
        "паутина",
        "романс",
        "салфетка",
        "скорбь",
        "хитрость",
        "верблюд",
        "воскресение",
        "доцент",
        "идентификация",
        "изоляция",
        "камин",
        "ком",
        "котлета",
        "лихорадка",
        "обыватель",
        "перегородка",
        "пир",
        "попадание",
        "правильность",
        "предисловие",
        "призвание",
        "приток",
        "прокат",
        "пульт",
        "стих",
        "студентка",
        "хребет",
        "электрон",
        "бегство",
        "гамма",
        "епархия",
        "индивидуальность",
        "консервы",
        "мерка",
        "мировоззрение",
        "неприязнь",
        "ноль",
        "осторожность",
        "подчинение",
        "приспособление",
        "проникновение",
        "пуск",
        "сближение",
        "снабжение",
        "сугроб",
        "титр",
        "траектория",
        "убитый",
        "ущелье",
        "хроника",
        "ячейка",
        "армянин",
        "афиша",
        "беспорядок",
        "вахта",
        "груда",
        "ель",
        "забава",
        "зонтик",
        "кипяток",
        "коммерсант",
        "комнатка",
        "милосердие",
        "обитель",
        "похвала",
        "профессионализм",
        "родня",
        "скотина",
        "транспортировка",
        "ускорение",
        "утешение",
        "царица",
        "юрисдикция",
        "авиакомпания",
        "алюминий",
        "атрибут",
        "бюллетень",
        "вечеринка",
        "выступ",
        "гол",
        "донос",
        "завещание",
        "заявитель",
        "инерция",
        "инициатор",
        "колонка",
        "незнакомец",
        "ориентир",
        "особа",
        "подошва",
        "пристань",
        "притча",
        "развал",
        "реорганизация",
        "средневековье",
        "темперамент",
        "теоретик",
        "трактовка",
        "шоколад",
        "шорох",
        "штамп",
        "безработица",
        "благословение",
        "боярин",
        "выброс",
        "выручка",
        "дивизион",
        "достояние",
        "дружка",
        "ёжик",
        "нарком",
        "перенос",
        "поздравление",
        "потерпевший",
        "приёмник",
        "пьяница",
        "револьвер",
        "соус",
        "стимул",
        "страховщик",
        "бланк",
        "бор",
        "брюхо",
        "вращение",
        "вторжение",
        "детектив",
        "зажигалка",
        "значок",
        "кинематограф",
        "кит",
        "клочок",
        "копыто",
        "крах",
        "крокодил",
        "монография",
        "мотивация",
        "пан",
        "предание",
        "привилегия",
        "расстройство",
        "резиденция",
        "санитар",
        "свекровь",
        "сострадание",
        "шрам",
        "абонент",
        "бинокль",
        "близнец",
        "вертикаль",
        "вестибюль",
        "выдержка",
        "груша",
        "гусеница",
        "демон",
        "дирекция",
        "звучание",
        "знаменитость",
        "ископаемое",
        "катание",
        "комплимент",
        "крайность",
        "магия",
        "обобщение",
        "обои",
        "пассаж",
        "перила",
        "перрон",
        "радиостанция",
        "реестр",
        "скелет",
        "скрип",
        "талия",
        "тесто",
        "футболист",
        "чистка",
        "штык",
        "архиепископ",
        "босс",
        "вышка",
        "гребень",
        "доктрина",
        "заём",
        "землетрясение",
        "изобретатель",
        "изъятие",
        "иномарка",
        "канцелярия",
        "конкурентоспособность",
        "ларёк",
        "лягушка",
        "мельница",
        "моча",
        "оговорка",
        "поросёнок",
        "правоотношение",
        "сверстник",
        "сенсация",
        "сквер",
        "социология",
        "таинство",
        "умница",
        "уточнение",
        "чернила",
        "широта",
        "эстетика",
        "автомашина",
        "братство",
        "вилла",
        "виноград",
        "воспроизводство",
        "выдумка",
        "инцидент",
        "клей",
        "колхозник",
        "котелок",
        "лак",
        "лапка",
        "листочек",
        "литургия",
        "отдельность",
        "пальчик",
        "популяция",
        "прикрытие",
        "прописка",
        "пух",
        "пшеница",
        "сборка",
        "сертификат",
        "тележка",
        "эпидемия",
        "автограф",
        "багажник",
        "богатырь",
        "ведущий",
        "визг",
        "выигрыш",
        "индеец",
        "кучка",
        "леди",
        "лорд",
        "несправедливость",
        "обморок",
        "отрезок",
        "подлец",
        "пострадавший",
        "пчела",
        "разлука",
        "рукоятка",
        "симфония",
        "скула",
        "стан",
        "таракан",
        "уборная",
        "абзац",
        "всплеск",
        "дружок",
        "залп",
        "землянка",
        "знакомая",
        "кол",
        "конунг",
        "коттедж",
        "маркетинг",
        "машинист",
        "оперативник",
        "пасха",
        "продовольствие",
        "прожектор",
        "расцвет",
        "родство",
        "солидарность",
        "спецназ",
        "таможня",
        "телеканал",
        "франк",
        "штучка",
        "альянс",
        "бакс",
        "бугор",
        "волшебник",
        "выборка",
        "доброволец",
        "заведующий",
        "злодей",
        "зрачок",
        "искренность",
        "истечение",
        "клавиша",
        "конвой",
        "мишень",
        "мудрец",
        "мучение",
        "напор",
        "отделка",
        "пианист",
        "радиус",
        "реализм",
        "репортаж",
        "содружество",
        "трепет",
        "фонарик",
        "электростанция",
        "яблоня",
        "алкоголик",
        "выяснение",
        "датчик",
        "заинтересованность",
        "казна",
        "копьё",
        "крещение",
        "марксизм",
        "миллионер",
        "островок",
        "официантка",
        "паренёк",
        "патент",
        "повязка",
        "погашение",
        "попугай",
        "разгром",
        "решимость",
        "стипендия",
        "тундра",
        "удобрение",
        "форточка",
        "шашлык",
        "балка",
        "ближний",
        "вексель",
        "весло",
        "дворянин",
        "диктатура",
        "зараза",
        "зачёт",
        "изобилие",
        "кубик",
        "люстра",
        "машиностроение",
        "нежелание",
        "осуждение",
        "папаша",
        "парашют",
        "паук",
        "подписка",
        "преграда",
        "прогнозирование",
        "реабилитация",
        "учредитель",
        "целесообразность",
        "червь",
        "щётка",
        "авангард",
        "главнокомандующий",
        "досуг",
        "запястье",
        "засада",
        "кабель",
        "конъюнктура",
        "лесхоз",
        "любимец",
        "магистраль",
        "нанесение",
        "недоразумение",
        "одноклассник",
        "помойка",
        "последователь",
        "пропорция",
        "рок",
        "солома",
        "странность",
        "техникум",
        "тополь",
        "экспонат",
        "антиген",
        "вихрь",
        "воспитатель",
        "выходец",
        "гарнизон",
        "гормон",
        "заповедник",
        "императрица",
        "контраст",
        "лёт",
        "монитор",
        "номенклатура",
        "причал",
        "пустырь",
        "резина",
        "сослуживец",
        "суша",
        "терминология",
        "трап",
        "четвёрка",
        "чутье",
        "блаженство",
        "божество",
        "бриллиант",
        "венец",
        "водород",
        "волосок",
        "газопровод",
        "глотка",
        "дружина",
        "заголовок",
        "компартия",
        "косяк",
        "ладошка",
        "минутка",
        "нюанс",
        "пастух",
        "предотвращение",
        "прут",
        "рабство",
        "реструктуризация",
        "синдром",
        "словечко",
        "смущение",
        "уполномоченный",
        "шарф",
        "шашка",
        "язва",
        "геолог",
        "геометрия",
        "детишки",
        "конфигурация",
        "кромка",
        "лепесток",
        "либерал",
        "массаж",
        "многообразие",
        "напарник",
        "привод",
        "рельеф",
        "рудник",
        "самолюбие",
        "серьёзность",
        "спираль",
        "старание",
        "ступня",
        "урод",
        "чудак",
        "яхта",
        "вестник",
        "винт",
        "воровство",
        "въезд",
        "вылет",
        "глубь",
        "двойка",
        "задержание",
        "иголка",
        "интеллектуал",
        "карета",
        "колокольчик",
        "корма",
        "корона",
        "надобность",
        "негодование",
        "опоздание",
        "передел",
        "псевдоним",
        "пульс",
        "сидение",
        "спад",
        "стабилизация",
        "страховка",
        "тапочка",
        "терапия",
        "украинец",
        "хищник",
        "цыпочки",
        "чек",
        "шишка",
        "акула",
        "благосостояние",
        "ведомость",
        "второе",
        "вычисление",
        "грош",
        "дуэт",
        "жизнедеятельность",
        "избрание",
        "караул",
        "лабиринт",
        "нарушитель",
        "небытие",
        "осложнение",
        "отправка",
        "отсек",
        "пень",
        "поговорка",
        "прицел",
        "пришелец",
        "приют",
        "психиатр",
        "равнина",
        "расправа",
        "рыбалка",
        "свита",
        "слесарь",
        "соавтор",
        "триумф",
        "хранитель",
        "алтарь",
        "антисемитизм",
        "вежливость",
        "гендиректор",
        "костыль",
        "кража",
        "лилия",
        "миля",
        "минерал",
        "няня",
        "пепельница",
        "пила",
        "полвека",
        "реставрация",
        "саммит",
        "свежесть",
        "сохранность",
        "социолог",
        "специализация",
        "спикер",
        "срыв",
        "стрелок",
        "факел",
        "чума",
        "шланг",
        "абсурд",
        "арбуз",
        "борщ",
        "венок",
        "воронка",
        "вранье",
        "дань",
        "джентльмен",
        "диета",
        "жадность",
        "жила",
        "журналистика",
        "имение",
        "испытуемый",
        "корка",
        "крюк",
        "несоответствие",
        "ниточка",
        "номинация",
        "обыкновение",
        "помада",
        "противоположность",
        "рапорт",
        "самосознание",
        "убежище",
        "углубление",
        "цветочек",
        "чемоданчик",
        "брызги",
        "гель",
        "дуэль",
        "ион",
        "канат",
        "коалиция",
        "котельная",
        "написание",
        "опухоль",
        "присоединение",
        "раскол",
        "рубин",
        "святыня",
        "сметана",
        "телохранитель",
        "автономия",
        "баран",
        "годовщина",
        "гостья",
        "дифференциация",
        "единомышленник",
        "жид",
        "кон",
        "контингент",
        "мерзавец",
        "монстр",
        "мох",
        "наведение",
        "непонимание",
        "открытость",
        "отсчёт",
        "пазуха",
        "пословица",
        "приезжий",
        "прутик",
        "пузырек",
        "растительность",
        "ребёночек",
        "революционер",
        "сени",
        "спиртное",
        "сплетня",
        "таксист",
        "туризм",
        "упорство",
        "фойе",
        "холст",
        "шнур",
        "бдительность",
        "вешалка",
        "водоросль",
        "грузчик",
        "дворик",
        "драгоценность",
        "духовенство",
        "жопа",
        "злоупотребление",
        "капелька",
        "кашель",
        "колея",
        "лом",
        "мафия",
        "мертвец",
        "морковь",
        "неловкость",
        "обострение",
        "пограничник",
        "подавление",
        "подсистема",
        "празднование",
        "престиж",
        "раскрытие",
        "рация",
        "роса",
        "суверенитет",
        "теорема",
        "удочка",
        "улочка",
        "фольклор",
        "агитация",
        "будни",
        "быстрота",
        "воевода",
        "возлюбленная",
        "выписка",
        "делегат",
        "износ",
        "калина",
        "канава",
        "каркас",
        "клин",
        "клумба",
        "комок",
        "лиса",
        "листовка",
        "макушка",
        "ножницы",
        "нотариус",
        "отрицание",
        "охлаждение",
        "пересечение",
        "позвоночник",
        "полемика",
        "полуостров",
        "робот",
        "солист",
        "станица",
        "стержень",
        "травка",
        "трон",
        "унитаз",
        "утюг",
        "фантастика",
        "артерия",
        "боб",
        "вынесение",
        "гимнастика",
        "гласность",
        "гражданка",
        "грант",
        "диаграмма",
        "женитьба",
        "идентичность",
        "избушка",
        "китель",
        "коктейль",
        "конечность",
        "линейка",
        "мэтр",
        "нива",
        "педаль",
        "подлость",
        "подрядчик",
        "предназначение",
        "прививка",
        "реалия",
        "самодеятельность",
        "секретариат",
        "стаканчик",
        "сумрак",
        "фермер",
        "этикетка",
        "автоматизация",
        "аксиома",
        "аллах",
        "антенна",
        "благородство",
        "бросок",
        "бюрократия",
        "вариация",
        "взаимопонимание",
        "виновник",
        "глыба",
        "доходность",
        "затвор",
        "избранник",
        "изгиб",
        "исправление",
        "козырёк",
        "маяк",
        "отработка",
        "отчуждение",
        "подсознание",
        "полугодие",
        "рифма",
        "свечка",
        "серёжка",
        "сова",
        "теплоход",
        "типография",
        "трус",
        "фланг",
        "фотоаппарат",
        "хранилище",
        "чеснок",
        "честность",
        "электроэнергетика",
        "эскиз",
        "ассистент",
        "богородица",
        "болтовня",
        "бутылочка",
        "бухта",
        "государственность",
        "добавление",
        "доверенность",
        "искушение",
        "катушка",
        "легион",
        "манер",
        "нутро",
        "отдача",
        "паёк",
        "паркет",
        "планка",
        "пополнение",
        "привлекательность",
        "сатира",
        "сбережение",
        "селение",
        "скромность",
        "счётчик",
        "тачка",
        "трибунал",
        "финиш",
        "химик",
        "шелк",
        "электроника",
        "аборт",
        "аквариум",
        "аспирант",
        "бессмертие",
        "возглас",
        "замминистра",
        "зэк",
        "катализатор",
        "коготь",
        "красноармеец",
        "носилки",
        "оживление",
        "очевидец",
        "племянница",
        "превышение",
        "преувеличение",
        "привязанность",
        "просвет",
        "режиссура",
        "роспись",
        "сертификация",
        "сечение",
        "скопление",
        "слог",
        "смирение",
        "сувенир",
        "тоннель",
        "труженик",
        "туловище",
        "укрытие",
        "амнистия",
        "бедняга",
        "боязнь",
        "бритва",
        "вальс",
        "вата",
        "весь",
        "горючее",
        "дарование",
        "джунгли",
        "добродетель",
        "затруднение",
        "изолят",
        "кайф",
        "карьер",
        "кнут",
        "кондиционер",
        "лирика",
        "материк",
        "обитание",
        "оперетта",
        "оратор",
        "ослабление",
        "перераспределение",
        "подделка",
        "предъявление",
        "преемник",
        "проблематика",
        "пролетариат",
        "протяжённость",
        "самочувствие",
        "светильник",
        "сирень",
        "сковородка",
        "следующее",
        "сокол",
        "строфа",
        "трубочка",
        "уборщица",
        "фаворит",
        "черёд",
        "чукча",
        "шестёрка",
        "шляпка",
        "арендатор",
        "балерина",
        "бессилие",
        "богиня",
        "бомбардировщик",
        "броня",
        "возня",
        "выговор",
        "грим",
        "грядка",
        "действо",
        "диссидент",
        "жрец",
        "извлечение",
        "индикатор",
        "камешек",
        "каска",
        "колпак",
        "коммуна",
        "мгла",
        "мел",
        "мрамор",
        "оборотень",
        "останки",
        "откровенность",
        "полководец",
        "предписание",
        "пьяный",
        "радуга",
        "расхождение",
        "регулятор",
        "репродукция",
        "салют",
        "свёрток",
        "условность",
        "цветение",
        "шахтёр",
        "актуальность",
        "баржа",
        "беседка",
        "воз",
        "воспаление",
        "говно",
        "двойник",
        "дебют",
        "девиз",
        "длительность",
        "зубр",
        "идол",
        "извинение",
        "ключик",
        "комбат",
        "кооперация",
        "крушение",
        "лень",
        "лира",
        "ложечка",
        "лошадка",
        "макет",
        "матрас",
        "мигрант",
        "опека",
        "отросток",
        "патруль",
        "пельмень",
        "переменная",
        "плот",
        "погреб",
        "подол",
        "почёт",
        "предсказание",
        "проклятие",
        "реформатор",
        "светофор",
        "следование",
        "слушание",
        "снайпер",
        "спасатель",
        "техник",
        "толща",
        "трос",
        "футляр",
        "чашечка",
        "череда",
        "азот",
        "бессонница",
        "вывоз",
        "дядюшка",
        "завоевание",
        "зрелость",
        "карабин",
        "клан",
        "кобура",
        "коммуналка",
        "краб",
        "личико",
        "малость",
        "настоятель",
        "отложение",
        "печенье",
        "платочек",
        "половинка",
        "презумпция",
        "протоиерей",
        "рента",
        "символика",
        "синева",
        "сладость",
        "соловей",
        "строгость",
        "супермаркет",
        "телёнок",
        "ухудшение",
        "церка",
        "башмак",
        "больная",
        "господство",
        "духовность",
        "идеолог",
        "израильтянин",
        "каток",
        "колокольня",
        "метель",
        "мученик",
        "надзиратель",
        "неудобство",
        "отрыв",
        "подозреваемый",
        "полумрак",
        "потомство",
        "приведение",
        "приказание",
        "примирение",
        "разоблачение",
        "сводка",
        "струйка",
        "твёрдость",
        "тестирование",
        "уклон",
        "урна",
        "утечка",
        "финн",
        "шахматист",
        "шприц",
        "энергетик",
        "веник",
        "генератор",
        "герб",
        "графиня",
        "гримаса",
        "дурачок",
        "жулик",
        "закрепление",
        "зов",
        "калибр",
        "кофта",
        "мироздание",
        "недуг",
        "особь",
        "отбой",
        "педагогика",
        "петрушка",
        "пошлость",
        "преподавание",
        "примесь",
        "причастность",
        "прототип",
        "разграничение",
        "романтик",
        "романтика",
        "состязание",
        "стирка",
        "танкист",
        "теплота",
        "тесть",
        "траншея",
        "третья",
        "физкультура",
        "щи",
        "апельсин",
        "бинт",
        "богач",
        "вакцинация",
        "воспитанник",
        "генерал-майор",
        "дельфин",
        "дюжина",
        "заполнение",
        "квартирка",
        "корт",
        "кофточка",
        "мячик",
        "навоз",
        "обстрел",
        "оргазм",
        "осмысление",
        "ощупь",
        "парадигма",
        "пианино",
        "прозрачность",
        "расклад",
        "рассудок",
        "резинка",
        "ровесник",
        "сироп",
        "сырость",
        "трест",
        "усмотрение",
        "уют",
        "чех",
        "аспирантура",
        "бактерия",
        "бездействие",
        "будда",
        "видео",
        "вишня",
        "выращивание",
        "горком",
        "грабёж",
        "грубость",
        "док",
        "донор",
        "дымок",
        "ёж",
        "забастовка",
        "захоронение",
        "извозчик",
        "искажение",
        "капот",
        "керосин",
        "клад",
        "кровля",
        "курение",
        "лизинг",
        "лысина",
        "металлург",
        "мюзикл",
        "настройка",
        "невестка",
        "опушка",
        "осёл",
        "погружение",
        "подданный",
        "подполье",
        "преддверие",
        "придаток",
        "пристав",
        "разведение",
        "скачок",
        "смола",
        "сосиска",
        "судорога",
        "сыщик",
        "тайник",
        "трансляция",
        "центнер",
        "шнурок",
        "автопилот",
        "барин",
        "биолог",
        "богослов",
        "бродяга",
        "бульдозер",
        "бюст",
        "виновный",
        "выкуп",
        "глобализация",
        "депо",
        "дикарь",
        "дискотека",
        "заёмщик",
        "заход",
        "королевство",
        "либерализм",
        "мамонт",
        "никель",
        "отвод",
        "отсрочка",
        "отчисление",
        "развёртывание",
        "рентабельность",
        "репортёр",
        "свёкла",
        "свекла",
        "сервер",
        "смертность",
        "смещение",
        "собрат",
        "сопка",
        "створка",
        "страничка",
        "телогрейка",
        "тетушка",
        "уголовник",
        "целина",
        "активизация",
        "банан",
        "блокада",
        "брошюра",
        "бумажник",
        "вирусология",
        "водопад",
        "выемка",
        "житие",
        "забвение",
        "загрузка",
        "зенит",
        "карикатура",
        "клещ",
        "книжечка",
        "летопись",
        "лунка",
        "матка",
        "муниципалитет",
        "несостоятельность",
        "новизна",
        "обогащение",
        "озабоченность",
        "пирожное",
        "погром",
        "подземелье",
        "подножие",
        "посев",
        "пресечение",
        "работоспособность",
        "разгон",
        "селедка",
        "силовик",
        "субсидия",
        "сухарь",
        "табурет",
        "трактат",
        "устье",
        "утопия",
        "фермент",
        "хождение",
        "чело",
        "баночка",
        "бородка",
        "бухгалтерия",
        "влажность",
        "возобновление",
        "вредитель",
        "гардероб",
        "горлышко",
        "деформация",
        "жёсткость",
        "застой",
        "канун",
        "компот",
        "контрразведка",
        "корректировка",
        "купля-продажа",
        "лай",
        "лука",
        "малина",
        "малое",
        "морг",
        "мускул",
        "навес",
        "наручник",
        "ностальгия",
        "обозреватель",
        "оккупация",
        "параллель",
        "парик",
        "патология",
        "переплёт",
        "плотник",
        "покровитель",
        "полпред",
        "полуфабрикат",
        "послушание",
        "пресс",
        "пятилетка",
        "рейд",
        "реквизит",
        "слепой",
        "стрижка",
        "тендер",
        "уведомление",
        "усик",
        "филармония",
        "эталон",
        "яр",
        "аванс",
        "аттракцион",
        "безработный",
        "берёзка",
        "будильник",
        "ватник",
        "восьмёрка",
        "голубчик",
        "дизель",
        "жилка",
        "загс",
        "зарубежье",
        "зек",
        "исчисление",
        "каприз",
        "кара",
        "коврик",
        "корреляция",
        "краешек",
        "лапоть",
        "ливень",
        "мужичок",
        "мутация",
        "национализм",
        "невежество",
        "неизбежность",
        "огорчение",
        "ордер",
        "откос",
        "перевал",
        "перстень",
        "пешеход",
        "подонок",
        "покрывало",
        "полушубок",
        "привидение",
        "присвоение",
        "пророчество",
        "сабля",
        "сатана",
        "скрипач",
        "служитель",
        "созерцание",
        "сословие",
        "ссуда",
        "староста",
        "стойкость",
        "уговор",
        "уклад",
        "ученица",
        "чернота",
        "чучело",
        "шествие",
        "шторм",
        "экспансия",
        "аккорд",
        "аллергия",
        "безразличие",
        "бомбёжка",
        "веточка",
        "вражда",
        "вручение",
        "генетика",
        "изгнание",
        "инвестирование",
        "кант",
        "командор",
        "консолидация",
        "мандат",
        "мешочек",
        "нефтяник",
        "объективность",
        "ор",
        "оснащение",
        "отправление",
        "пешка",
        "похмелье",
        "придурок",
        "прокладка",
        "пряник",
        "псих",
        "пыл",
        "сбой",
        "сирена",
        "смятение",
        "сотрудница",
        "спешка",
        "торможение",
        "уран",
        "аномалия",
        "брюшко",
        "военкомат",
        "графин",
        "дырочка",
        "киллер",
        "красотка",
        "медвежонок",
        "наглость",
        "нефтепродукты",
        "обиход",
        "отход",
        "перечисление",
        "поводок",
        "поглощение",
        "подворотня",
        "пресс-секретарь",
        "прилив",
        "процветание",
        "пьянка",
        "рассказчик",
        "родник",
        "рядовой",
        "святость",
        "седина",
        "семечко",
        "сортир",
        "тракт",
        "узник",
        "ураган",
        "фига",
        "хлам",
        "шасси",
        "эмиссия",
        "арифметика",
        "вакуум",
        "горсть",
        "дробь",
        "жемчужина",
        "зайчик",
        "клубок",
        "криминал",
        "крупа",
        "кувшин",
        "кукуруза",
        "курьер",
        "ложе",
        "манипуляция",
        "мираж",
        "мыс",
        "периферия",
        "пластик",
        "полимер",
        "полис",
        "помещик",
        "поминки",
        "приклад",
        "прихожанин",
        "прозаик",
        "промах",
        "рефлекс",
        "роддом",
        "розыгрыш",
        "семинария",
        "сигара",
        "улика",
        "хлопок",
        "ходьба",
        "цемент",
        "цыплёнок",
        "четвереньки",
        "швед",
        "шип",
        "этнос",
        "адресат",
        "адъютант",
        "ария",
        "браслет",
        "бронза",
        "бум",
        "влечение",
        "восхождение",
        "генерал-лейтенант",
        "герцог",
        "гестапо",
        "губка",
        "декабрист",
        "десантник",
        "забой",
        "запчасти",
        "католик",
        "кинжал",
        "лицензирование",
        "личинка",
        "лужайка",
        "метла",
        "миллиметр",
        "монополист",
        "наивность",
        "наставление",
        "несовершенство",
        "опаска",
        "пакетик",
        "пародия",
        "побуждение",
        "портвейн",
        "презерватив",
        "продление",
        "промышленник",
        "разложение",
        "размножение",
        "резонанс",
        "ров",
        "родственница",
        "розочка",
        "руины",
        "свинец",
        "симметрия",
        "синоним",
        "сливки",
        "снаряжение",
        "столбик",
        "сторонка",
        "талон",
        "туберкулёз",
        "упадок",
        "фиг",
        "электорат",
        "этюд",
        "братия",
        "веер",
        "вкладчик",
        "возмездие",
        "вхождение",
        "генерация",
        "голосок",
        "денежки",
        "заведующая",
        "заседатель",
        "интерфейс",
        "испанец",
        "клоп",
        "клюв",
        "колдун",
        "крепление",
        "кузнец",
        "лоток",
        "мембрана",
        "монарх",
        "наращивание",
        "начинание",
        "неспособность",
        "новелла",
        "обозрение",
        "одеколон",
        "ожог",
        "папочка",
        "парикмахерская",
        "плазма",
        "подражание",
        "постулат",
        "похищение",
        "почтение",
        "присяга",
        "пробуждение",
        "прохлада",
        "реактор",
        "римлянин",
        "рукавица",
        "сауна",
        "светило",
        "секта",
        "сенат",
        "словесность",
        "созвездие",
        "солдатик",
        "трение",
        "тусовка",
        "умерший",
        "файл",
        "хуй",
        "энтузиаст",
        "бич",
        "богословие",
        "брезент",
        "бюджетник",
        "витязь",
        "волкодав",
        "вскрытие",
        "грабитель",
        "гриф",
        "донесение",
        "изолятор",
        "импровизация",
        "исполком",
        "кавказец",
        "квитанция",
        "кинофильм",
        "кластер",
        "комбинезон",
        "лось",
        "мавзолей",
        "мах",
        "мачта",
        "обжалование",
        "острие",
        "очищение",
        "панорама",
        "парнишка",
        "передышка",
        "пластина",
        "пленник",
        "политолог",
        "поприще",
        "порох",
        "приятельница",
        "прочтение",
        "прядь",
        "пьедестал",
        "расписка",
        "стеллаж",
        "сук",
        "тамбур",
        "тахта",
        "трапеза",
        "указатель",
        "флакон",
        "шакал",
        "ярус",
        "астрономия",
        "гибкость",
        "горох",
        "грешник",
        "десант",
        "диктатор",
        "династия",
        "драматургия",
        "завал",
        "карлик",
        "кишечник",
        "кнопочка",
        "корыто",
        "ласточка",
        "лимит",
        "маньяк",
        "напоминание",
        "настрой",
        "нововведение",
        "окисление",
        "округа",
        "отвага",
        "парикмахер",
        "пастырь",
        "перегрузка",
        "подключение",
        "попутчик",
        "предрассудок",
        "прецедент",
        "пята",
        "расспрос",
        "раунд",
        "рейх",
        "ромашка",
        "сапожок",
        "сговор",
        "скобка",
        "смазка",
        "соприкосновение",
        "туша",
        "тщеславие",
        "фельдшер",
        "хвоя",
        "шрифт",
        "штурман",
        "авто",
        "активист",
        "бармен",
        "беглец",
        "ввоз",
        "великан",
        "взрывчатка",
        "военнопленный",
        "достаток",
        "дымка",
        "живописец",
        "заезд",
        "залежь",
        "камушек",
        "клевета",
        "колечко",
        "колода",
        "компонента",
        "корешок",
        "мак",
        "макароны",
        "менталитет",
        "миниатюра",
        "мистика",
        "мобилизация",
        "могущество",
        "мольба",
        "нашествие",
        "неисправность",
        "неприятие",
        "нянька",
        "овчарка",
        "оздоровление",
        "опьянение",
        "отравление",
        "паста",
        "передовая",
        "полушарие",
        "пробел",
        "пролив",
        "прощанье",
        "пятачок",
        "развязка",
        "страж",
        "стык",
        "сукно",
        "султан",
        "узелок",
        "умиление",
        "утёс",
        "уха",
        "филин",
        "фрак",
        "хирургия",
        "храбрость",
        "черепаха",
        "чета",
        "щелчок",
        "артель",
        "астроном",
        "баллон",
        "благоустройство",
        "блядь",
        "внутренность",
        "водопровод",
        "восход",
        "выдвижение",
        "гаишник",
        "гетто",
        "гравюра",
        "директива",
        "докладчик",
        "древостой",
        "духовка",
        "жеребец",
        "канцлер",
        "караван",
        "клён",
        "княжна",
        "конструирование",
        "концентрат",
        "кормление",
        "метрополитен",
        "молот",
        "неравенство",
        "новшество",
        "озноб",
        "плотина",
        "пляска",
        "пожарный",
        "получатель",
        "понижение",
        "поощрение",
        "припадок",
        "протез",
        "процессор",
        "работяга",
        "разлом",
        "расторжение",
        "серб",
        "синод",
        "спутница",
        "сценарист",
        "теснота",
        "топик",
        "топот",
        "тошнота",
        "углерод",
        "удел",
        "устремление",
        "фронтовик",
        "фургон",
        "бренд",
        "вибрация",
        "генотип",
        "гладь",
        "горничная",
        "графа",
        "греза",
        "гроссмейстер",
        "дивиденд",
        "достопримечательность",
        "дурочка",
        "железнодорожник",
        "загар",
        "зола",
        "извещение",
        "издевательство",
        "изыскание",
        "иллюминатор",
        "кабак",
        "кедр",
        "крапива",
        "лада",
        "льдина",
        "мечеть",
        "микрорайон",
        "монархия",
        "наводнение",
        "натиск",
        "небоскрёб",
        "незнание",
        "осадок",
        "подтекст",
        "правопорядок",
        "присяжный",
        "радиатор",
        "распорядок",
        "расставание",
        "румянец",
        "сельсовет",
        "терминал",
        "толстяк",
        "транзит",
        "фартук",
        "физиология",
        "фотка",
        "фриц",
        "хищение",
        "хруст",
        "цветник",
        "чадо",
        "чехол",
        "алименты",
        "ас",
        "байка",
        "беспомощность",
        "валун",
        "взмах",
        "возведение",
        "вулкан",
        "выпад",
        "гармошка",
        "генерал-полковник",
        "графика",
        "журналистка",
        "иудей",
        "кинематографист",
        "козырь",
        "крот",
        "крылышко",
        "лепёшка",
        "леший",
        "линза",
        "магнит",
        "маэстро",
        "мерзость",
        "микроскоп",
        "нажим",
        "наполнение",
        "настойчивость",
        "насыпь",
        "неизвестность",
        "нелепость",
        "неудачник",
        "ничья",
        "орнамент",
        "отображение",
        "оттепель",
        "пастор",
        "пересадка",
        "планировка",
        "погрузка",
        "порядочность",
        "постановщик",
        "приличие",
        "продуктивность",
        "пуд",
        "пылесос",
        "ревизор",
        "резидент",
        "ренессанс",
        "россыпь",
        "ручеёк",
        "самогон",
        "сновидение",
        "срез",
        "стерва",
        "стимулирование",
        "странник",
        "трусость",
        "туннель",
        "уксус",
        "урожайность",
        "фарш",
        "филолог",
        "флажок",
        "хамство",
        "шампунь",
        "шелест",
        "азербайджанец",
        "аристократ",
        "артиллерист",
        "белизна",
        "белый",
        "блузка",
        "бодрость",
        "брань",
        "брэнд",
        "бычок",
        "верёвочка",
        "вожак",
        "вонь",
        "гуманизм",
        "гуща",
        "духота",
        "единение",
        "избежание",
        "изгородь",
        "имитация",
        "карниз",
        "кефир",
        "кивок",
        "клык",
        "коллекционер",
        "комплектация",
        "конвоир",
        "контролёр",
        "концлагерь",
        "коран",
        "косарь",
        "крестик",
        "ладоши",
        "милорд",
        "мойка",
        "наезд",
        "наклон",
        "наркомания",
        "неприкосновенность",
        "неуверенность",
        "носик",
        "обустройство",
        "осадки",
        "пелена",
        "переселение",
        "подставка",
        "полочка",
        "порча",
        "пригород",
        "путаница",
        "работница",
        "разворот",
        "раса",
        "рыдание",
        "свинина",
        "стечение",
        "субстанция",
        "тракторист",
        "уезд",
        "управа",
        "формальность",
        "хвостик",
        "хорёк",
        "цинизм",
        "цистерна",
        "штрих",
        "эгоизм",
        "яркость",
        "абориген",
        "алкоголизм",
        "алфавит",
        "арестант",
        "арестованный",
        "атаман",
        "атлас",
        "бутик",
        "вакансия",
        "виток",
        "воспроизведение",
        "деградация",
        "детище",
        "диспетчер",
        "дурь",
        "застава",
        "канализация",
        "кобыла",
        "коллективизация",
        "конфискация",
        "лавра",
        "мятеж",
        "охапка",
        "парламентарий",
        "пароль",
        "передняя",
        "переправа",
        "периметр",
        "писательница",
        "питомник",
        "премудрость",
        "пренебрежение",
        "присмотр",
        "проходная",
        "ругань",
        "священнослужитель",
        "сигнализация",
        "сквозняк",
        "сковорода",
        "смешок",
        "согражданин",
        "сорока",
        "сосок",
        "стукач",
        "сцепление",
        "трата",
        "ушко",
        "фанера",
        "флейта",
        "шлюпка",
        "явка",
        "ямка",
        "архипелаг",
        "борозда",
        "булат",
        "буржуазия",
        "вымысел",
        "генсек",
        "гонение",
        "дождик",
        "доступность",
        "заграница",
        "зампред",
        "запорожец",
        "зародыш",
        "иммунизация",
        "кладка",
        "колючка",
        "корочка",
        "куб",
        "кушетка",
        "либерализация",
        "монгол",
        "мягкость",
        "неудовольствие",
        "нотка",
        "ограждение",
        "опубликование",
        "отблеск",
        "перелёт",
        "подопечный",
        "поэтесса",
        "приверженец",
        "прислуга",
        "программирование",
        "проработка",
        "простуда",
        "противопоставление",
        "прямоугольник",
        "публицист",
        "самосвал",
        "серьга",
        "словосочетание",
        "созыв",
        "таможенник",
        "телекомпания",
        "трюм",
        "уныние",
        "усердие",
        "устой",
        "усыновление",
        "фрау",
        "хан",
        "хватка",
        "хижина",
        "хит",
        "цыганка",
        "членство",
        "шеренга",
        "экскаватор",
        "эпопея",
        "абстракция",
        "археолог",
        "булыжник",
        "бурение",
        "бутыль",
        "вентилятор",
        "воротничок",
        "ворох",
        "вставка",
        "вычет",
        "глас",
        "гостеприимство",
        "диктор",
        "дозор",
        "доработка",
        "ипподром",
        "картон",
        "коррекция",
        "крестьянство",
        "лидерство",
        "ликование",
        "луковица",
        "мозаика",
        "муравейник",
        "мякиш",
        "нападающий",
        "неточность",
        "облачко",
        "оглядка",
        "ознакомление",
        "оцепенение",
        "очарование",
        "паразит",
        "пиар",
        "повозка",
        "преемственность",
        "принуждение",
        "присутствующий",
        "прихоть",
        "причинение",
        "прямая",
        "раскаяние",
        "расстановка",
        "рыжик",
        "собеседование",
        "стайка",
        "суспензия",
        "трусики",
        "убор",
        "угощение",
        "умысел",
        "фишка",
        "художница",
        "швейцар",
        "шейка",
        "шпага",
        "экспортёр",
        "эпитет",
        "авантюра",
        "аксессуар",
        "апартаменты",
        "арбитраж",
        "архимандрит",
        "барахло",
        "баритон",
        "взятие",
        "водоснабжение",
        "галлюцинация",
        "галочка",
        "гипс",
        "глобус",
        "гранит",
        "дефолт",
        "домработница",
        "заболеваемость",
        "замешательство",
        "замужество",
        "зеркальце",
        "зигзаг",
        "ипотека",
        "карусель",
        "катаклизм",
        "квартет",
        "клеточка",
        "кудри",
        "лимузин",
        "лоно",
        "лопух",
        "люд",
        "мазь",
        "макияж",
        "модерн",
        "морщинка",
        "награждение",
        "наёмник",
        "обмундирование",
        "отчим",
        "партком",
        "пластика",
        "подвижник",
        "подлинность",
        "покойный",
        "прадед",
        "преобладание",
        "призывник",
        "приостановление",
        "программист",
        "пролёт",
        "разность",
        "разок",
        "роды",
        "рысь",
        "симпозиум",
        "синагога",
        "слива",
        "смертный",
        "сорочка",
        "трофей",
        "угодье",
        "уголёк",
        "фанатик",
        "фейерверк",
        "часовня",
        "шорты",
        "штатское",
        "штурмовик",
        "щепка",
        "щетина",
        "экосистема",
        "эпиграф",
        "яичко",
        "автотранспорт",
        "агроном",
        "арбитр",
        "бархат",
        "блик",
        "болван",
        "водохранилище",
        "возбудитель",
        "выходка",
        "горница",
        "грамматика",
        "дерзость",
        "дошкольник",
        "заправка",
        "знамение",
        "избавление",
        "интернат",
        "каскад",
        "катюша",
        "кинорежиссёр",
        "кореец",
        "корзинка",
        "кубометр",
        "купание",
        "курточка",
        "лексика",
        "ловля",
        "мадонна",
        "миномёт",
        "минуточка",
        "мытье",
        "наём",
        "националист",
        "непоседа",
        "ник",
        "окошечко",
        "определённость",
        "отдыхающий",
        "переселенец",
        "подсказка",
        "поклонение",
        "покрышка",
        "полено",
        "полог",
        "полянка",
        "помин",
        "пудра",
        "раздевалка",
        "реанимация",
        "резьба",
        "ресторанчик",
        "садовник",
        "смешение",
        "смута",
        "соперничество",
        "телеграф",
        "ударение",
        "упрямство",
        "фея",
        "фреска",
        "царапина",
        "черновик",
        "шаблон",
        "акциз",
        "антракт",
        "архиерей",
        "афоризм",
        "батон",
        "блюдце",
        "боксёр",
        "веха",
        "ворон",
        "вошь",
        "врата",
        "вязкость",
        "габарит",
        "галка",
        "гарь",
        "героизм",
        "горение",
        "грива",
        "дамочка",
        "денёк",
        "диалектика",
        "дубль",
        "запой",
        "зверёк",
        "изящество",
        "кабан",
        "кальций",
        "камыш",
        "капиталист",
        "карцер",
        "клеёнка",
        "кобель",
        "комендатура",
        "компромат",
        "кочка",
        "лакей",
        "магазинчик",
        "мисс",
        "набросок",
        "наган",
        "начинка",
        "немка",
        "объектив",
        "овация",
        "овёс",
        "отдаление",
        "отличник",
        "переводчица",
        "планшет",
        "повадка",
        "подножка",
        "посвящение",
        "поэтика",
        "пояснение",
        "принтер",
        "проигрыш",
        "противовес",
        "пузо",
        "пурга",
        "раввин",
        "радикал",
        "радист",
        "раскладушка",
        "ревизия",
        "редколлегия",
        "розетка",
        "самоопределение",
        "спекуляция",
        "тряпочка",
        "узы",
        "уйма",
        "усовершенствование",
        "флора",
        "цент",
        "чугун",
        "шалаш",
        "шаман",
        "шатёр",
        "ширма",
        "шкатулка",
        "шкафчик",
        "шпана",
        "шуточка",
        "щука",
        "экзотика",
        "экспериментатор",
        "экстремизм",
        "альфа",
        "ассамблея",
        "аттестат",
        "бант",
        "беспредел",
        "боцман",
        "бункер",
        "вещание",
        "вздор",
        "властитель",
        "влюблённость",
        "воск",
        "гайка",
        "героин",
        "гибрид",
        "гильза",
        "горячка",
        "гряда",
        "дипломатия",
        "допуск",
        "живучесть",
        "журавль",
        "звездолёт",
        "ива",
        "инъекция",
        "исцеление",
        "комментатор",
        "комод",
        "кроватка",
        "кулачок",
        "манифест",
        "метан",
        "милая",
        "мифология",
        "моль",
        "недостаточность",
        "ночлег",
        "ноша",
        "оргкомитет",
        "осина",
        "относительность",
        "отпор",
        "отставание",
        "пал",
        "панихида",
        "партнёрша",
        "перевозчик",
        "переносица",
        "позвонок",
        "полслова",
        "полутьма",
        "помысел",
        "попечение",
        "пощёчина",
        "притяжение",
        "просчёт",
        "пятнышко",
        "рацион",
        "резон",
        "рождаемость",
        "романтизм",
        "свинка",
        "сдержанность",
        "секретность",
        "скат",
        "смета",
        "социум",
        "старение",
        "стишок",
        "сток",
        "сценка",
        "счеты",
        "тенор",
        "тина",
        "топка",
        "тренажёр",
        "убеждённость",
        "фактура",
        "хам",
        "холодок",
        "хромосома",
        "шайка",
        "шаль",
        "аккумулятор",
        "аура",
        "баллада",
        "баррикада",
        "бездельник",
        "блиндаж",
        "болт",
        "братва",
        "вдох",
        "великолепие",
        "выздоровление",
        "выключатель",
        "вырубка",
        "гастроном",
        "говядина",
        "гордыня",
        "гусар",
        "девчата",
        "диаспора",
        "диванчик",
        "дискриминация",
        "досье",
        "дружинник",
        "дубинка",
        "елочка",
        "жемчуг",
        "жуть",
        "запрещение",
        "зонт",
        "карнавал",
        "качели",
        "колготки",
        "крейсер",
        "курган",
        "курок",
        "лотерея",
        "лояльность",
        "маятник",
        "мобильник",
        "ничтожество",
        "ограниченность",
        "перебор",
        "перехват",
        "подвижность",
        "пола",
        "порождение",
        "приверженность",
        "пригорок",
        "притязание",
        "пробег",
        "проповедник",
        "рожок",
        "своеобразие",
        "семантика",
        "сложение",
        "фильтрация",
        "фокусник",
        "хоровод",
        "чан",
        "штукатурка",
        "экстракт",
        "эмблема",
        "альт",
        "бюрократ",
        "варвар",
        "ветчина",
        "вич-инфекция",
        "возвышение",
        "выдох",
        "высадка",
        "галька",
        "географ",
        "гном",
        "дичь",
        "древо",
        "еврейка",
        "единоборство",
        "жандарм",
        "жилет",
        "жилплощадь",
        "зной",
        "изменчивость",
        "капюшон",
        "квас",
        "кисель",
        "кисточка",
        "коллизия",
        "кораблик",
        "кустик",
        "лавина",
        "лифчик",
        "лицей",
        "лучшее",
        "масть",
        "медикамент",
        "местонахождение",
        "местоположение",
        "мечтание",
        "мизинец",
        "морковка",
        "мошенник",
        "мякоть",
        "народность",
        "невыполнение",
        "опровержение",
        "отключение",
        "охват",
        "палестинец",
        "переделка",
        "пика",
        "питье",
        "погибель",
        "подрыв",
        "покорность",
        "праймер",
        "преображение",
        "ракурс",
        "рефлексия",
        "рябина",
        "свисток",
        "сердцевина",
        "сжатие",
        "скрежет",
        "смородина",
        "творог",
        "терпимость",
        "топчан",
        "трактир",
        "тренинг",
        "трость",
        "тряпьё",
        "тыква",
        "тюльпан",
        "универмаг",
        "упоение",
        "фанат",
        "частушка",
        "чрево",
        "щедрость",
        "эссе",
        "абажур",
        "авоська",
        "автоматика",
        "амбар",
        "анатомия",
        "античность",
        "баронесса",
        "болгарин",
        "булавка",
        "бушлат",
        "вереница",
        "влагалище",
        "военачальник",
        "воззрение",
        "вотум",
        "галоша",
        "гарант",
        "голландец",
        "дачник",
        "девятка",
        "деревце",
        "детка",
        "доблесть",
        "дощечка",
        "закладка",
        "зарядка",
        "изголовье",
        "йод",
        "конденсат",
        "контрактник",
        "косынка",
        "лава",
        "лавр",
        "лайнер",
        "ленточка",
        "ломтик",
        "мгновенье",
        "мушка",
        "натрий",
        "наушник",
        "нефтепровод",
        "одночасье",
        "ореол",
        "перекрытие",
        "перепад",
        "пехотинец",
        "поверка",
        "подлинник",
        "подушечка",
        "полицай",
        "полсотни",
        "поместье",
        "портянка",
        "почтальон",
        "правозащитник",
        "процессия",
        "раскопка",
        "рвение",
        "родинка",
        "рукопожатие",
        "самооценка",
        "санки",
        "свиданье",
        "сестренка",
        "снежок",
        "сочинитель",
        "сыночек",
        "титан",
        "трудоустройство",
        "тушь",
        "тягота",
        "укор",
        "упрощение",
        "усвоение",
        "фауна",
        "художество",
        "часик",
        "шпионаж",
        "экстаз",
        "эмбрион",
        "ягодица",
        "аз",
        "алкаш",
        "баскетбол",
        "берет",
        "бильярд",
        "булка",
        "валентность",
        "варяг",
        "влюбленный",
        "вратарь",
        "гвоздика",
        "гипноз",
        "главврач",
        "дворянство",
        "декрет",
        "дотация",
        "дрожжи",
        "дуло",
        "естество",
        "жалованье",
        "завеса",
        "заговорщик",
        "задумчивость",
        "иероглиф",
        "империализм",
        "искусствовед",
        "йога",
        "калашников",
        "каторга",
        "клавиатура",
        "ковш",
        "компас",
        "комплектующие",
        "консерватор",
        "консул",
        "конус",
        "кузнечик",
        "куколка",
        "лазер",
        "лапша",
        "ледник",
        "медуза",
        "мизансцена",
        "монетка",
        "наваждение",
        "надбавка",
        "опер",
        "опилки",
        "орда",
        "остроумие",
        "пересылка",
        "персик",
        "пилотка",
        "плед",
        "погрешность",
        "подвода",
        "подворье",
        "прибой",
        "прозрение",
        "производная",
        "простой",
        "разгадка",
        "резервуар",
        "рецептор",
        "рис",
        "сапер",
        "сапожник",
        "спец",
        "сплетение",
        "странствие",
        "танго",
        "танцор",
        "турбина",
        "хобби",
        "хоккеист",
        "хрусталь",
        "хулиганство",
        "шайба",
        "энцефалит",
        "эстафета",
        "альманах",
        "аминокислота",
        "антибиотик",
        "антисемит",
        "баррель",
        "баян",
        "булочка",
        "вентиляция",
        "вырезка",
        "дисплей",
        "изречение",
        "именинник",
        "кабинка",
        "калека",
        "капитализация",
        "келья",
        "клон",
        "колёсико",
        "коробок",
        "краситель",
        "лесенка",
        "ловкость",
        "маркиз",
        "мегаполис",
        "микроб",
        "монумент",
        "мошенничество",
        "наркоз",
        "неделька",
        "обидчик",
        "обретение",
        "оконце",
        "пай",
        "пелёнка",
        "переключение",
        "перестановка",
        "пижама",
        "подтип",
        "пометка",
        "праведник",
        "признательность",
        "путник",
        "разоружение",
        "разрядка",
        "риторика",
        "рожь",
        "рюмочка",
        "скорпион",
        "созревание",
        "страхователь",
        "сушка",
        "тиран",
        "тоталитаризм",
        "трансмиссия",
        "тыща",
        "убедительность",
        "уроженец",
        "фараон",
        "фарфор",
        "цитокин",
        "чаща",
        "шкурка",
        "энтропия",
        "яичница",
        "авторство",
        "амплитуда",
        "атеист",
        "батя",
        "благоговение",
        "блондин",
        "бутон",
        "взаимность",
        "вовлечение",
        "восклицание",
        "воспитательница",
        "выкрик",
        "гигиена",
        "гид",
        "гравий",
        "гран-при",
        "детдом",
        "догма",
        "домовой",
        "естествознание",
        "зав",
        "запор",
        "землянин",
        "инновация",
        "кактус",
        "капсула",
        "каштан",
        "киностудия",
        "кипение",
        "коллектор",
        "компрессор",
        "кругозор",
        "кукушка",
        "куплет",
        "латынь",
        "листик",
        "лох",
        "матрац",
        "неисполнение",
        "неразбериха",
        "несогласие",
        "нестабильность",
        "неумение",
        "обвал",
        "обоз",
        "овладение",
        "окрик",
        "пивная",
        "плацдарм",
        "пожертвование",
        "полезность",
        "полмиллиона",
        "постоянство",
        "предплечье",
        "прибавка",
        "пристав-исполнитель",
        "пролетарий",
        "прошение",
        "птенец",
        "разврат",
        "распятие",
        "расходование",
        "расшифровка",
        "регуляция",
        "робость",
        "росток",
        "рыло",
        "самосохранение",
        "сборище",
        "северо-запад",
        "семерка",
        "скачка",
        "смотр",
        "снос",
        "составитель",
        "сотка",
        "соцветие",
        "сударь",
        "телезритель",
        "телекамера",
        "телеэкран",
        "уклонение",
        "укроп",
        "участница",
        "фальшь",
        "флюид",
        "шест",
        "шизофрения",
        "шлюха",
        "автоматчик",
        "арматура",
        "аудитор",
        "бабуля",
        "безделье",
        "буксир",
        "венчание",
        "возрастание",
        "главк",
        "глубинка",
        "гольф",
        "госбезопасность",
        "деревушка",
        "дискурс",
        "днище",
        "доказывание",
        "дублёнка",
        "ересь",
        "железка",
        "зачатие",
        "иней",
        "ипостась",
        "искание",
        "каратэ",
        "клубника",
        "клякса",
        "колыбель",
        "комплектование",
        "константа",
        "литературовед",
        "марево",
        "материализм",
        "монахиня",
        "нерешительность",
        "обойма",
        "ограбление",
        "одуванчик",
        "оптимист",
        "пах",
        "пептид",
        "подкладка",
        "понятой",
        "портной",
        "привкус",
        "радиация",
        "разгрузка",
        "разлив",
        "разруха",
        "разъезд",
        "распознавание",
        "регистр",
        "свершение",
        "совмещение",
        "спекулянт",
        "старое",
        "старпом",
        "стрелец",
        "тара",
        "томик",
        "тумба",
        "ударник",
        "улов",
        "умник",
        "успокоение",
        "утилизация",
        "фельетон",
        "фиксация",
        "фужер",
        "хлопец",
        "штурвал",
        "электрик",
        "эсер",
        "американка",
        "бардак",
        "бега",
        "безумец",
        "бразилец",
        "браток",
        "брюнетка",
        "везение",
        "впадина",
        "гиря",
        "главарь",
        "дебаты",
        "дежурная",
        "дилетант",
        "еженедельник",
        "ер",
        "ефрейтор",
        "жаба",
        "заимствование",
        "иврит",
        "изъян",
        "инсульт",
        "истинность",
        "истощение",
        "казах",
        "керамика",
        "клеймо",
        "клинок",
        "колорит",
        "коммерция",
        "кремний",
        "лишнее",
        "любезность",
        "любимая",
        "лютик",
        "мания",
        "муть",
        "нацист",
        "невзгода",
        "нейрон",
        "неподвижность",
        "неполноценность",
        "новостройка",
        "овал",
        "окружающие",
        "оптика",
        "основоположник",
        "отрок",
        "паралич",
        "парковка",
        "паром",
        "перевес",
        "перевооружение",
        "перекладина",
        "пермяк",
        "пинок",
        "почитатель",
        "поясница",
        "председательствующий",
        "провокатор",
        "проекция",
        "проституция",
        "равноправие",
        "ракушка",
        "резкость",
        "рекламодатель",
        "родословная",
        "серп",
        "си",
        "сиятельство",
        "скороговорка",
        "слежка",
        "сорняк",
        "стюардесса",
        "тональность",
        "тушёнка",
        "тяготение",
        "удвоение",
        "уникальность",
        "феня",
        "фигня",
        "футболка",
        "халатик",
        "хмель",
        "царевич",
        "царствование",
        "ценитель",
        "центрист",
        "циркуляция",
        "человечность",
        "червяк",
        "чужак",
        "шик",
        "шпиль",
        "щиколотка",
        "эйфория",
        "эквивалент",
        "эмаль",
        "эрудиция",
        "эскалатор",
        "автозавод",
        "автосалон",
        "альпинист",
        "баранина",
        "бедолага",
        "блоха",
        "богоматерь",
        "бусы",
        "взыскатель",
        "видеокамера",
        "водичка",
        "вольность",
        "вырез",
        "гавань",
        "глушь",
        "голень",
        "головокружение",
        "горб",
        "горец",
        "громада",
        "динамик",
        "домашние",
        "дяденька",
        "житье",
        "зала",
        "зарождение",
        "застройщик",
        "изнасилование",
        "иконостас",
        "инвентарь",
        "калий",
        "камуфляж",
        "коварство",
        "ковбой",
        "консорциум",
        "контрабанда",
        "контрабас",
        "краса",
        "ладья",
        "ленинградец",
        "лимфоцит",
        "мамка",
        "маскировка",
        "молодожен",
        "москвичка",
        "навигация",
        "накладка",
        "наместник",
        "нападки",
        "напасть",
        "наркомат",
        "наследственность",
        "начальница",
        "немощь",
        "новорождённый",
        "ножик",
        "норка",
        "озарение",
        "оксид",
        "отсвет",
        "пластмасса",
        "погребение",
        "подшипник",
        "покровительство",
        "посланец",
        "посягательство",
        "приманка",
        "прослойка",
        "прослушивание",
        "раздача",
        "ремешок",
        "ругательство",
        "рулон",
        "сажень",
        "самоубийца",
        "сан",
        "свечение",
        "скамеечка",
        "слепота",
        "словцо",
        "сперма",
        "сталинизм",
        "стационар",
        "стражник",
        "сучок",
        "тонус",
        "удержание",
        "укладка",
        "уступ",
        "ухмылка",
        "финансист",
        "фольга",
        "частник",
        "шантаж",
        "штаб-квартира",
        "эгида",
        "юнец",
        "агрессивность",
        "айсберг",
        "акватория",
        "аккордеон",
        "батька",
        "брезгливость",
        "буханка",
        "варежка",
        "веретено",
        "водочка",
        "вожжа",
        "всевышний",
        "геология",
        "горелка",
        "диагональ",
        "диво",
        "долька",
        "доченька",
        "животноводство",
        "заверение",
        "зажигание",
        "закоулок",
        "излишек",
        "икс",
        "инвалидность",
        "информатика",
        "кабачок",
        "коротышка",
        "котировка",
        "кучер",
        "легкомыслие",
        "локатор",
        "материнство",
        "нагрев",
        "накал",
        "неосторожность",
        "новация",
        "обрез",
        "общага",
        "одеяние",
        "окружность",
        "оправа",
        "опус",
        "отцовство",
        "очевидность",
        "пайка",
        "пансионат",
        "пари",
        "партер",
        "партитура",
        "паства",
        "петушок",
        "писк",
        "плац",
        "плуг",
        "подвох",
        "подоплёка",
        "потоп",
        "предосторожность",
        "призёр",
        "прицеп",
        "прораб",
        "прошествие",
        "путч",
        "растворитель",
        "репродуктор",
        "ржавчина",
        "рисование",
        "русалка",
        "самодержавие",
        "самоорганизация",
        "связист",
        "сгусток",
        "совместительство",
        "сосуществование",
        "стог",
        "счастливчик",
        "тепловоз",
        "торец",
        "травинка",
        "трек",
        "туфелька",
        "углеводород",
        "узбек",
        "укус",
        "уловка",
        "устроитель",
        "фортепиано",
        "харя",
        "холмик",
        "чёткость",
        "член-корреспондент",
        "шиворот",
        "штабель",
        "штанга",
        "шубка",
        "энергоноситель",
        "яблочко",
        "ярлык",
        "авантюрист",
        "акация",
        "ателье",
        "баранка",
        "бугорок",
        "буран",
        "вахтёр",
        "вердикт",
        "вождение",
        "возлюбленный",
        "волхв",
        "газик",
        "гвардеец",
        "голенище",
        "госслужба",
        "гранатомёт",
        "деликатес",
        "деликатность",
        "диктофон",
        "довольствие",
        "дыня",
        "жаргон",
        "задаток",
        "запятая",
        "изнеможение",
        "инфицирование",
        "иудаизм",
        "казачество",
        "каморка",
        "кардинал",
        "кейс",
        "кинофестиваль",
        "клерк",
        "конфессия",
        "копа",
        "корреспонденция",
        "кроха",
        "крылечко",
        "лесничество",
        "лиственница",
        "ломоть",
        "лоскут",
        "мантия",
        "мещанин",
        "мясорубка",
        "наличные",
        "незнакомка",
        "нетерпимость",
        "ношение",
        "ода",
        "оригинальность",
        "ослик",
        "отвар",
        "отлёт",
        "оценивание",
        "палисадник",
        "паломник",
        "пашня",
        "пенёк",
        "перипетия",
        "пикник",
        "плесень",
        "подиум",
        "подмена",
        "поклонница",
        "портал",
        "посланник",
        "приёмка",
        "пристанище",
        "психушка",
        "пупок",
        "решительность",
        "самозванец",
        "сварка",
        "селекция",
        "скептик",
        "снежинка",
        "стрекоза",
        "тата",
        "технолог",
        "умелец",
        "умножение",
        "флигель",
        "цинк",
        "чаёк",
        "чешуя",
        "школьница",
        "экстремист",
        "этикет",
        "абитуриент",
        "автобиография",
        "ажиотаж",
        "акварель",
        "аттестация",
        "базирование",
        "баклажан",
        "биосфера",
        "благодетель",
        "буфетчица",
        "вампир",
        "вероисповедание",
        "ветеринар",
        "веяние",
        "волшебство",
        "вынос",
        "глухарь",
        "декан",
        "десерт",
        "диверсант",
        "дознание",
        "дубина",
        "естественность",
        "жребий",
        "завхоз",
        "зажим",
        "зазор",
        "засов",
        "затворник",
        "земледелие",
        "игумен",
        "избиение",
        "инвентаризация",
        "инопланетянин",
        "интервьюер",
        "кило",
        "кладовка",
        "клип",
        "коми",
        "компаньон",
        "комсомолка",
        "конспект",
        "корь",
        "котлован",
        "купальник",
        "леска",
        "ляжка",
        "мазут",
        "маскарад",
        "масон",
        "машинистка",
        "мимика",
        "молва",
        "мумия",
        "мушкетёр",
        "насморк",
        "нэп",
        "обвинитель",
        "обруч",
        "огурчик",
        "озон",
        "оккупант",
        "омар",
        "орешек",
        "отлив",
        "пакт",
        "панк",
        "пенсне",
        "плеть",
        "подозрительность",
        "подстилка",
        "помост",
        "посох",
        "постижение",
        "превосходительство",
        "приписка",
        "пряжка",
        "прямота",
        "разгул",
        "райцентр",
        "рассол",
        "расценка",
        "реал",
        "реприза",
        "рознь",
        "самооборона",
        "северо-восток",
        "сера",
        "склеп",
        "сношение",
        "сотрясение",
        "сподвижник",
        "спортзал",
        "стилистика",
        "тёзка",
        "телепередача",
        "томат",
        "травля",
        "тропка",
        "тулуп",
        "ультиматум",
        "управляемость",
        "упряжка",
        "француженка",
        "хлопья",
        "хозяйствование",
        "холостяк",
        "царствие",
        "цельность",
        "чаепитие",
        "черенок",
        "шевелюра",
        "шелуха",
        "шиповник",
        "шпала",
        "электрод",
        "язычник",
        "алгебра",
        "амин",
        "амур",
        "барс",
        "баталия",
        "безнадёжность",
        "благотворительность",
        "большевизм",
        "бомбардировка",
        "буддизм",
        "булочная",
        "буфер",
        "ведомо",
        "вездеход",
        "вентиль",
        "веснушка",
        "вогул",
        "выведение",
        "выравнивание",
        "вышивка",
        "гениальность",
        "гирлянда",
        "грузоподъёмность",
        "декор",
        "допущение",
        "дюйм",
        "завсегдатай",
        "завязка",
        "замещение",
        "замполит",
        "затишье",
        "идиотизм",
        "иммигрант",
        "индексация",
        "испарение",
        "истребление",
        "кавычка",
        "капитуляция",
        "кокаин",
        "колос",
        "комочек",
        "консерватизм",
        "конфронтация",
        "косичка",
        "кровотечение",
        "кульминация",
        "легионер",
        "лектор",
        "лицемерие",
        "локализация",
        "людоед",
        "магнат",
        "настил",
        "непогода",
        "непосредственность",
        "обладание",
        "обнимка",
        "одарённость",
        "отрава",
        "па",
        "падла",
        "перебой",
        "перестрелка",
        "пингвин",
        "пирс",
        "питомец",
        "поднятие",
        "подсветка",
        "подступ",
        "полив",
        "политрук",
        "поломка",
        "полпути",
        "помощница",
        "почести",
        "пощада",
        "прииск",
        "проектировщик",
        "происходящее",
        "прообраз",
        "пятак",
        "радиоприёмник",
        "рвота",
        "реликвия",
        "рыболов",
        "санитарка",
        "сброс",
        "сифон",
        "слобода",
        "соболезнование",
        "созидание",
        "сотворение",
        "социалист",
        "спазм",
        "стандартизация",
        "табор",
        "телец",
        "тест-система",
        "тревожность",
        "туз",
        "тюбик",
        "умывальник",
        "утварь",
        "фальсификация",
        "фантом",
        "формация",
        "хвала",
        "червонец",
        "честолюбие",
        "шитье",
        "экспрессия",
        "автомобилист",
        "авторучка",
        "аккредитив",
        "астра",
        "банальность",
        "батарейка",
        "бачок",
        "башенка",
        "беззаконие",
        "безысходность",
        "белорус",
        "варка",
        "виртуоз",
        "герпес",
        "горошек",
        "гулянье",
        "дезертир",
        "дельта",
        "дневальный",
        "домохозяйка",
        "домохозяйство",
        "духовник",
        "забытье",
        "зайка",
        "заморозок",
        "запоминание",
        "засуха",
        "зелье",
        "земляника",
        "зонд",
        "иго",
        "идиотка",
        "извращение",
        "историография",
        "кассир",
        "кассирша",
        "клоунада",
        "кока-кола",
        "консульство",
        "кров",
        "куратор",
        "лета",
        "лингвистика",
        "лохмотья",
        "лямка",
        "макака",
        "метаморфоза",
        "микроорганизм",
        "молебен",
        "монашество",
        "мор",
        "наклонность",
        "налоговик",
        "наработка",
        "нейтрон",
        "несоблюдение",
        "нормализация",
        "обеспеченность",
        "обет",
        "омут",
        "осанка",
        "отпрыск",
        "отречение",
        "отток",
        "панцирь",
        "пастбище",
        "письменность",
        "портсигар",
        "портьера",
        "постоялец",
        "похождение",
        "предвкушение",
        "предтеча",
        "преисподняя",
        "прения",
        "приобщение",
        "промедление",
        "протон",
        "пучина",
        "пыльца",
        "различение",
        "размерность",
        "реагирование",
        "реванш",
        "регулировка",
        "резюме",
        "реле",
        "референт",
        "сарафан",
        "сев",
        "сервиз",
        "скоба",
        "соломинка",
        "соната",
        "сор",
        "стажировка",
        "старожил",
        "стружка",
        "стычка",
        "суеверие",
        "телескоп",
        "туземец",
        "упорядочение",
        "фарс",
        "фляжка",
        "фонограмма",
        "холера",
        "циферблат",
        "чемпионка",
        "шоу-бизнес",
        "эскадрилья",
        "эскадрон",
        "явь",
        "ясли",
        "автодром",
        "азбука",
        "аккомпанемент",
        "активация",
        "амплуа",
        "ампула",
        "амулет",
        "аргументация",
        "ареал",
        "аудит",
        "базис",
        "бампер",
        "банкрот",
        "банька",
        "бастион",
        "бедняк",
        "безмолвие",
        "богема",
        "вагончик",
        "видеокассета",
        "винтик",
        "вице-губернатор",
        "внушение",
        "волейбол",
        "гинеколог",
        "грабли",
        "гроздь",
        "дамба",
        "декада",
        "делец",
        "депозит",
        "диабет",
        "дислокация",
        "досмотр",
        "доспехи",
        "егерь",
        "жвачка",
        "жучок",
        "завуч",
        "зубец",
        "идеалист",
        "инструментарий",
        "интернационал",
        "интурист",
        "капкан",
        "клонирование",
        "колодка",
        "конспирация",
        "кордон",
        "кортеж",
        "корысть",
        "костюмчик",
        "креветка",
        "кретин",
        "кружева",
        "легализация",
        "лейкоцит",
        "люкс",
        "матерь",
        "мегафон",
        "мемориал",
        "мётка",
        "мирянин",
        "мичман",
        "мультфильм",
        "набег",
        "нарастание",
        "настой",
        "неведение",
        "невиновность",
        "нелюбовь",
        "ножны",
        "обличье",
        "опись",
        "отголосок",
        "отмывание",
        "пакость",
        "папоротник",
        "пароходство",
        "пациентка",
        "платьице",
        "повеление",
        "повтор",
        "подкова",
        "подложка",
        "подлокотник",
        "подряд",
        "получас",
        "потёмки",
        "преосвященный",
        "преследователь",
        "призма",
        "пролог",
        "пропажа",
        "проступок",
        "противогаз",
        "пюре",
        "разумение",
        "расизм",
        "расплата",
        "ремесленник",
        "ртуть",
        "рулетка",
        "рыбина",
        "северянин",
        "сифилис",
        "скорлупа",
        "соперница",
        "спецназовец",
        "списание",
        "старшеклассник",
        "статуэтка",
        "стенограмма",
        "субтип",
        "суматоха",
        "тельце",
        "термос",
        "удав",
        "уродство",
        "фиалка",
        "храп",
        "чувак",
        "штанина",
        "язычок",
        "ананас",
        "аршин",
        "барыня",
        "басня",
        "беспамятство",
        "блат",
        "братишка",
        "бруствер",
        "бублик",
        "вещица",
        "визитка",
        "водоворот",
        "выкладка",
        "гнёт",
        "говор",
        "годность",
        "грудка",
        "данность",
        "дебри",
        "дёготь",
        "держатель",
        "десна",
        "дешёвка",
        "джин",
        "диспут",
        "догмат",
        "дознаватель",
        "дружище",
        "жгут",
        "жердь",
        "жратва",
        "заглавие",
        "закуток",
        "зарево",
        "здравие",
        "имя-отчество",
        "ингредиент",
        "индивидуум",
        "индустриализация",
        "истукан",
        "канадец",
        "кинематография",
        "киса",
        "ключица",
        "княжество",
        "комета",
        "комнатушка",
        "корифей",
        "кормушка",
        "коррозия",
        "космодром",
        "кочегар",
        "кружево",
        "лексикон",
        "лесник",
        "лингвист",
        "логотип",
        "лоджия",
        "лотос",
        "магистр",
        "манекенщица",
        "маркер",
        "механизация",
        "млекопитающее",
        "могилка",
        "мясокомбинат",
        "неверие",
        "невинность",
        "неволя",
        "недруг",
        "неоднородность",
        "нечисть",
        "осада",
        "отвал",
        "отстранение",
        "параграф",
        "переоценка",
        "перст",
        "плетень",
        "повседневность",
        "подгруппа",
        "подмога",
        "подсудность",
        "полынь",
        "порнография",
        "потенция",
        "предикат",
        "предохранитель",
        "придание",
        "придача",
        "природопользование",
        "провидение",
        "провинциал",
        "произношение",
        "путеводитель",
        "разбег",
        "распродажа",
        "рентген",
        "рой",
        "саженец",
        "саксофон",
        "сгорание",
        "сито",
        "скок",
        "смокинг",
        "снегопад",
        "сосредоточение",
        "соучастник",
        "спелеолог",
        "специя",
        "спица",
        "столешница",
        "столп",
        "субстрат",
        "такса",
        "татуировка",
        "торф",
        "транс",
        "трезвость",
        "тупость",
        "убыль",
        "уединение",
        "урон",
        "усопший",
        "устрица",
        "фарисей",
        "функционер",
        "хрип",
        "шах",
        "шлак",
        "шпилька",
        "ювелир",
        "ястреб",
        "ящерица",
        "аббревиатура",
        "агрессор",
        "ангар",
        "ассигнование",
        "афера",
        "бабуся",
        "блюдечко",
        "бурьян",
        "ведущая",
        "видеозапись",
        "виноградник",
        "виолончель",
        "вираж",
        "витраж",
        "водолаз",
        "воодушевление",
        "вылазка",
        "выносливость",
        "высокомерие",
        "вэ",
        "газетчик",
        "гонец",
        "гороскоп",
        "гостинец",
        "демократизация",
        "жезл",
        "жижа",
        "замедление",
        "затмение",
        "защищённость",
        "зуд",
        "излишество",
        "изменник",
        "индус",
        "интеллигентность",
        "интервенция",
        "информатизация",
        "казус",
        "католицизм",
        "кипа",
        "клюква",
        "копоть",
        "кореш",
        "корректива",
        "кубрик",
        "кузина",
        "лаз",
        "лекарь",
        "лексема",
        "лиана",
        "майонез",
        "массовка",
        "махинация",
        "медальон",
        "мессия",
        "миллиардер",
        "мозоль",
        "мсье",
        "наводка",
        "наездник",
        "налаживание",
        "наплыв",
        "насильник",
        "наследование",
        "невесомость",
        "новобранец",
        "ночёвка",
        "окунь",
        "отмель",
        "палитра",
        "парторг",
        "патриархия",
        "передатчик",
        "песнопение",
        "плантация",
        "плательщик",
        "предостережение",
        "представительница",
        "привал",
        "причуда",
        "проницаемость",
        "рассрочка",
        "ратификация",
        "реагент",
        "риф",
        "рубец",
        "садовод",
        "самобытность",
        "самозащита",
        "самоуверенность",
        "сандалия",
        "свержение",
        "седан",
        "сень",
        "слоган",
        "снасть",
        "снотворное",
        "собеседница",
        "старейшина",
        "танцовщица",
        "тиф",
        "траур",
        "трудолюбие",
        "ужесточение",
        "униформа",
        "упругость",
        "фанатизм",
        "фасон",
        "фляга",
        "халтура",
        "хер",
        "цензор",
        "цигарка",
        "чёлка",
        "шлагбаум",
        "шлейф",
        "шмель",
        "щёчка",
        "экскурсовод",
        "экспресс",
        "эпилог",
        "эпос",
        "юго-запад",
        "юнкер",
        "автостоянка",
        "агентура",
        "апелляция",
        "астролог",
        "аттитюд",
        "афганец",
        "бестселлер",
        "бидон",
        "бойня",
        "бордюр",
        "бостон",
        "бронетранспортёр",
        "валик",
        "властелин",
        "враждебность",
        "всхлип",
        "выразительность",
        "газель",
        "голодовка",
        "горчица",
        "гробница",
        "демагогия",
        "диверсификация",
        "дискомфорт",
        "домино",
        "домишко",
        "доплата",
        "единоросс",
        "жетон",
        "загон",
        "зачистка",
        "значительность",
        "изготовитель",
        "изнанка",
        "изумруд",
        "изюм",
        "именины",
        "инициал",
        "искорка",
        "калоша",
        "канистра",
        "ковчег",
        "конфетка",
        "крематорий",
        "кулик",
        "лаборант",
        "лазарет",
        "ледокол",
        "лесопользование",
        "магний",
        "марксист",
        "махорка",
        "мачеха",
        "метафизика",
        "мобильность",
        "морфология",
        "наклейка",
        "насыщение",
        "невидимка",
        "нейтрализация",
        "несение",
        "неустойчивость",
        "нюх",
        "обоснованность",
        "обсерватория",
        "одёжка",
        "одессит",
        "оплошность",
        "оранжерея",
        "отзвук",
        "отрочество",
        "отшельник",
        "ошейник",
        "папаха",
        "первенец",
        "переподготовка",
        "пересказ",
        "периодичность",
        "плеск",
        "поголовье",
        "подпункт",
        "пожарник",
        "полиомиелит",
        "половица",
        "полтинник",
        "получка",
        "поручень",
        "поршень",
        "посиделки",
        "постамент",
        "прачечная",
        "председательство",
        "прикол",
        "припас",
        "пришествие",
        "пробирка",
        "прокол",
        "пропагандист",
        "прорубь",
        "публицистика",
        "ребятки",
        "рецензент",
        "рябь",
        "самоходка",
        "свая",
        "сказание",
        "слиток",
        "сообщник",
        "сопредседатель",
        "сосредоточенность",
        "специфичность",
        "ссадина",
        "стояние",
        "стройматериал",
        "сумасшествие",
        "сюртук",
        "танкер",
        "тембр",
        "теннисист",
        "тора",
        "туманность",
        "турне",
        "тюк",
        "ублюдок",
        "угон",
        "угрызение",
        "универсал",
        "усилитель",
        "ушанка",
        "хобот",
        "ценообразование",
        "чаяние",
        "челнок",
        "чуб",
        "чулан",
        "чуткость",
        "швабра",
        "шельф",
        "шифр",
        "шквал",
        "эгоист",
        "экземляр",
        "эротика",
        "эсэсовец",
        "юбиляр",
        "ящичек",
        "абрикос",
        "аграрий",
        "апофеоз",
        "бандероль",
        "бантик",
        "бард",
        "бирюк",
        "блатной",
        "боеголовка",
        "буклет",
        "вымпел",
        "геноцид",
        "главком",
        "градиент",
        "диверсия",
        "дикость",
        "домысел",
        "желток",
        "идеализм",
        "идиллия",
        "иерарх",
        "изобретательность",
        "инсценировка",
        "ирод",
        "искатель",
        "кабаре",
        "капиталовложение",
        "картер",
        "картотека",
        "каста",
        "кибернетика",
        "конверсия",
        "конфуз",
        "кормилец",
        "котик",
        "кровообращение",
        "кухарка",
        "лагуна",
        "лентяй",
        "ликёр",
        "лоза",
        "мари",
        "масонство",
        "меньшевик",
        "милостыня",
        "моллюск",
        "молодняк",
        "молочко",
        "настойка",
        "настояние",
        "натяжка",
        "недомогание",
        "низина",
        "ноу-хау",
        "оазис",
        "обелиск",
        "обёртка",
        "обманщик",
        "однокашник",
        "ожерелье",
        "оперативность",
        "освидетельствование",
        "оставление",
        "отёк",
        "параша",
        "патефон",
        "пейджер",
        "пёрышко",
        "печёнка",
        "пижон",
        "пицца",
        "плавник",
        "пневмония",
        "побои",
        "подборка",
        "подкрепление",
        "попечитель",
        "поплавок",
        "предводитель",
        "прелюдия",
        "пресс-центр",
        "пробор",
        "протекание",
        "противоборство",
        "профком",
        "пылинка",
        "разбой",
        "разминка",
        "ракетка",
        "рассада",
        "растяжка",
        "ринг",
        "ром",
        "рукоять",
        "связывание",
        "седьмая",
        "скафандр",
        "смерч",
        "смычок",
        "соболь",
        "сокрытие",
        "стимуляция",
        "стужа",
        "толерантность",
        "тропарь",
        "улыбочка",
        "унификация",
        "управленец",
        "фигурист",
        "халява",
        "хронология",
        "хрящ",
        "четвертая",
        "шалость",
        "шейх",
        "шило",
        "шипение",
        "шмотки",
        "шоколадка",
        "щиток",
        "эстонец",
        "агитатор",
        "антиквариат",
        "аспирантка",
        "банкнота",
        "бездарность",
        "бисер",
        "болтун",
        "болячка",
        "брусок",
        "вареник",
        "верзила",
        "вилы",
        "внедорожник",
        "волеизъявление",
        "воссоздание",
        "вояка",
        "высочество",
        "гадина",
        "галифе",
        "гильдия",
        "гитлеровец",
        "годик",
        "горизонталь",
        "городишко",
        "грамотность",
        "грот",
        "дверка",
        "деревенька",
        "донышко",
        "дочурка",
        "еврейство",
        "жесть",
        "завоеватель",
        "задачка",
        "затяжка",
        "захватчик",
        "зимовка",
        "злодеяние",
        "злоумышленник",
        "иеромонах",
        "изваяние",
        "известь",
        "инструктаж",
        "йогурт",
        "кадка",
        "кальсоны",
        "каменщик",
        "квадратик",
        "кладовая",
        "компакт-диск",
        "консенсус",
        "короб",
        "королевич",
        "культивирование",
        "кум",
        "латыш",
        "лукавство",
        "малютка",
        "манекен",
        "медитация",
        "металлолом",
        "меценат",
        "миротворец",
        "младенчество",
        "мурашки",
        "надстройка",
        "нажатие",
        "накидка",
        "начисление",
        "новоселье",
        "норд-ост",
        "овен",
        "однообразие",
        "озеленение",
        "опекун",
        "оплот",
        "оса",
        "освящение",
        "панно",
        "пансион",
        "папироска",
        "парадное",
        "пересчёт",
        "песец",
        "плевок",
        "подведение",
        "подмышка",
        "позволение",
        "посад",
        "потеха",
        "предбанник",
        "привязка",
        "прима",
        "проклятье",
        "прорезь",
        "псалом",
        "радиосвязь",
        "разорение",
        "рассеяние",
        "рафик",
        "реквием",
        "речушка",
        "рус",
        "рыболовство",
        "сарказм",
        "сестричка",
        "сеянец",
        "сноп",
        "соплеменник",
        "сотник",
        "спас",
        "тайм",
        "талисман",
        "тельняшка",
        "темень",
        "тишь",
        "убранство",
        "узнавание",
        "формализм",
        "фюзеляж",
        "хиппи",
        "циник",
        "чернильница",
        "членик",
        "шотландец",
        "экранизация",
        "агония",
        "аист",
        "алан",
        "анализатор",
        "апокалипсис",
        "аптечка",
        "аркан",
        "балаган",
        "берлога",
        "беспечность",
        "бессмысленность",
        "библиотекарь",
        "бормотание",
        "бородач",
        "братан",
        "брод",
        "буржуй",
        "вещмешок",
        "виселица",
        "воссоединение",
        "высылка",
        "вытеснение",
        "вьетнамец",
        "генпрокурор",
        "горстка",
        "дебил",
        "джинн",
        "динозавр",
        "дрессировщик",
        "дробление",
        "дятел",
        "египтянин",
        "живность",
        "задвижка",
        "задворки",
        "замер",
        "зверство",
        "зевака",
        "знакомец",
        "изыск",
        "импортёр",
        "ингибитор",
        "индукция",
        "исповедник",
        "каблучок",
        "капелла",
        "карп",
        "картофелина",
        "керн",
        "классификатор",
        "кожура",
        "кокетство",
        "колба",
        "комиссариат",
        "компетентность",
        "координатор",
        "кореянка",
        "космонавтика",
        "космополит",
        "костяк",
        "кредо",
        "крен",
        "кружочек",
        "крупица",
        "левша",
        "лесть",
        "ликвидность",
        "лимонад",
        "локон",
        "ломка",
        "мандарин",
        "мансарда",
        "маринад",
        "масленица",
        "мелодрама",
        "месиво",
        "метеорит",
        "минимизация",
        "миссис",
        "мудак",
        "мясник",
        "наглядность",
        "наказ",
        "насторожённость",
        "неясность",
        "облава",
        "образованность",
        "обречённость",
        "ом",
        "онанизм",
        "оспаривание",
        "отчаянье",
        "отчизна",
        "парашютист",
        "патриархат",
        "пенсионерка",
        "пеня",
        "пережитое",
        "петербуржец",
        "пловец",
        "подводник",
        "подмостки",
        "покои",
        "полотнище",
        "понос",
        "похититель",
        "правдивость",
        "празднество",
        "примус",
        "просека",
        "проток",
        "психиатрия",
        "пул",
        "разброс",
        "реалист",
        "репликация",
        "родич",
        "сердечко",
        "сибиряк",
        "случившееся",
        "смертник",
        "собирание",
        "совок",
        "сосулька",
        "страдалец",
        "страус",
        "существительное",
        "табу",
        "тазик",
        "талиб",
        "тапка",
        "теплица",
        "тир",
        "топ",
        "торпеда",
        "торпедо",
        "триада",
        "триллион",
        "трудоёмкость",
        "тушка",
        "умирающий",
        "усложнение",
        "феникс",
        "фосфор",
        "хохол",
        "шлепанец",
        "щупальце",
        "экспедитор",
        "эрекция",
        "авианосец",
        "автолюбитель",
        "апатия",
        "архангел",
        "атташе",
        "бальзам",
        "бегемот",
        "бесстрашие",
        "бочок",
        "британец",
        "бритье",
        "брокер",
        "вельможа",
        "вол",
        "воскрешение",
        "врачиха",
        "вскрик",
        "выпадение",
        "выселение",
        "высотка",
        "гадание",
        "газетка",
        "газификация",
        "гарем",
        "гвоздик",
        "глазница",
        "глушитель",
        "гниль",
        "горловина",
        "градостроительство",
        "грибок",
        "гудение",
        "густота",
        "детёныш",
        "детина",
        "директриса",
        "доброжелательность",
        "домочадец",
        "доярка",
        "жаворонок",
        "желвак",
        "женственность",
        "жертвоприношение",
        "животик",
        "задержанный",
        "заслон",
        "застенок",
        "изгой",
        "исключительность",
        "истолкование",
        "каламбур",
        "кинозвезда",
        "клиентура",
        "кожица",
        "копна",
        "красавчик",
        "курево",
        "ладан",
        "либретто",
        "линолеум",
        "литье",
        "малышка",
        "маникюр",
        "марафон",
        "марксизм-ленинизм",
        "мерзлота",
        "меридиан",
        "метанол",
        "милочка",
        "многогранник",
        "множественность",
        "модельер",
        "мордочка",
        "наречие",
        "натюрморт",
        "национализация",
        "небрежность",
        "негатив",
        "недопустимость",
        "неуважение",
        "неудовлетворённость",
        "ноутбук",
        "нянечка",
        "обмотка",
        "обшивка",
        "однокурсник",
        "одышка",
        "околица",
        "ополчение",
        "отнесение",
        "паломничество",
        "первоисточник",
        "перебежчик",
        "передник",
        "переполох",
        "песчинка",
        "петлица",
        "пивко",
        "плато",
        "плаха",
        "подъёмник",
        "пойма",
        "полметра",
        "полукруг",
        "полярник",
        "попечительство",
        "поступь",
        "правнук",
        "преобразователь",
        "приданое",
        "проводка",
        "проводница",
        "проволочка",
        "программка",
        "пуговка",
        "районирование",
        "ранка",
        "распорядитель",
        "распространитель",
        "расцветка",
        "рациональность",
        "редактирование",
        "резец",
        "риэлтор",
        "рогатка",
        "рок-н-ролл",
        "ряса",
        "сажа",
        "самообладание",
        "самородок",
        "сантехник",
        "святейшество",
        "склока",
        "служанка",
        "смотритель",
        "снисхождение",
        "собачонка",
        "соломка",
        "сопли",
        "столбец",
        "судак",
        "табло",
        "таджик",
        "тарелочка",
        "темя",
        "тент",
        "терем",
        "товарооборот",
        "толчея",
        "тоник",
        "топ-менеджер",
        "торжественность",
        "трясина",
        "увечье",
        "уик-энд",
        "укрупнение",
        "улитка",
        "умолчание",
        "утроба",
        "фасоль",
        "флотилия",
        "централизация",
        "частичка",
        "черёмуха",
        "чёрточка",
        "шажок",
        "шутник",
        "щебень",
        "абсолют",
        "автоответчик",
        "антропология",
        "безответственность",
        "бессмыслица",
        "блестка",
        "бубен",
        "бургомистр",
        "бытность",
        "вахтенный",
        "веление",
        "великодушие",
        "верование",
        "вознесение",
        "вредительство",
        "гамак",
        "гистограмма",
        "горесть",
        "гортань",
        "грация",
        "грызун",
        "гуманист",
        "движок",
        "деточка",
        "диалект",
        "дилемма",
        "доверчивость",
        "доминирование",
        "жабра",
        "живость",
        "жизнеспособность",
        "жэк",
        "завиток",
        "заминка",
        "зануда",
        "застёжка",
        "злорадство",
        "зодчий",
        "индивидуализм",
        "индиец",
        "исполнительница",
        "кадет",
        "канарейка",
        "концовка",
        "костяшка",
        "крахмал",
        "крестьянка",
        "крутизна",
        "кулёк",
        "кулуары",
        "кураж",
        "ландыш",
        "летописец",
        "лупа",
        "люлька",
        "маразм",
        "маузер",
        "метание",
        "мироощущение",
        "надгробие",
        "недействительность",
        "недоброжелатель",
        "непрерывность",
        "обивка",
        "облучение",
        "обожание",
        "обслуга",
        "обязательность",
        "осведомлённость",
        "остов",
        "откат",
        "очерёдность",
        "падеж",
        "пальба",
        "парапет",
        "партбилет",
        "перевязка",
        "пережиток",
        "переименование",
        "перекличка",
        "переплетение",
        "писарь",
        "плав",
        "плащ-палатка",
        "подсвечник",
        "покрой",
        "почитание",
        "предвидение",
        "пропитание",
        "проректор",
        "протестант",
        "прыщ",
        "пушок",
        "пчёлка",
        "разлад",
        "расческа",
        "репетитор",
        "реферат",
        "рецептура",
        "самиздат",
        "связной",
        "сгиб",
        "сдерживание",
        "сексуальность",
        "сепаратист",
        "сестрица",
        "синица",
        "скверик",
        "скитание",
        "скольжение",
        "совместимость",
        "солярка",
        "сом",
        "ставень",
        "стриптиз",
        "сундучок",
        "сучка",
        "теплоснабжение",
        "тётенька",
        "тор",
        "тягач",
        "уазик",
        "угода",
        "уплотнение",
        "ухищрение",
        "фабула",
        "фальшивка",
        "федерализм",
        "фотокарточка",
        "фугас",
        "хворост",
        "чурка",
        "шарм",
        "швейцарец",
        "штабс-капитан",
        "шток",
        "эколог",
        "эритроцит",
        "автоспорт",
        "адепт",
        "адмиралтейство",
        "адреналин",
        "аммиак",
        "амортизация",
        "археология",
        "астма",
        "атеизм",
        "береста",
        "бесплодие",
        "бочонок",
        "бронежилет",
        "бронь",
        "брус",
        "букетик",
        "вазочка",
        "вермахт",
        "видеомагнитофон",
        "внезапность",
        "водила",
        "вотчина",
        "вояж",
        "вымогательство",
        "вьюга",
        "гитарист",
        "голубизна",
        "гомосексуалист",
        "госслужащий",
        "девчушка",
        "диакон",
        "диафрагма",
        "дискета",
        "дите",
        "доведение",
        "загривок",
        "задор",
        "злодейство",
        "изотоп",
        "изюминка",
        "инок",
        "каньон",
        "капрал",
        "карась",
        "картонка",
        "кварц",
        "кикимора",
        "кинетика",
        "козерог",
        "колбаска",
        "комдив",
        "кондуктор",
        "коровник",
        "кочерга",
        "красноречие",
        "кроссовок",
        "кьянти",
        "лама",
        "лесок",
        "лесосека",
        "лисица",
        "логово",
        "лыжник",
        "лязг",
        "мазок",
        "малолетка",
        "марля",
        "мастерица",
        "медпункт",
        "меморандум",
        "механизатор",
        "мигалка",
        "миссионер",
        "можжевельник",
        "мораторий",
        "мундштук",
        "нагромождение",
        "нарта",
        "наружность",
        "невроз",
        "неровность",
        "неурядица",
        "нигилист",
        "новатор",
        "нуклеотид",
        "обезьянка",
        "оборонка",
        "огласка",
        "ожесточение",
        "открытая",
        "партиец",
        "парусник",
        "пахан",
        "пессимизм",
        "побоище",
        "побратим",
        "повелитель",
        "повстанец",
        "позолота",
        "полминуты",
        "полуфинал",
        "помёт",
        "помилование",
        "поучение",
        "практик",
        "прерогатива",
        "прибежище",
        "пристройка",
        "пробоина",
        "прогон",
        "происки",
        "пятиэтажка",
        "радиограмма",
        "развилка",
        "раздор",
        "разнос",
        "расселение",
        "расслабление",
        "расщепление",
        "ремарка",
        "рецидив",
        "рутина",
        "сага",
        "самоутверждение",
        "семестр",
        "советчик",
        "соискатель",
        "сонет",
        "соучастие",
        "софа",
        "спящий",
        "стульчик",
        "сход",
        "табун",
        "телка",
        "тирада",
        "транзистор",
        "трансформатор",
        "трёшка",
        "триллер",
        "тщательность",
        "умозаключение",
        "универсальность",
        "фрегат",
        "хлыст",
        "хореограф",
        "хризантема",
        "хромой",
        "царевна",
        "чад",
        "частокол",
        "чиновничество",
        "эластичность",
        "эмитент",
        "энергоресурсы",
        "эпиграмма",
        "эпицентр",
        "юродивый",
        "ямочка",
        "а",
        "абвер",
        "автодорога",
        "адекватность",
        "анархист",
        "анархия",
        "ангина",
        "антихрист",
        "антология",
        "аншлаг",
        "баланда",
        "барокко",
        "бдение",
        "беготня",
        "бедняжка",
        "безделушка",
        "бек",
        "беспризорник",
        "благородие",
        "бракосочетание",
        "брешь",
        "бук",
        "бюстгальтер",
        "валерьянка",
        "вертушка",
        "взимание",
        "вице-спикер",
        "вожделение",
        "воздержание",
        "воинство",
        "гарнитур",
        "гликопротеин",
        "глухой",
        "гнида",
        "гравитация",
        "давка",
        "дан",
        "двойственность",
        "детектор",
        "дискредитация",
        "дистрибьютер",
        "драматизм",
        "дрозд",
        "евангелист",
        "жительница",
        "задумка",
        "закалка",
        "замкнутость",
        "замыкание",
        "зачисление",
        "змейка",
        "извилина",
        "издёвка",
        "импотент",
        "интерферон",
        "исповедание",
        "кадык",
        "капилляр",
        "карантин",
        "кастрюлька",
        "кафель",
        "квартплата",
        "клич",
        "клубень",
        "кобра",
        "конфедерация",
        "копеечка",
        "кощунство",
        "краткость",
        "кроссворд",
        "кроссовка",
        "круговорот",
        "круиз",
        "кузен",
        "кузница",
        "кушак",
        "кювет",
        "лазейка",
        "лакомство",
        "лацкан",
        "лебёдка",
        "лилипут",
        "литера",
        "лихва",
        "лыко",
        "мадемуазель",
        "маляр",
        "манжета",
        "матрёшка",
        "медведица",
        "мечтатель",
        "микроавтобус",
        "мотылёк",
        "мура",
        "наволочка",
        "нагота",
        "надрыв",
        "нажива",
        "наколка",
        "наконечник",
        "натуга",
        "нахал",
        "некролог",
        "немота",
        "неполадка",
        "непривычка",
        "несвобода",
        "несовместимость",
        "обрезок",
        "одноклассница",
        "олицетворение",
        "оракул",
        "оспа",
        "оттиск",
        "оцепление",
        "пассивность",
        "пекарня",
        "переброска",
        "передок",
        "переустройство",
        "пикет",
        "пилюля",
        "питерец",
        "пластырь",
        "плюрализм",
        "подкуп",
        "полиэтилен",
        "полстакана",
        "полустанок",
        "попа",
        "попка",
        "поспешность",
        "посудина",
        "потепление",
        "похоть",
        "почин",
        "предыстория",
        "префектура",
        "приземление",
        "приставка",
        "проблеск",
        "прореха",
        "просёлок",
        "психоз",
        "психотерапия",
        "пятерня",
        "радар",
        "раздвоение",
        "раскрутка",
        "рассылка",
        "регистратор",
        "риза",
        "рокот",
        "румын",
        "рупор",
        "рык",
        "самбо",
        "сачок",
        "сверхдержава",
        "свинство",
        "селезёнка",
        "славянофил",
        "слив",
        "слякоть",
        "собутыльник",
        "сопровождающий",
        "сортировка",
        "средоточие",
        "сруб",
        "столяр",
        "стратег",
        "субботник",
        "сухость",
        "сфинкс",
        "тигрица",
        "томление",
        "торс",
        "трагизм",
        "транспарант",
        "трель",
        "троцкист",
        "тушение",
        "укоризна",
        "улей",
        "успешность",
        "ухаб",
        "учительская",
        "фитиль",
        "хакер",
        "хлев",
        "черепок",
        "чужбина",
        "шифровка",
        "шуршание",
        "экстрасенс",
        "юрта",
        "як",
        "австриец",
        "автовладелец",
        "алиби",
        "аристократия",
        "арфа",
        "архетип",
        "аспирин",
        "африканец",
        "балалайка",
        "бамбук",
        "барашек",
        "баул",
        "бахрома",
        "билетик",
        "биограф",
        "биотехнология",
        "благочестие",
        "бобёр",
        "браунинг",
        "брусника",
        "вдохновитель",
        "ведёрко",
        "вещичка",
        "визитёр",
        "воронок",
        "гадалка",
        "гармонь",
        "гемагглютинин",
        "гимназист",
        "глупец",
        "горняк",
        "гребешок",
        "двусмысленность",
        "дез",
        "дзюдо",
        "дробилка",
        "думец",
        "дуновение",
        "дупло",
        "ельник",
        "еретик",
        "жизнеобеспечение",
        "жираф",
        "заплата",
        "запустение",
        "земледелец",
        "иезуит",
        "известняк",
        "измельчение",
        "имам",
        "инквизиция",
        "кавалерист",
        "кавалерия",
        "капельница",
        "кисет",
        "колдовство",
        "колесница",
        "кольт",
        "компоновка",
        "конференц-зал",
        "кофейня",
        "крупинка",
        "куранты",
        "курильщик",
        "курочка",
        "лесничий",
        "лис",
        "лов",
        "лодочка",
        "лосьон",
        "лужица",
        "малышок",
        "мальчонка",
        "мартышка",
        "меланхолия",
        "мель",
        "мерцание",
        "морж",
        "мучитель",
        "наблюдательность",
        "наводчик",
        "неимение",
        "неприятель",
        "несовпадение",
        "объятье",
        "обыденность",
        "овечка",
        "огрызок",
        "олово",
        "онтология",
        "опала",
        "ориентировка",
        "отпевание",
        "перевоплощение",
        "перелив",
        "перемирие",
        "пианистка",
        "пиршество",
        "пищеварение",
        "плодородие",
        "подлодка",
        "подследственный",
        "покойница",
        "политтехнолог",
        "потроха",
        "правка",
        "пригоршня",
        "прилагательное",
        "принципиальность",
        "причастие",
        "проводимость",
        "проигрыватель",
        "прок",
        "проходимец",
        "психопат",
        "пудель",
        "разглашение",
        "раздражитель",
        "раскат",
        "распря",
        "рассеянность",
        "расслоение",
        "растворение",
        "рать",
        "результативность",
        "религиозность",
        "репа",
        "реставратор",
        "ротный",
        "самурай",
        "санчасть",
        "сатирик",
        "сберкасса",
        "свастика",
        "свидетельница",
        "свиток",
        "седмица",
        "сет",
        "символизм",
        "слизь",
        "снедь",
        "сода",
        "сопля",
        "стерлинг",
        "судимость",
        "сухожилие",
        "счастливец",
        "сынишка",
        "телевизионщик",
        "телепрограмма",
        "терапевт",
        "токарь",
        "требовательность",
        "третье",
        "тюлень",
        "ушиб",
        "фикус",
        "фикция",
        "филология",
        "форвард",
        "форпост",
        "фортуна",
        "фут",
        "харч",
        "холуй",
        "хохолок",
        "чередование",
        "чернь",
        "чудик",
        "чудище",
        "шелкопряд",
        "шлюз",
        "эрозия",
        "ясень",
        "авансцена",
        "автострада",
        "агнец",
        "акробат",
        "англичанка",
        "антипатия",
        "атлет",
        "баба-яга",
        "бабник",
        "балда",
        "барабанщик",
        "бездорожье",
        "безнаказанность",
        "бензоколонка",
        "бестактность",
        "бордель",
        "босоножка",
        "велосипедист",
        "венгр",
        "венчик",
        "вербовка",
        "вернисаж",
        "верстак",
        "вёрстка",
        "верфь",
        "вето",
        "вечерок",
        "вирион",
        "вмятина",
        "вытрезвитель",
        "выучка",
        "галоп",
        "герр",
        "глюкоза",
        "гортензия",
        "горшочек",
        "горячность",
        "госкомитет",
        "государыня",
        "гранат",
        "двадцатка",
        "девственница",
        "делёж",
        "депортация",
        "десятая",
        "доверитель",
        "доминанта",
        "дрёма",
        "дублёр",
        "дудка",
        "дурдом",
        "желоб",
        "желтизна",
        "жернов",
        "жилетка",
        "забегаловка",
        "заварка",
        "заграждение",
        "запал",
        "земство",
        "зёрнышко",
        "злак",
        "знать",
        "ингуш",
        "информирование",
        "кадастр",
        "кармашек",
        "кеп",
        "килька",
        "киноактёр",
        "кирпичик",
        "классицизм",
        "клизма",
        "клише",
        "кокон",
        "кольчуга",
        "консервация",
        "консистенция",
        "констатация",
        "контрреволюция",
        "конура",
        "концепт",
        "конюх",
        "копирование",
        "коромысло",
        "кошёлка",
        "кошечка",
        "креститель",
        "кровопролитие",
        "купон",
        "курилка",
        "курсив",
        "курьёз",
        "курятник",
        "леденец",
        "лепта",
        "литовец",
        "маркировка",
        "ментальность",
        "мерин",
        "мистерия",
        "мотоциклист",
        "назидание",
        "напутствие",
        "неверующий",
        "недропользование",
        "неузнаваемость",
        "неявка",
        "одиссея",
        "однородность",
        "оповещение",
        "особист",
        "отгрузка",
        "отсталость",
        "паз",
        "парторганизация",
        "первоклассник",
        "перегар",
        "перекос",
        "перл",
        "песочница",
        "пиджачок",
        "пихта",
        "платина",
        "плечико",
        "повинность",
        "подогрев",
        "подпоручик",
        "подтяжка",
        "понт",
        "поросль",
        "прабабушка",
        "президентство",
        "преклонение",
        "приказчик",
        "приправа",
        "присказка",
        "протока",
        "психотерапевт",
        "пулеметчик",
        "пунктир",
        "пуп",
        "пятая",
        "радиоизлучение",
        "райисполком",
        "садист",
        "садоводство",
        "саквояж",
        "самогонка",
        "саркофаг",
        "сектант",
        "сельдерей",
        "сервант",
        "синтаксис",
        "скепсис",
        "склероз",
        "смягчение",
        "собиратель",
        "сопляк",
        "сперматозоид",
        "стезя",
        "стёклышко",
        "стенгазета",
        "стоматолог",
        "сторожка",
        "стройность",
        "сужение",
        "сумасшедший",
        "суровость",
        "телосложение",
        "темница",
        "термометр",
        "типология",
        "тиски",
        "товаропроизводитель",
        "топорик",
        "транскрипция",
        "трёп",
        "трио",
        "тростник",
        "тяжба",
        "указка",
        "усталь",
        "усыновитель",
        "фасция",
        "фельдмаршал",
        "фельдфебель",
        "финка",
        "фуфайка",
        "хлорофилл",
        "хозяйственник",
        "хореография",
        "худсовет",
        "целомудрие",
        "черная",
        "черника",
        "чёртик",
        "читательница",
        "шаровары",
        "шиш",
        "эксцесс",
        "элегантность",
        "электролит",
        "эмульсия",
        "эстакада",
        "эстет",
        "этажерка",
        "юго-восток",
        "яблочник",
        "язычество",
        "яство",
        "адвокатура",
        "администрирование",
        "аккуратность",
        "антипод",
        "апогей",
        "армеец",
        "асимметрия",
        "бакенбарда",
        "бегун",
        "бифштекс",
        "бледность",
        "близорукость",
        "блокпост",
        "бойкот",
        "бомбоубежище",
        "бортик",
        "ботаника",
        "брюнет",
        "буйство",
        "бурка",
        "варварство",
        "взрыватель",
        "виновность",
        "вице-мэр",
        "военком",
        "вожатый",
        "возвышенность",
        "вокал",
        "волчок",
        "вольт",
        "выслуга",
        "гадюка",
        "гам",
        "гауптвахта",
        "генетик",
        "глухота",
        "гобелен",
        "гомосексуализм",
        "горбушка",
        "госаппарат",
        "госпитализация",
        "градусник",
        "гурман",
        "дарение",
        "девальвация",
        "девственность",
        "делание",
        "демонтаж",
        "довершение",
        "докторша",
        "долговечность",
        "домен",
        "дурень",
        "жакет",
        "задник",
        "замглавы",
        "замначальника",
        "замораживание",
        "зарисовка",
        "заслонка",
        "зубок",
        "императив",
        "импресарио",
        "ирис",
        "ирландец",
        "испарина",
        "исправность",
        "калькулятор",
        "капустник",
        "картуз",
        "кают-компания",
        "квазар",
        "кед",
        "кипарис",
        "кирка",
        "книжник",
        "кожух",
        "козни",
        "колобок",
        "колпачок",
        "конница",
        "контрагент",
        "контратака",
        "конферансье",
        "коньячок",
        "коренья",
        "коридорчик",
        "корневище",
        "корректность",
        "космополитизм",
        "кочевник",
        "крепыш",
        "курд",
        "лейтмотив",
        "лесоповал",
        "лесоустройство",
        "лечебница",
        "литинститут",
        "ловец",
        "лодочник",
        "лоскуток",
        "марионетка",
        "махина",
        "медиа",
        "медсанбат",
        "метаданные",
        "мистификация",
        "мразь",
        "мытарство",
        "наниматель",
        "насадка",
        "невежда",
        "недопущение",
        "нектар",
        "неохота",
        "нераспространение",
        "неуплата",
        "норвежец",
        "облицовка",
        "одолжение",
        "олигархия",
        "омоновец",
        "оперуполномоченный",
        "оплеуха",
        "опознание",
        "опция",
        "орава",
        "оргия",
        "орфография",
        "орхидея",
        "отвёртка",
        "павлин",
        "паводок",
        "пантеон",
        "пасьянс",
        "пепелище",
        "передовик",
        "переключатель",
        "перина",
        "периодика",
        "пиявка",
        "повариха",
        "поверенный",
        "подачка",
        "подельник",
        "подмножество",
        "подписчик",
        "подсолнух",
        "пожитки",
        "политотдел",
        "полуторка",
        "полынья",
        "портье",
        "порука",
        "поручительство",
        "послесловие",
        "посредничество",
        "потерпевшая",
        "проницательность",
        "просветление",
        "проситель",
        "прославление",
        "простак",
        "противоречивость",
        "пружинка",
        "пряность",
        "психоаналитик",
        "пядь",
        "раба",
        "розница",
        "ротация",
        "ротор",
        "саботаж",
        "сарайчик",
        "сборщик",
        "свояк",
        "сентенция",
        "скарб",
        "скит",
        "сладкое",
        "слепец",
        "снисходительность",
        "совладелец",
        "сокурсник",
        "сонм",
        "социал-демократ",
        "соцреализм",
        "спецификация",
        "спецподразделение",
        "срам",
        "старинка",
        "стремя",
        "ступа",
        "субординация",
        "сударыня",
        "таинственность",
        "тамада",
        "текстиль",
        "теплушка",
        "тик",
        "тождество",
        "торговка",
        "труха",
        "угар",
        "угнетение",
        "угодник",
        "удаль",
        "утеха",
        "ухаживание",
        "федерал",
        "ферзь",
        "формализация",
        "фраер",
        "френч",
        "халатность",
        "ходок",
        "хоромы",
        "целлофан",
        "часок",
        "чудотворец",
        "чужое",
        "штанишки",
        "шумок",
        "эклектика",
        "электропередача",
        "эмоциональность",
        "энергосистема",
        "эпидемиология",
        "ягнёнок",
        "яйцеклетка",
        "яранга",
        "авеню",
        "авиабаза",
        "авиалиния",
        "автореферат",
        "азиат",
        "актант",
        "аллегория",
        "амбразура",
        "амплификация",
        "антресоли",
        "атлетика",
        "бабёнка",
        "барельеф",
        "ботва",
        "брожение",
        "буйвол",
        "валет",
        "варево",
        "ватага",
        "ваххабит",
        "весёлость",
        "вливание",
        "вместимость",
        "волновод",
        "всходы",
        "генезис",
        "генералиссимус",
        "гениталии",
        "герц",
        "гомон",
        "горошина",
        "горсовет",
        "грач",
        "громкость",
        "деревяшка",
        "диспансер",
        "дисперсия",
        "диффузия",
        "доброжелатель",
        "добросовестность",
        "докука",
        "допинг",
        "дыбы",
        "жало",
        "жасмин",
        "занос",
        "западник",
        "западня",
        "записочка",
        "звездочёт",
        "зодчество",
        "иголочка",
        "извержение",
        "излечение",
        "импотенция",
        "интенсификация",
        "интернационализм",
        "искупление",
        "казначейство",
        "калачик",
        "калейдоскоп",
        "канонизация",
        "командарм",
        "комбриг",
        "комик",
        "комп",
        "комсорг",
        "конкретность",
        "контроллер",
        "контузия",
        "конфиденциальность",
        "коралл",
        "коршун",
        "косметолог",
        "костерок",
        "лайка",
        "легитимность",
        "лепка",
        "литературоведение",
        "личина",
        "лоббирование",
        "лоцман",
        "людишки",
        "мастер-класс",
        "медлительность",
        "месье",
        "метелица",
        "метрополия",
        "могильщик",
        "мольберт",
        "мошка",
        "мускулатура",
        "мышеловка",
        "нагревание",
        "накат",
        "напев",
        "нахальство",
        "нацизм",
        "непорядок",
        "непредсказуемость",
        "нервозность",
        "облачение",
        "односельчанин",
        "однофамилец",
        "окрас",
        "оливье",
        "оплодотворение",
        "опросник",
        "ординарец",
        "осечка",
        "отбросы",
        "отпущение",
        "отторжение",
        "очкарик",
        "пайщик",
        "пани",
        "пас",
        "пекло",
        "пенис",
        "первооткрыватель",
        "переборка",
        "перекур",
        "перелесок",
        "перепонка",
        "перхоть",
        "пион",
        "платёжеспособность",
        "плеяда",
        "погост",
        "подкласс",
        "подрост",
        "подручный",
        "постмодернизм",
        "постовой",
        "посыльный",
        "правообладатель",
        "предместье",
        "прейскурант",
        "примат",
        "присуждение",
        "пробежка",
        "продолжатель",
        "промывка",
        "психоанализ",
        "птицефабрика",
        "размежевание",
        "раскрой",
        "расчленение",
        "регулярность",
        "редиска",
        "релаксация",
        "реформация",
        "ропот",
        "ротмистр",
        "ручонка",
        "рыбёшка",
        "самовыражение",
        "самопожертвование",
        "саморегуляция",
        "самоцель",
        "сановник",
        "секвенирование",
        "сельхозпредприятие",
        "сенокос",
        "серость",
        "сигаретка",
        "синьор",
        "сионизм",
        "скальпель",
        "скиф",
        "скрепка",
        "скупка",
        "слезинка",
        "слюни",
        "смекалка",
        "смельчак",
        "совершеннолетие",
        "солярий",
        "сообразительность",
        "спорткомитет",
        "стекольщик",
        "стерилизация",
        "стихира",
        "сытость",
        "талмуд",
        "тампон",
        "телекоммуникация",
        "толика",
        "торшер",
        "трафик",
        "турникет",
        "умирание",
        "ухажёр",
        "фантик",
        "фломастер",
        "хана",
        "химера",
        "холоп",
        "хром",
        "целость",
        "чертовщина",
        "четверостишие",
        "шезлонг",
        "шторка",
        "экипировка",
        "экспромт",
        "электровоз",
        "эпителий",
        "эпитоп",
        "ягодка",
        "янтарь",
        "австралиец",
        "акустика",
        "амортизатор",
        "ампер",
        "антураж",
        "аорта",
        "арендодатель",
        "артистизм",
        "архипастырь",
        "аудиенция",
        "аул",
        "аэроплан",
        "балахон",
        "баловство",
        "банкомат",
        "безденежье",
        "бесценок",
        "благоденствие",
        "блуд",
        "боеготовность",
        "боулинг",
        "браконьер",
        "бровка",
        "буддист",
        "бузина",
        "вар",
        "вдовец",
        "весточка",
        "взаимопомощь",
        "взводный",
        "взяточник",
        "взяточничество",
        "визуализация",
        "владелица",
        "вокалист",
        "восьмая",
        "всячина",
        "выделка",
        "вымирание",
        "выпечка",
        "выправка",
        "вяз",
        "вялость",
        "газовик",
        "гак",
        "гелий",
        "геморрой",
        "гипертония",
        "голубка",
        "голубушка",
        "градация",
        "градоначальник",
        "гуж",
        "демонстрант",
        "добывание",
        "дозаправка",
        "дороговизна",
        "доходяга",
        "евнух",
        "ежевика",
        "жестянка",
        "жизнеописание",
        "задел",
        "замашка",
        "затопление",
        "заточение",
        "зачаток",
        "знаменатель",
        "зондирование",
        "играющий",
        "изверг",
        "излом",
        "имаго",
        "иммуноглобулин",
        "инквизитор",
        "иностранка",
        "информатор",
        "калория",
        "кальмар",
        "камзол",
        "кантон",
        "карьерист",
        "касание",
        "катализ",
        "каталка",
        "квант",
        "киль",
        "кимоно",
        "княжич",
        "кок",
        "колышек",
        "кондиция",
        "концессия",
        "копилка",
        "коряга",
        "косогор",
        "кросс",
        "кувалда",
        "кулинария",
        "лещ",
        "лоббист",
        "лощина",
        "лыжня",
        "любимица",
        "мануфактура",
        "маслина",
        "материалист",
        "межа",
        "менструация",
        "металлопродукция",
        "микроэлемент",
        "миндаль",
        "монослой",
        "мочалка",
        "мужичонка",
        "муфта",
        "мята",
        "наличность",
        "нарекание",
        "наследница",
        "насыщенность",
        "находчивость",
        "невыезд",
        "нега",
        "недостача",
        "нелегал",
        "необычность",
        "нигилизм",
        "ножичек",
        "номинал",
        "носильщик",
        "нумерация",
        "обгон",
        "обод",
        "обоняние",
        "обороноспособность",
        "оглашение",
        "окорок",
        "окупаемость",
        "омерзение",
        "опёнок",
        "осведомитель",
        "осетрина",
        "осока",
        "отворот",
        "отморозок",
        "отождествление",
        "отправитель",
        "отсечение",
        "пединститут",
        "пенка",
        "первопроходец",
        "пергамент",
        "перс",
        "перспективность",
        "пигмент",
        "пищевод",
        "плавки",
        "пластилин",
        "пломба",
        "подкормка",
        "подметка",
        "позёмка",
        "поимка",
        "поклёп",
        "покорение",
        "половодье",
        "помои",
        "послабление",
        "посредственность",
        "потуга",
        "правомерность",
        "предоплата",
        "прилёт",
        "примерка",
        "притон",
        "причитание",
        "провожатый",
        "проделка",
        "прожилка",
        "протектор",
        "противень",
        "противопоказание",
        "профицит",
        "пяток",
        "ракета-носитель",
        "рампа",
        "ратник",
        "регламентация",
        "регрессия",
        "родоначальник",
        "рычажок",
        "рядно",
        "самотёк",
        "сверхзадача",
        "святая",
        "селянин",
        "сепаратизм",
        "серьёз",
        "сжигание",
        "симбиоз",
        "систематизация",
        "сканер",
        "скворец",
        "слежение",
        "сноровка",
        "согласная",
        "согласованность",
        "содрогание",
        "сокровищница",
        "спортсменка",
        "спячка",
        "ссыльный",
        "старшой",
        "стимулятор",
        "сыпь",
        "табель",
        "тантра",
        "твердь",
        "теология",
        "тесёмка",
        "типаж",
        "токсин",
        "трубач",
        "трудодень",
        "тряпица",
        "убожество",
        "углевод",
        "удушье",
        "утёнок",
        "ущемление",
        "феодализм",
        "фехтование",
        "фиаско",
        "фольксваген",
        "фора",
        "форель",
        "хартия",
        "холодность",
        "хомут",
        "шевеление",
        "шестидесятник",
        "школа-студия",
        "шлепок",
        "шпора",
        "штатский",
        "щёлка",
        "щёлочка",
        "элеватор",
        "энергоснабжение",
        "юбочка",
        "юморист",
        "ячмень",
        "автосервис",
        "аккредитация",
        "актуализация",
        "албанец",
        "амбал",
        "ампир",
        "аппендицит",
        "аутсайдер",
        "аэростат",
        "балетмейстер",
        "бандитизм",
        "барбарис",
        "безумство",
        "беллетристика",
        "бизнес-план",
        "блеф",
        "блинчик",
        "блокирование",
        "блокировка",
        "бобби",
        "боеспособность",
        "большак",
        "братик",
        "бульдог",
        "быдло",
        "быль",
        "валидол",
        "верховье",
        "винегрет",
        "вобла",
        "возница",
        "волчара",
        "выгрузка",
        "вытяжка",
        "выхлоп",
        "вышина",
        "гарнир",
        "герань",
        "гласная",
        "горн",
        "госдепартамент",
        "госзаказ",
        "готика",
        "гримёр",
        "группка",
        "дедовщина",
        "дееспособность",
        "дезинформация",
        "делишки",
        "деловитость",
        "демарш",
        "демобилизация",
        "диктат",
        "дог",
        "дубненец",
        "дужка",
        "жалюзи",
        "желе",
        "желчь",
        "завистник",
        "заноза",
        "засилье",
        "зелёнка",
        "игнорирование",
        "иконка",
        "инкубация",
        "искоренение",
        "истерия",
        "калач",
        "калька",
        "карбас",
        "кенгуру",
        "клиентка",
        "когорта",
        "кодирование",
        "козлёнок",
        "колдунья",
        "контр-адмирал",
        "кормилица",
        "коровка",
        "корректор",
        "кратер",
        "кудряшка",
        "лаборантка",
        "лагерник",
        "леопард",
        "лепет",
        "липид",
        "лирик",
        "лог",
        "логос",
        "лодыжка",
        "лучик",
        "малец",
        "монтёр",
        "морозец",
        "мотель",
        "мотивировка",
        "мультик",
        "мышонок",
        "наглец",
        "наркота",
        "небылица",
        "неизменность",
        "нейтралитет",
        "некомпетентность",
        "ненадобность",
        "нимб",
        "нимфа",
        "новаторство",
        "новгородец",
        "нонсенс",
        "носорог",
        "нотация",
        "обиняк",
        "обладательница",
        "обогрев",
        "оборванец",
        "обрубок",
        "обуза",
        "оглобля",
        "озорство",
        "оковы",
        "осветитель",
        "осетин",
        "оскал",
        "остервенение",
        "отдушина",
        "откуп",
        "отрада",
        "падаль",
        "пальтишко",
        "памперс",
        "панама",
        "пантера",
        "партбюро",
        "пат",
        "перегон",
        "перепалка",
        "песнописец",
        "пит-стоп",
        "плешь",
        "поверье",
        "подведомственность",
        "подвижка",
        "поджог",
        "поленница",
        "похлёбка",
        "поясок",
        "прагматизм",
        "предгорье",
        "преподавательница",
        "прищур",
        "провозглашение",
        "профи",
        "прыть",
        "пряжа",
        "путы",
        "радикулит",
        "радиотелефон",
        "раритет",
        "ревнитель",
        "рулевой",
        "самодовольство",
        "самоидентификация",
        "самокрутка",
        "сверчок",
        "свора",
        "секундант",
        "селектор",
        "сентиментальность",
        "сеньор",
        "сидор",
        "склянка",
        "снадобье",
        "соглядатай",
        "созвучие",
        "солидность",
        "солистка",
        "сосняк",
        "спецоперация",
        "стабилизатор",
        "старикашка",
        "стекляшка",
        "стеснение",
        "стойло",
        "транспортёр",
        "тренд",
        "тупица",
        "тягость",
        "уверение",
        "упование",
        "управдом",
        "уродец",
        "ускоритель",
        "успеваемость",
        "успение",
        "утреня",
        "фаланга",
        "фантазёр",
        "флагман",
        "фотосинтез",
        "функциональность",
        "хозрасчёт",
        "холестерин",
        "циклон",
        "чайная",
        "чалма",
        "чахотка",
        "челядь",
        "червячок",
        "чернозём",
        "чертополох",
        "шарлатан",
        "шериф",
        "шестая",
        "шлягер",
        "штуковина",
        "шуруп",
        "щетинка",
        "электросвязь",
        "эм",
        "эскимос",
        "юриспруденция",
        "японка",
        "андеграунд",
        "аська",
        "аферист",
        "аэродинамика",
        "аэрозоль",
        "бандформирование",
        "беззащитность",
        "бизнес-процесс",
        "благоразумие",
        "блажь",
        "блюз",
        "большее",
        "бот",
        "брага",
        "вакханалия",
        "вершок",
        "вечеря",
        "водопроводчик",
        "воззвание",
        "волок",
        "волокита",
        "волонтёр",
        "вольер",
        "времяпрепровождение",
        "вуаль",
        "вязь",
        "гангстер",
        "гастролёр",
        "генерал-губернатор",
        "генералитет",
        "германец",
        "гестаповец",
        "глухонемой",
        "глюк",
        "городище",
        "госсекретарь",
        "графоман",
        "гулянка",
        "гуманность",
        "гуру",
        "датчанин",
        "деканат",
        "депеша",
        "десятник",
        "детсад",
        "дешевизна",
        "дирижабль",
        "дисбаланс",
        "долголетие",
        "доносчик",
        "дружочек",
        "дюна",
        "железяка",
        "жигулёнок",
        "жрица",
        "забывчивость",
        "завалинка",
        "закусочная",
        "залегание",
        "заменитель",
        "зарница",
        "застенчивость",
        "здоровяк",
        "землевладелец",
        "изысканность",
        "иракец",
        "испытатель",
        "йог",
        "какао",
        "каменка",
        "канва",
        "капище",
        "карма",
        "католичество",
        "качка",
        "кинопроба",
        "кишлак",
        "коммунальщик",
        "компресс",
        "корица",
        "коронка",
        "кочан",
        "кротость",
        "круча",
        "курортник",
        "лампада",
        "ландграф",
        "лежанка",
        "лесозаготовка",
        "лучина",
        "льготник",
        "любительница",
        "любознательность",
        "магнетизм",
        "малёк",
    ];
    // const withE = ["актёр", "берёза", "бурёнка", "взлёт", "дёрн", "дирижёр", "ёж", "ёлка", "ёмкость", "жёсткость", "зёрнышко", "излёт", "лётчик", "овёс", "огонёк", "орёл", "клёв", "клён", "клёпка", "костёр", "ёжик", "клёпка", "лёд", "лён", "огонёк", "орёл", "осёл", "отвёртка", "паникёр", "пелёнка", "пенёк", "пёрышко", "пёс", "плёнка", "плёс", "поклёвка", "позёмка", "полёт", "приём", "свёкор", "сгущёнка", "сёмга", "съёмка", "телёнок", "утёс", "шофёр", "чёлка", "чёрт", "чёрточка"]
    //
    // const toFix = withE.map(x => [x.replace("ё", "е"), x])
    //   .filter(x => _.includes(freqDictionary, x[0]))
    // console.log("fix", toFix)
    var excludeWords = ["дурак", "труп", "секс", "дура", "идиот", "сука", "го", "тварь", "дерьмо", "проститутка", "урод", "жопа", "изолят", "говно", "церка", "хуй", "блядь", "дурочка", "вич-инфекция", "праймер", "алкаш", "тыща", "лох", "шлюха", "сперма", "прошествие", "харя", "коми", "падла", "субтип", "имя-отчество", "вэ", "хер", "эротика", "эсэсовец", "параша", "пресс-центр", "норд-ост", "рафик", "мудак", "сучка", "чурка", "гомосексуалист", "а", "абвер", "гнида", "мура", "попа", "попка", "дез", "имам", "баба-яга", "герр", "кеп", "паз", "гомосексуализм", "мразь", "оргия", "пенис", "геморрой", "евнух", "быдло", "дубненец", "наркота", "тупица", "уродец", "эм", "аська"];
    Words.words = freqDictionary.filter(function (x) { return excludeWords.indexOf(x) < 0 && x.indexOf("-") < 0; });
    Words.wordsForTask = __WEBPACK_IMPORTED_MODULE_0_lodash__(Words.words).filter(function (x) { return Words.minLength <= x.length && x.length <= Words.maxLength; })
        .take(1000).groupBy(function (x) { return x.length; }).value();
    Words.allWords = Words.words.filter(function (x) { return x.length >= Words.minLength; }).sort();
    function getWords(maxLength) {
        return {
            byLength: Words.wordsForTask,
            words: __WEBPACK_IMPORTED_MODULE_0_lodash__["flatMap"](__WEBPACK_IMPORTED_MODULE_0_lodash__["range"](Words.minLength, maxLength + 1), function (i) { return Words.wordsForTask[i] || []; }),
            minLength: Words.minLength,
            maxLength: maxLength
        };
    }
    Words.getWords = getWords;
})(Words || (Words = {}));


/***/ }),

/***/ 310:
/*!************************************!*\
  !*** ./src/games/utils/mazeUtl.ts ***!
  \************************************/
/*! exports provided: MazeUtl */
/*! exports used: MazeUtl */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return MazeUtl; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator__ = __webpack_require__(/*! @sbj42/maze-generator */ 213);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_backtrack__ = __webpack_require__(/*! @sbj42/maze-generator-backtrack */ 314);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_backtrack___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_backtrack__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__sbj42_maze_generator_core__ = __webpack_require__(/*! @sbj42/maze-generator-core */ 183);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__sbj42_maze_generator_core___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2__sbj42_maze_generator_core__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_core_utils_numbers__ = __webpack_require__(/*! core/utils/numbers */ 31);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_core_utils_tuples__ = __webpack_require__(/*! core/utils/tuples */ 24);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_core_utils_utl__ = __webpack_require__(/*! core/utils/utl */ 212);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_games_utils_unicursalGenerator__ = __webpack_require__(/*! games/utils/unicursalGenerator */ 315);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_lodash__ = __webpack_require__(/*! lodash */ 2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_lodash___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_7_lodash__);








var MazeUtl;
(function (MazeUtl) {
    var dirs = __WEBPACK_IMPORTED_MODULE_2__sbj42_maze_generator_core__["directions"];
    function createLabyrinthPath(w, h) {
        var maze = __WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator__["generate"](w, h, { generator: __WEBPACK_IMPORTED_MODULE_6_games_utils_unicursalGenerator__["a" /* UnicursalGenerator */].generator, originalGenerator: __WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_backtrack__ });
        function getDirs(_a) {
            var x = _a[0], y = _a[1];
            return dirs.ALL.filter(function (dir) { return maze.getPassage(x, y, dir); });
        }
        var startPos = Object(__WEBPACK_IMPORTED_MODULE_4_core_utils_tuples__["a" /* tuple */])(0, 0);
        var path = [];
        function generatePath(_a, dir) {
            var x = _a[0], y = _a[1];
            path.push([x, y]);
            var newPos = dirs.move(x, y, dir);
            var newDir = getDirs(newPos).find(function (x) { return x != dirs.opposite(dir); });
            if (!__WEBPACK_IMPORTED_MODULE_7_lodash__["isEqual"](newPos, startPos)) {
                generatePath(newPos, newDir);
            }
        }
        generatePath(startPos, __WEBPACK_IMPORTED_MODULE_7_lodash__["sample"](getDirs(startPos)));
        return path;
    }
    MazeUtl.createLabyrinthPath = createLabyrinthPath;
    function simplePath(rng, w, h) {
        var horizontal = function (reverse) { return __WEBPACK_IMPORTED_MODULE_7_lodash__["flatMap"](__WEBPACK_IMPORTED_MODULE_7_lodash__["range"](h), function (y) {
            var _a = __WEBPACK_IMPORTED_MODULE_5_core_utils_utl__["a" /* Utl */].swap([[0, w], [w - 1, -1]], reverse), right = _a[0], left = _a[1];
            var _b = __WEBPACK_IMPORTED_MODULE_3_core_utils_numbers__["a" /* Numbers */].odd(y) ? right : left, start = _b[0], end = _b[1];
            return __WEBPACK_IMPORTED_MODULE_7_lodash__["range"](start, end).map(function (x) { return Object(__WEBPACK_IMPORTED_MODULE_4_core_utils_tuples__["a" /* tuple */])(x, y); });
        }); };
        var vertical = function (reverse) { return __WEBPACK_IMPORTED_MODULE_7_lodash__["flatMap"](__WEBPACK_IMPORTED_MODULE_7_lodash__["range"](w), function (x) {
            var _a = __WEBPACK_IMPORTED_MODULE_5_core_utils_utl__["a" /* Utl */].swap([[0, h], [h - 1, -1]], reverse), right = _a[0], left = _a[1];
            var _b = __WEBPACK_IMPORTED_MODULE_3_core_utils_numbers__["a" /* Numbers */].odd(x) ? right : left, start = _b[0], end = _b[1];
            return __WEBPACK_IMPORTED_MODULE_7_lodash__["range"](start, end).map(function (y) { return Object(__WEBPACK_IMPORTED_MODULE_4_core_utils_tuples__["a" /* tuple */])(x, y); });
        }); };
        var generators = [function () { return vertical(true); }, function () { return vertical(false); }];
        var path = rng.sample(generators)();
        return rng.bool() ? path : __WEBPACK_IMPORTED_MODULE_7_lodash__["reverse"](path);
    }
    MazeUtl.simplePath = simplePath;
})(MazeUtl || (MazeUtl = {}));


/***/ }),

/***/ 311:
/*!************************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator/src/generate.js ***!
  \************************************************************/
/*! dynamic exports provided */
/*! all exports used */
/***/ (function(module, exports, __webpack_require__) {

var Maze = __webpack_require__(/*! @sbj42/maze-generator-core */ 183).Maze;

function generate(width, height, options) {
    var noptions = {};
    for (var x in options)
        noptions[x] = options[x];

    // Default random number generator: Math.random.
    // Math.random isn't seedable, so if you want reproducible
    // results you should pass a different RNG, for instance
    // the following code using the random-js package would work:
    //
    //     var randomjs = require('random-js');
    //     var engine = randomjs.engines.mt19937().seed(seed);
    //     var real = randomjs.real(0, 1);
    //     options.random = function() {
    //         return real(engine);
    //     };
    if (!noptions.random)
        options.random = Math.random;

    var maze = new Maze(width, height);

    options.generator(maze, options);

    return maze;
}

module.exports = generate;


/***/ }),

/***/ 312:
/*!*****************************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator-core/src/GridMask.js ***!
  \*****************************************************************/
/*! dynamic exports provided */
/*! all exports used */
/***/ (function(module, exports) {

/**
 * @typedef {Object} GridMaskOptions
 * @property {boolean} [interior] The value to intialize interior cells to
 * @property {boolean} [exterior] The value to return for exterior cells
 */

/**
 * A GridMask is a rectangular grid of boolean values.
 *
 * @constructor
 * @param {integer} width
 * @param {integer} height
 * @param {GridMaskOptions} options
 */
function GridMask(width, height, options) {
    if (width < 0 || height < 0)
        throw new Error('invalid size: ' + width + 'x' + height);
    options = options || {};
    var interior = false;
    if (options.interior != null)
        interior = !!options.interior;
    this._width = width;
    this._height = height;
    this._exterior = false;
    if (options.exterior != null)
        this._exterior = !!options.exterior;
    this._blockWidth = (width+31) >> 5;
    if (options._grid) {
        this._grid = options._grid;
        return;
    }
    this._grid = new Array(this._blockWidth * height);
    var initBlock = interior ? ~0 : 0;
    for (var i = 0; i < this._blockWidth * height; i ++) {
        this._grid[i] = initBlock;
    }
}

/**
 * The width of the GridMask
 *
 * @return {integer}
 */
GridMask.prototype.width = function() {
    return this._width;
};

/**
 * The height of the GridMask
 *
 * @return {integer}
 */
GridMask.prototype.height = function() {
    return this._height;
};

/**
 * Returns a copy of the GridMask.
 *
 * @return {GridMask}
 */
GridMask.prototype.clone = function() {
    return new GridMask(this.width(), this.height(), {
        exterior: this._exterior,
        _grid: this._grid.slice()
    });
};

/**
 * Returns the boolean value at the specified cell
 *
 * @param {integer} x
 * @param {integer} y
 */
GridMask.prototype.get = function(x, y) {
    if (x < 0 || x >= this._width || y < 0 || y >= this._height) {
        return this._exterior;
    }
    var index = y * this._blockWidth + (x >> 5);
    var mask = 1 << (x & 31);
    return (this._grid[index] & mask) != 0;
};

/**
 * Sets the boolean value at the specified cell.  Throws an
 * error for coordinates that lie outside the grid.
 *
 * @param {integer} x
 * @param {integer} y
 * @param {boolean} [value=true]
 */
GridMask.prototype.set = function(x, y, value) {
    if (value == null)
        value = true;
    if (x < 0 || x >= this._width || y < 0 || y >= this._height) {
        throw new Error('cell out of bounds: ' + x + ',' + y);
    }
    var index = y * this._blockWidth + (x >> 5);
    var mask = 1 << (x & 31);
    if (value)
        this._grid[index] |= mask;
    else
        this._grid[index] &= ~mask;
    return this;
};

/**
 * ...
 *
 * @param {integer} x
 * @param {integer} y
 * @param {boolean} [value=true]
 */
GridMask.prototype.testAndSet = function(x, y, value) {
    if (value == null)
        value = true;
    if (x < 0 || x >= this._width || y < 0 || y >= this._height) {
        if (this._exterior == value)
            return false;
        throw new Error('cell out of bounds: ' + x + ',' + y);
    }
    var index = y * this._blockWidth + (x >> 5);
    var mask = 1 << (x & 31);
    if (((this._grid[index] & mask) != 0) == value)
        return false;
    if (value)
        this._grid[index] |= mask;
    else
        this._grid[index] &= ~mask;
    return true;
};

module.exports = GridMask;


/***/ }),

/***/ 313:
/*!*************************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator-core/src/Maze.js ***!
  \*************************************************************/
/*! dynamic exports provided */
/*! all exports used */
/***/ (function(module, exports, __webpack_require__) {

var dirs = __webpack_require__(/*! ./directions */ 214);

/**
 * A Cell is a wrapper object that makes it easier to
 * ask for passage information by name.
 *
 * @constructor
 * @private
 * @param {integer} width
 * @param {integer} height
 */
function Cell(north, east, south, west) {
    this._north = north;
    this._east = east;
    this._south = south;
    this._west = west;
}

Cell.prototype.north = function() {
    return this._north;
};

Cell.prototype.west = function() {
    return this._west;
};

Cell.prototype.south = function() {
    return this._south;
};

Cell.prototype.east = function() {
    return this._east;
};

/**
 * A Maze is a rectangular grid of cells, where each cell
 * may have passages in each of the cardinal directions.
 * The maze is initialized with each cell having no passages.
 *
 * @constructor
 * @param {integer} width
 * @param {integer} height
 */
function Maze(width, height) {
    if (width < 0 || height < 0)
        throw new Error('invalid size: ' + width + 'x' + height);
    this._width = width;
    this._height = height;
    this._blockWidth = ((width+1)+15) >> 4;
    this._grid = new Array(this._blockWidth * (height + 1));
    for (var i = 0; i < this._blockWidth * (height + 1); i ++)
        this._grid[i] = 0;
}

/**
 * The width of the Maze
 *
 * @return {integer}
 */
Maze.prototype.width = function() {
    return this._width;
};

/**
 * The height of the Maze
 *
 * @return {integer}
 */
Maze.prototype.height = function() {
    return this._height;
};

// function cellData(grid, blockWidth, x, y) {
//     var index = y * blockWidth + (x >> 3);
//     var shift = ((x & 7) * 4);
//     return (grid[index] >> shift) & 15;
// }
//
// function setCellPassage(grid, blockWidth, x, y, dir, value) {
//     var index = y * blockWidth + (x >> 3);
//     var shift = ((x & 7) * 4);
//     var mask = dirs.bitmask(dir) << shift;
//     if (value)
//         grid[index] |= mask;
//     else
//         grid[index] &= ~mask;
// }

/**
 * Returns the cell at the given position.
 *
 * @param {integer} x
 * @param {integer} y
 * @return {Cell}
 */
Maze.prototype.cell = function(x, y) {
    if (x < 0 || y < 0 || x >= this._width || y >= this._height)
        return new Cell(false, false, false, false);
    var north, east, south, west;
    var index = y * this._blockWidth + (x >> 4);
    var mask = 1 << ((x & 15) * 2);
    north = (this._grid[index] & mask) != 0;
    west = (this._grid[index] & (mask << 1)) != 0;
    south = (this._grid[index + this._blockWidth] & mask) != 0;
    var x2 = x + 1;
    var index2 = y * this._blockWidth + (x2 >> 4);
    var mask2 = 1 << ((x2 & 15) * 2);
    east = (this._grid[index2] & (mask2 << 1)) != 0;
    return new Cell(north, east, south, west);
};

/**
 * Returns whether there is a passage at the given position and
 * direction
 *
 * @param {integer} x
 * @param {integer} y
 * @param {Direction} dir
 */
Maze.prototype.getPassage = function(x, y, dir) {
    if (x < 0 || y < 0 || x >= this._width || y >= this._height)
        return false;
    var index, mask;
    if (dir != dirs.EAST) {
        index = y * this._blockWidth + (x >> 4);
        mask = 1 << ((x & 15) * 2);
        if (dir == dirs.NORTH)
            /* pass through */;
        else if (dir == dirs.WEST)
            mask <<= 1;
        else
            index += this._blockWidth;
    } else {
        index = y * this._blockWidth + ((x + 1) >> 4);
        mask = 1 << (((x + 1) & 15) * 2 + 1);
    }
    return (this._grid[index] & mask) != 0;
};

/**
 * Creates or removes a passage at the given position and
 * direction.  Note that this also creates the corresponding
 * passage in the neighboring cell.
 *
 * @param {integer} x
 * @param {integer} y
 * @param {Direction} dir
 * @param {boolean} value
 */
Maze.prototype.setPassage = function(x, y, dir, value) {
    if (value == null)
        value = true;
    var index, mask;
    if (dir != dirs.EAST) {
        index = y * this._blockWidth + (x >> 4);
        mask = 1 << ((x & 15) * 2);
        if (dir == dirs.NORTH) {
            if (x < 0 || y < 1 || x >= this._width || y >= this._height)
                throw new Error('passage out of bounds: ' + x + ',' + y + ' dir=' + dir);
        } else if (dir == dirs.WEST) {
            if (x < 1 || y < 0 || x >= this._width || y >= this._height)
                throw new Error('passage out of bounds: ' + x + ',' + y + ' dir=' + dir);
            mask <<= 1;
        } else {
            if (x < 0 || y < 0 || x >= this._width || y >= this._height - 1)
                throw new Error('passage out of bounds: ' + x + ',' + y + ' dir=' + dir);
            index += this._blockWidth;
        }
    } else {
        if (x < 0 || y < 0 || x >= this._width - 1 || y >= this._height)
            throw new Error('passage out of bounds: ' + x + ',' + y + ' dir=' + dir);
        index = y * this._blockWidth + ((x + 1) >> 4);
        mask = 1 << (((x + 1) & 15) * 2 + 1);
    }
    if (value)
        this._grid[index] |= mask;
    else
        this._grid[index] &= ~mask;
};

module.exports = Maze;


/***/ }),

/***/ 314:
/*!***********************************************************************!*\
  !*** ./node_modules/@sbj42/maze-generator-backtrack/src/backtrack.js ***!
  \***********************************************************************/
/*! dynamic exports provided */
/*! all exports used */
/***/ (function(module, exports, __webpack_require__) {

var core = __webpack_require__(/*! @sbj42/maze-generator-core */ 183);
var GridMask = core.GridMask;
var dirs = core.directions;

/**
 * @typedef {Object} MGOptions
 * @property {Function} random Returns a random float between 0 (inclusive) and 1 (exclusive)
 */

/**
 * Chooses a random integer between 0 (inclusive) and max (exclusive)
 *
 * @param {MGOptions} options
 * @param {integer} max
 */
function randomInt(options, max) {
    return Math.floor(options.random() * max);
}

/**
 * Chooses a random element from an array
 *
 * @param {MGOptions} options
 * @param {Array} array
 */
function randomChoice(options, array) {
    if (array.length == 1)
        return array[0];
    return array[randomInt(options, array.length)];
}

/**
 * Returns an array of directions toward neighboring
 * cells that have not been visited (i.e. cells for
 * which visited.get returns false).
 *
 * @param {MGOptions} options
 * @param {GridMask} visited
 * @param {integer[]} pos
 */
function getUnvisitedDirections(options, visited, pos) {
    var ret = [];
    if (!visited.get(pos[0], pos[1] - 1) && (!options.mask || options.mask.get(pos[0], pos[1] - 1)))
        ret.push(dirs.NORTH);
    if (!visited.get(pos[0] + 1, pos[1]) && (!options.mask || options.mask.get(pos[0] + 1, pos[1])))
        ret.push(dirs.EAST);
    if (!visited.get(pos[0], pos[1] + 1) && (!options.mask || options.mask.get(pos[0], pos[1] + 1)))
        ret.push(dirs.SOUTH);
    if (!visited.get(pos[0] - 1, pos[1]) && (!options.mask || options.mask.get(pos[0] - 1, pos[1])))
        ret.push(dirs.WEST);
    return ret;
}

/**
 * Recursive-backtracking maze generator.
 *
 * @param {Maze} maze
 * @param {MGOptions} options
 */
function backtrack(maze, options) {
    var width = maze.width();
    var height = maze.height();

    // The 'visited' mask marks cells that have been added to the maze
    // We use this to see which neighbors are available for connecting
    // with a passage.  To avoid making a passage to the outside of
    // the grid, we mark the exterior cells as if they were already
    // included.
    var visited = new GridMask(width, height, {exterior: true});

    var cur;
    // Start with a random cell in the grid
    do {
        cur = [randomInt(options, width), randomInt(options, height)];
    } while (options.mask && !options.mask.get(cur[0], cur[1]));
    // Mark the first cell as visited
    visited.set(cur[0], cur[1], true);
    // This array will hold the cells that we've moved through,
    // to which we may return later to create a new branch
    var stack = [];

    // Loop until the maze is complete
    for (;;) {

        // Which directions can we go from this cell,
        // that lead to cells that aren't yet in the maze?
        var neighbors = getUnvisitedDirections(options, visited, cur);

        if (neighbors.length) {
            // If there is at least one such direction, pick
            // one at random
            var dir = randomChoice(options, neighbors);
            // If there were other choices, then push the
            // current cell onto the stack.
            if (neighbors.length > 1)
                stack.push(cur);
            // Dig a passage from the curent cell to the next cell
            maze.setPassage(cur[0], cur[1], dir, true);
            // Move on to the next cell
            cur = dirs.move(cur[0], cur[1], dir);
            // Which is now in the maze
            visited.set(cur[0], cur[1], true);
        } else if (stack.length) {
            // If there are no available directions here, but
            // there are still cells in the stack, then pop
            // the last cell off the stack ("backtracking"),
            // and continue from there ("recusrive")
            cur = stack.pop();
        } else {
            // If there are no available directions and the
            // stack is empty, then the maze is complete
            break;
        }
    }
}

backtrack.id = 'backtrack';
backtrack.name = 'recursive-backtracking';
backtrack.features = {
    mask: true
};

module.exports = backtrack;


/***/ }),

/***/ 315:
/*!***********************************************!*\
  !*** ./src/games/utils/unicursalGenerator.ts ***!
  \***********************************************/
/*! exports provided: UnicursalGenerator */
/*! exports used: UnicursalGenerator */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return UnicursalGenerator; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator__ = __webpack_require__(/*! @sbj42/maze-generator */ 213);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_core__ = __webpack_require__(/*! @sbj42/maze-generator-core */ 183);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_core___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_core__);


var UnicursalGenerator;
(function (UnicursalGenerator) {
    var dirs = __WEBPACK_IMPORTED_MODULE_1__sbj42_maze_generator_core__["directions"];
    function randomInt(options, max) {
        return Math.floor(options.random() * max);
    }
    function generator(maze, options) {
        options.generator = options.originalGenerator;
        var originalMaze = __WEBPACK_IMPORTED_MODULE_0__sbj42_maze_generator__["generate"](Math.floor(maze.width() / 2), Math.floor(maze.height() / 2), options);
        function setPassage(x, y, dir, closed) {
            try {
                return maze.setPassage(x, y, dir, closed);
            }
            catch (e) {
            }
        }
        for (var y = 0; y < originalMaze.height(); y++) {
            for (var x = 0; x < originalMaze.width(); x++) {
                var cell = originalMaze.cell(x, y);
                setPassage(x * 2, y * 2, dirs.NORTH, cell.north());
                setPassage((x * 2) + 1, y * 2, dirs.NORTH, cell.north());
                setPassage((x * 2) + 1, y * 2, dirs.EAST, cell.east());
                setPassage((x * 2) + 1, (y * 2) + 1, dirs.EAST, cell.east());
                setPassage((x * 2), (y * 2) + 1, dirs.SOUTH, cell.south());
                setPassage((x * 2) + 1, (y * 2) + 1, dirs.SOUTH, cell.south());
                setPassage(x * 2, y * 2, dirs.WEST, cell.west());
                setPassage(x * 2, (y * 2) + 1, dirs.WEST, cell.west());
                setPassage(x * 2, y * 2, dirs.EAST, !cell.north());
                setPassage((x * 2) + 1, y * 2, dirs.SOUTH, !cell.east());
                setPassage(x * 2, (y * 2) + 1, dirs.EAST, !cell.south());
                setPassage(x * 2, y * 2, dirs.SOUTH, !cell.west());
            }
        }
    }
    UnicursalGenerator.generator = generator;
})(UnicursalGenerator || (UnicursalGenerator = {}));


/***/ })

});
//# sourceMappingURL=3.bundle.js.map